...
function check(s, diagnostics) {
  requireAll(diagnostics, FILES.workflow, s.workflow, [
    ...
    'V3/project Lisp, Board open task summary, event tail, provider durable logs, recent commits, worker telemetry, and final convergence static snapshot',
    'commit-Lisp drift',
    'legacy direct workers',
  ...
  requireAll(diagnostics, FILES.runtime, s.runtime, [
    ...
    'read_recent_commits',
    'build_findings',
    'commit-lisp-drift-loop',
    'legacy-direct-worker-dual-track',
    'pty-diagnostic-only-audit',
    'lisp-density-review',
    'create_low_risk_followup_if_needed',
    'auto_execute: Some(false)',
    'hidden: Some(false)',
    'status_snapshot',
    'nightly-evolution-report',
    'findings_include_fixed_master_audit_topics',
  ]);
