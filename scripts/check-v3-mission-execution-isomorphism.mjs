...
agent_execution.rs runtime is deliberately split into three V3 surfaces:
  ...
  - agent_execution/log_deviation.rs, log_decision.rs, and log_issue.rs:
    governance action writers plus matching live event projection
  ...
  - agent_execution/completion_records.rs: completion record parser shared by
    log/status/audit/preflight surfaces
  - agent_execution/completion_durability.rs: dashboard durability projection
    over parsed completion records
  ...
  - agent_execution/completion_audit_findings.rs: pure audit findings over
    balanced Lisp, id counters, claims, issues, and scoped commits
  - agent_execution/completion_id_audit.rs: duplicate completion/log ID audit
  - agent_execution/completion_handoff_audit.rs: scoped commit handoff findings
  - agent_execution/completion_contract_gate.rs: task-contract completion gate
  - agent_execution/completion_gates.rs: scoped-commit gate plus compatibility
    re-exports for split completion gates
  ...
  - agent_execution/preflight_contract.rs: task-contract projection onto the
    preflight response, including drift promotion and next_step override
  - agent_execution/preflight_contract_scope.rs: task-contract scope loader
    and pure write-scope / must-not-touch projection
  - agent_execution/preflight_cwd.rs: canonical cwd projection and project
    boundary rejection for preflight
  - agent_execution/preflight_trace.rs: opt-in preflight observation projection
    into the task session-trace
...
const DEFAULT_FILES = {
  ...
  logIssue: 'crates/missiond-daemon/src/handlers/knowledge/agent_execution/log_issue.rs',
  logList: 'crates/missiond-daemon/src/handlers/knowledge/agent_execution/log_list.rs',
  logOpen: 'crates/missiond-daemon/src/handlers/knowledge/agent_execution/log_open.rs',
...
const SURFACES = [
  {
    ...
    noteNeedles: ['agent_execution/log_store.rs', 'agent_execution/log_mutation.rs', 'agent_execution/log_paths.rs', 'agent_execution/log_template.rs', 'agent_execution/log_dispatch.rs', 'agent_execution/log_counters.rs', 'agent_execution/log_governance.rs', 'agent_execution/log_deviation.rs', 'agent_execution/log_decision.rs', 'agent_execution/log_issue.rs', 'agent_execution/log_open.rs', 'agent_execution/log_list.rs', 'agent_execution/log_status.rs', 'agent_execution/lisp_syntax.rs', 'agent_execution/lisp_syntax_node.rs', 'agent_execution/lisp_syntax_balance.rs', 'emit_execution_event', 'agent_execution/session_trace.rs', 'agent_execution/session_trace_event.rs'],
  },
  {
...
const DAEMON_NEEDLES = [
  ...
  'use self::completion_durability',
...
const LOG_GOVERNANCE_NEEDLES = [
  'pub(super) use super::log_deviation::action_deviate',
  'pub(super) use super::log_decision::action_decide',
  'pub(super) use super::log_issue::action_issue',
];

const LOG_DEVIATION_NEEDLES = [
...
const COMPLETION_INPUTS_NEEDLES = [
  'pub(super) struct CompletionRequest',
  'pub(super) fn parse_completion_request',
  'fn parse_commit_status',
  'fn parse_verifier_status',
  'fn parse_task_run_verifier_status',
  'fn trimmed_string_arg',
  'collect_string_list',
  'normalize_commit_status',
  'normalize_verifier_status',
  'normalize_task_run_verifier_status',
];
...
const COMPLETION_AUDIT_FINDINGS_NEEDLES = [
  'pub(super) struct AuditFindings',
  'pub(super) fn collect_audit_findings',
  'sexp::check_balance',
  'check_id_monotonic',
  'audit_scoped_commit_handoff',
  'stale-claim',
  'claim-overlap',
  'open-issue-no-owner',
];

const COMPLETION_REPAIR_NEEDLES = [
  'pub(super) async fn action_repair',
  'ExecutionEvent::Repaired',
  'insert_id_counters_block',
  'rebuild_derived_indexes',
  'find_claim_node',
];

const COMPLETION_INDEXES_NEEDLES = [
  'pub(super) fn rebuild_derived_indexes',
  'derived-indexes',
  'active-claims',
  'open-issues',
...
const COMPLETION_FIELDS_NEEDLES = [
...
const COMPLETION_DURABILITY_NEEDLES = [
...
const COMPLETION_ID_AUDIT_NEEDLES = [
...
const COMPLETION_HANDOFF_AUDIT_NEEDLES = [
  'pub(super) fn audit_scoped_commit_handoff',
  'parse_completions',
  'FINDING_COMMIT_STATUS_NO_HASH',
  'FINDING_COMMIT_BLOCKED_NO_BLOCKER',
  'FINDING_SCOPED_COMMIT_VIOLATION',
];

const COMPLETION_CONTRACT_GATE_NEEDLES = [
...
const TASK_VERIFIER_AUTO_ARTIFACTS_NEEDLES = [
  'pub(super) fn resolve_verifier_artifact_path',
  'pub(super) fn read_task_contract_artifact',
  'pub(super) fn read_report_artifact',
  'pub(super) fn read_shared_memory_artifact',
  'load_task_contract',
  'TASK_CONTRACT_MALFORMED',
...
const TASK_VERIFIER_PRECONDITIONS_NEEDLES = [
  'pub(super) struct VerifiedCompletionInputs',
  'pub(super) fn require_verified_completion_inputs',
...
const PREFLIGHT_NEEDLES = [
  ...
  'resolve_preflight_inspect_dir',
  'apply_task_contract_projection',
  'append_preflight_trace_if_requested',
];

const PREFLIGHT_CWD_NEEDLES = [
...
const PREFLIGHT_CONTRACT_NEEDLES = [
  'pub(super) fn apply_task_contract_projection',
...
const PREFLIGHT_CONTRACT_SCOPE_NEEDLES = [
...
function checkFiles(root, files) {
  ...
  requireAll(diagnostics, files.logDeviation, sources.logDeviation, LOG_DEVIATION_NEEDLES);
  requireAll(diagnostics, files.logDecision, sources.logDecision, LOG_DECISION_NEEDLES);
  requireAll(diagnostics, files.logIssue, sources.logIssue, LOG_ISSUE_NEEDLES);
  ...
  requireAll(
    diagnostics,
    files.completionAuditFindings,
    sources.completionAuditFindings,
    COMPLETION_AUDIT_FINDINGS_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionRepair,
    sources.completionRepair,
    COMPLETION_REPAIR_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionIndexes,
    sources.completionIndexes,
    COMPLETION_INDEXES_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionInputs,
    sources.completionInputs,
    COMPLETION_INPUTS_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionFields,
    sources.completionFields,
    COMPLETION_FIELDS_NEEDLES,
  );
  requireAll(
    diagnostics,
  ...
  requireAll(
    diagnostics,
    files.completionDurability,
    sources.completionDurability,
    COMPLETION_DURABILITY_NEEDLES,
  );
  requireAll(
    diagnostics,
  ...
  requireAll(
    diagnostics,
    files.completionIdAudit,
    sources.completionIdAudit,
    COMPLETION_ID_AUDIT_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionHandoffAudit,
    sources.completionHandoffAudit,
    COMPLETION_HANDOFF_AUDIT_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.completionContractGate,
    sources.completionContractGate,
    COMPLETION_CONTRACT_GATE_NEEDLES,
  );
  ...
  requireAll(
    diagnostics,
    files.preflightContract,
    sources.preflightContract,
    PREFLIGHT_CONTRACT_NEEDLES,
  );
  requireAll(
    diagnostics,
    files.preflightContractScope,
    sources.preflightContractScope,
    PREFLIGHT_CONTRACT_SCOPE_NEEDLES,
  );
  requireAll(diagnostics, files.preflightCwd, sources.preflightCwd, PREFLIGHT_CWD_NEEDLES);
  requireAll(
    diagnostics,
  ...
  requireAll(diagnostics, files.preflightTrace, sources.preflightTrace, PREFLIGHT_TRACE_NEEDLES);
  ...
  requireAll(
    diagnostics,
    files.taskVerifierAutoArtifacts,
    sources.taskVerifierAutoArtifacts,
    TASK_VERIFIER_AUTO_ARTIFACTS_NEEDLES,
  );
  requireAll(
    diagnostics,
  ...
  requireAll(
    diagnostics,
    files.taskVerifierPreconditions,
...
function runFixtures(json) {
  const goodFiles = {
    ...
    [DEFAULT_FILES.logDeviation]: buildGoodLogDeviation(),
    [DEFAULT_FILES.logDecision]: buildGoodLogDecision(),
    [DEFAULT_FILES.logIssue]: buildGoodLogIssue(),
    ...
    [DEFAULT_FILES.completionHandoffAudit]: buildGoodCompletionHandoffAudit(),
    [DEFAULT_FILES.completionIdAudit]: buildGoodCompletionIdAudit(),
    [DEFAULT_FILES.completionIndexes]: buildGoodCompletionIndexes(),
    [DEFAULT_FILES.completionInputs]: buildGoodCompletionInputs(),
    [DEFAULT_FILES.completionMaintenance]: buildGoodCompletionMaintenance(),
    [DEFAULT_FILES.completionAuditFindings]: buildGoodCompletionAuditFindings(),
    [DEFAULT_FILES.completionRepair]: buildGoodCompletionRepair(),
    [DEFAULT_FILES.completionFields]: buildGoodCompletionFields(),
    [DEFAULT_FILES.completionRecords]: buildGoodCompletionRecords(),
    [DEFAULT_FILES.completionDurability]: buildGoodCompletionDurability(),
    ...
    [DEFAULT_FILES.preflightContract]: buildGoodPreflightContract(),
    [DEFAULT_FILES.preflightContractScope]: buildGoodPreflightContractScope(),
    [DEFAULT_FILES.preflightCwd]: buildGoodPreflightCwd(),
    ...
    [DEFAULT_FILES.preflightTrace]: buildGoodPreflightTrace(),
...
function buildGoodLogPaths() {
  return `pub(super) const COMPANION_DIR: &str = ".missiond/v3/runtime/executions";
pub(super) const LEGACY_COMPANION_DIR: &str = ".missiond/v2";
pub(super) async fn resolve_project_root() {}
pub(super) fn companion_path() {}
pub(super) fn project_or_target_project() {}
...
function buildGoodLogSurface() {
  return `pub(super) use super::log_list::action_list;
pub(super) use super::log_open::action_open;
pub(super) async fn emit_execution_event() {}
`;
}

function buildGoodLogOpen() {
  return `pub(super) async fn action_open() {
  render_canonical_template();
  build_opened_event();
  emit_execution_event();
  TraceKind::Dispatch;
...
function buildGoodLogList() {
  return `pub(super) async fn action_list() {
  parse_claims();
  parse_completions();
  summarize_durability();
  "dispatch_strategy";
...
function buildGoodLogGovernance() {
  return `pub(super) use super::log_deviation::action_deviate;
pub(super) use super::log_decision::action_decide;
pub(super) use super::log_issue::action_issue;
`;
}

function buildGoodLogDeviation() {
...
function buildGoodCompletionInputs() {
  return `pub(super) struct CompletionRequest {}
pub(super) fn parse_completion_request() {
  collect_string_list();
  normalize_commit_status();
  normalize_verifier_status();
...
function buildGoodCompletionIndexes() {
  return `pub(super) fn rebuild_derived_indexes() {
  "derived-indexes";
  "active-claims";
  "open-issues";
  "unresolved-deviations";
...
function buildGoodCompletionMaintenance() {
  return `pub(super) use super::completion_repair::action_repair;
pub(super) async fn action_audit() {
  collect_audit_findings();
  ExecutionEvent::Audited;
  ExecutionEvent::StaleClaim;
...
function buildGoodCompletionAuditFindings() {
  return `pub(super) struct AuditFindings {}
pub(super) fn collect_audit_findings() {
  sexp::check_balance();
  check_id_monotonic();
  audit_scoped_commit_handoff();
...
function buildGoodCompletionRepair() {
  return `pub(super) async fn action_repair() {
  ExecutionEvent::Repaired;
  insert_id_counters_block();
  rebuild_derived_indexes();
  find_claim_node();
...
function buildGoodCompletionFields() {
...
function buildGoodCompletionRecords() {
  return `use super::completion_fields::parse_string_list;
pub(super) struct CompletionRecord {}
pub(super) fn parse_completions() {}
`;
}

function buildGoodCompletionDurability() {
  return `use super::completion_fields::VALID_COMMIT_STATUSES;
use super::completion_records::CompletionRecord;
pub(super) fn summarize_durability() {}
fn canonical_status_str() {}
`;
...
function buildGoodCompletionIdAudit() {
  return `use super::log_counters::Counter;
fn f() { parse_kv_pairs(); }
pub(super) fn check_id_monotonic() {
  "duplicate-id";
}
...
function buildGoodCompletionHandoffAudit() {
  return `pub(super) fn audit_scoped_commit_handoff() {
  parse_completions();
  FINDING_COMMIT_STATUS_NO_HASH;
  FINDING_COMMIT_BLOCKED_NO_BLOCKER;
  FINDING_SCOPED_COMMIT_VIOLATION;
...
function buildGoodCompletionContractGate() {
  return `pub(super) fn enforce_task_contract_completion() {
  "TASK_CONTRACT_REQUIRED";
  "TASK_CONTRACT_MALFORMED";
  "COMMIT_HASH_REQUIRED_FOR_CONTRACT";
  "CLAIM_SCOPE_MISSING";
...
function buildGoodCompletionGates() {
  return `pub(super) use super::completion_contract_gate::enforce_task_contract_completion;
pub(super) use super::completion_handoff_audit::audit_scoped_commit_handoff;
pub(super) use super::completion_id_audit::check_id_monotonic;
pub(super) fn enforce_scoped_commit_completion() {
  "COMMIT_HASH_REQUIRED";
...
function buildGoodPreflightCwd() {
  return `pub(super) fn resolve_preflight_inspect_dir() {
  error_codes::INVALID_PARAM;
  canonicalize();
  "resolves outside the project root";
}
...
function buildGoodPreflightContract() {
  return `pub(super) fn apply_task_contract_projection() {
  evaluate_task_contract_for_preflight();
  "task_contract_status";
  "staged_out_of_scope";
  "staged_forbidden";
...
function buildGoodPreflightContractScope() {
  return `pub(super) fn build_contract_scope_summary() {
  pattern_matches_path();
  "staged_out_of_scope";
  "staged_forbidden";
  "unstaged_in_scope";
...
function buildGoodPreflightTrace() {
  return `pub(super) fn append_preflight_trace_if_requested() {
  resolve_session_trace_path();
  resolve_trace_task_id();
  TraceKind::Observation;
  append_session_trace_event();
...
function buildGoodTaskVerifierAutoArtifacts() {
  return `pub(super) fn resolve_verifier_artifact_path() {}
pub(super) fn read_task_contract_artifact() {
  load_task_contract();
  "TASK_CONTRACT_MALFORMED";
}
...
function buildGoodTaskVerifierPreconditions() {
  return `pub(super) struct VerifiedCompletionInputs {}
pub(super) fn require_verified_completion_inputs() {
  "VERIFIED_REQUIRES_ENFORCEMENT";
  "VERIFIED_REQUIRES_TASK_CONTRACT";
  "VERIFIED_REQUIRES_TASK_REPORT";
