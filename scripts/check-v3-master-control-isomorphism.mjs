...
function check(s, diagnostics) {
  requireAll(diagnostics, FILES.blueprint, s.blueprint, [
    ...
    'architectural SSOT review prompt',
    'what is logically inconsistent, missing, or optimizable',
    'hard workflow contracts stay enforced by MissionD runtime, workflow.lisp, Board schema, and checkers',
    ...
    'treat PTY recognition as diagnostic only',
    'avoid recursive delegation for BoardTasks that already contain ## Swarm metadata or task_class=context-pack/code',
    'only use mission_swarm_run or mission_task_delegate when the next shard is concrete enough',
    'use mission_swarm_run for productized investigate -> integrate -> implement -> verify fanout',
    'mission_compute_slot',
    'defer long work to BoardTask/Autopilot and provider durable logs',
    ...
    'send control turns to slot-codex-master-control only on event-wakeup or pending periodic-heartbeat retry',
    ...
    '(master-delegation',
    '(master-recovery',
    'recover the latest open/running MissionD infra BoardTask whose title or description references project SSOT convergence workflow',
    '(night-scheduler',
  ...
  requireAll(diagnostics, FILES.masterControl, s.masterControl, [
    ...
    '以现在为例',
    '有什么逻辑不通',
    'Hard workflow contracts are enforced by MissionD runtime/workflow.lisp/checkers',
    'Query the active BoardTask by id before deciding',
    'PTY recognition is diagnostic only',
    'If this event is already a worker BoardTask',
    'do not recursively delegate',
    'Use mission_swarm_run or mission_task_delegate only when the next shard is already concrete enough',
    'context-pack diagnosis',
    'mission_swarm_run',
