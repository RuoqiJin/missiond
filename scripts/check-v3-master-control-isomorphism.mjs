...
function check(s, diagnostics) {
  requireAll(diagnostics, FILES.blueprint, s.blueprint, [
    ...
    'event_cursor, event_summary, phase, active_objective_id, and context_pack_path',
    ...
    'never recursively call mission_task_delegate for BoardTasks that already contain ## Swarm metadata',
    'verify any delegated task id with mission_board_query(action=get) before writing a success note',
  ...
  requireAll(diagnostics, FILES.masterControl, s.masterControl, [
    ...
    'For BoardTaskCreated/Updated, query that BoardTask by id before deciding',
	    'If a BoardTask description contains \\"## Swarm metadata\\"',
	    'it is already a worker BoardTask; do not recursively call mission_task_delegate for it',
	    'For implementation work, use mission_swarm_run for productized investigate -> integrate -> implement -> verify fanout',
	    'context_pack_path, write_scope, must_not_touch, acceptance, model_profile, timeout_secs, task_class, pool_hint, and engine_hint',
	    'Before claiming a delegated task id in a Board note, verify it exists via mission_board_query(action=\\"get\\")',
    'mission_swarm_run',
