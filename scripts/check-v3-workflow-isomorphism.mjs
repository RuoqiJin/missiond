...
function checkFiles(root, files) {
  ...
  requireAll(diagnostics, files.intentIntakeGrounding, sources.intentIntakeGrounding, [
    ...
    ':id review-packet',
    ':id plan-authoring-worker',
    ':id intent-memory-candidate',
    '第一轮：用户现在想做什么？',
    '第二轮：用最小查询查一遍记忆库、项目注册、SSOT Lisp、skill 证据、任务记录、相关会话和必要代码锚点',
    ...
    'Run mission_context_gather as the primary aggregate over KB, project registry, SSOT Lisp, skill evidence, infra evidence, active Board task records, and bounded conversation logs',
    'intent.lisp 已确认后，是否需要生成 plan.lisp？',
    ...
    'plan.lisp authoring is a worker/runtime stage unless an existing workflow.lisp explicitly matches',
  ...
  requireAll(diagnostics, files.m6DeploymentRollout, sources.m6DeploymentRollout, [
    ...
    'naked gh api polling loops are forbidden',
    'xjp_build_wait/xjp_deploy_watch',
