(workflow typed-lisp-compiler-convergence
  ...
  :core ((step s1 :logic "run the OCaml structural gate first when toolchain exists; otherwise return a typed toolchain diagnostic and keep JS gates as compatibility wrappers")
         ...
         (step s6 :logic "validate registered project .missiond blueprint directories through missiond-lispc check-project-dir before maturity evidence can claim M10")
         (step s7 :logic "migrate external project semantic checks one sample at a time, starting with Auth domain hardening")
         (step s8 :logic "allow code-anchor JS checkers to continue validating Rust/TS/MJS file anchors after the OCaml Lisp structural gate succeeds"))
