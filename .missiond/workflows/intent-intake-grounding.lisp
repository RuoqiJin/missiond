  ...
  :steps
    ((step s1 :id capture-request
     ...
     (step s8 :id review-packet
       ...
       :core "Return interaction intent_archived for default Jarvis/iOS/OpenAI flow, interaction intent_draft + confirm_required only for explicit manual-review/legacy confirmation mode, mission_request review_packet, or Board note projection that points to the request-local intent artifact. Human/external broad requests must not compile plan or dispatch workers before a grounded intent artifact exists; default Jarvis no-confirm mode may continue into grounding/key-judgment/plan while keeping intent.lisp archive-only for replay analysis.")
