(workflow intent-intake-grounding
  ...
  :steps
    ((step s1 :id capture-request
     ...
     (step s4 :id scoped-grounding-search
       ...
       :core "Run only bounded queries derived from s3. mission_kb_query is allowed here only with query terms, project filter, and max result cap; mission_intent reads project SSOT; mission_skill_context/mission_infra_query provide operational facts.")
     ...
     (step s9 :id intent-memory-candidate
  ...
  :egress [request.lisp intent-alignment.lisp review_packet context-pack BoardTask-note intent_memory_candidate]
  :risk-gates
    ((gate g1 :rule "No plan.lisp or worker dispatch before grounded intent exists and review policy is satisfied.")
