  ...
  :steps
    ((step s1 :name observe-deployment-event
     ...
     (step s2 :name classify-risk
       :logic "Classify deploy_failed, smoke_failed, rollback_failed, and agent_update_failed as actionable; classify succeeded/provenance_changed/heartbeat as observe-only unless a waiting BoardTask requested them.")
     ...
     (step s5 :name wait-or-close
       :logic "If the event completes a waiting deploy task, close only after durable event evidence and deploy-center provenance agree; otherwise leave a diagnostic note."))
  :risk-gates
