(workflow pcea-deployment-rollout
  ...
  :entry ["mission_skill_context(resolve,skill=pcea,include_kb=false)" "scripts/check-m6-deployment-status.mjs --json" "deploy-center provenance endpoint" "deployment-event-response"]
  :components
  ...
  :steps
    ((step s1 :name review-question
     ...
     (step s3 :name build-evidence-plan
     ...
     (step s4 :name classify-component-order
     ...
     (step s5 :name dispatch-deploy-ops-shard
     ...
     (step s6 :name wait-deploy-center-events
     ...
     (step s8 :name write-rollout-report
  ...
  :completion
    ((criterion c1 :rule "Each targeted component has deploy-center provenance for the requested commit or an explicit blocked reason.")
     ...
     (criterion c3 :rule "The rollout report captures any skill drift, manifest gap, migration/manual step gap, or deploy-agent evidence gap as follow-up work.")))
