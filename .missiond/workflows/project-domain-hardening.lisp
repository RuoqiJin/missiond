;; MissionD workflow: project domain hardening.
;;
;; Purpose: after a raw project reaches M6/M10 shape, force the second pass
;; that Auth taught us was necessary: prove the architecture is clear, the
;; compatibility debt is explicit, and every critical contract is wired into
;; real runtime paths rather than existing only in Lisp or tests.

(workflow project-domain-hardening
  :schema "missiond.workflow.project-domain-hardening.v1"
  ...
  :match_rules
    ((trigger :kind manual :tool mission_swarm_run :when "objective requests production-ready architecture, V3-hardening, or hot-path wiring")
     (trigger :kind boardtask :title-prefix "Harden project domain architecture")
     (trigger :kind maturity :after M10 :when "project is a core infrastructure or long-lived application dependency"))
  ...
  :steps
    ((step s1 :id domain-model-audit
       :logic "Extract the durable domain nouns, ownership chain, identity boundaries, permission boundaries, state machines, and external dependencies from Lisp plus code. Reject naming that lets protocol clients, runtime configs, or bridge tables masquerade as domain objects.")
     ...
     (step s5 :id runtime-registration-check
       :logic "Prove that registering new business objects is runtime data through DB/API/audit/event surfaces, not env/config/rebuild/redeploy. If registration still needs deploy, create implementation shards before claiming production-ready.")
     ...
     (step s7 :id hot-path-wiring-check
       :logic "For every architecture contract and policy service, grep-map the real runtime callers. A contract that is only in Lisp, tests, or dead service code remains design-only and cannot satisfy production-ready.")
     ...
     (step s10 :id final-production-readiness-report
       :logic "Write a report that classifies each function as code-aligned, runtime-projected, hot-path-wired, design-only, blocked, or requires-user-decision. Do not call production-ready while any critical hot path is design-only."))
  :egress [domain-hardening-report refined-blueprints compatibility-ledger checker-hard-gates code-shards regression-results decision-items]
  :risk-gates
    ((gate g1 :rule "Lisp-first: target architecture and checker gates are updated before runtime code changes.")
     (gate g2 :rule "No destructive DB migration in a hardening pass; use additive-first migrations plus compatibility ledger exit criteria.")
