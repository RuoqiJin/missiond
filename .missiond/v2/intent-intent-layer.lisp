...
(pillar intent-layer
  :version "v0.4"
  :status "phase-B recursive architecture contract 2026-04-26 — stimulus/directive → reasoning → prescription/output; unified-entry pipeline (message → alignment → plan → execution → workflow) code-aligned partial: directive-compiler v0 / plan-compiler v0 / plan-runner v0 + auto-selection v1 / workflow-distiller v0 / methodology compiler v0 / generated flow loader / unified-entry pipeline v0 internal helper / evidence-collector typed EvidenceEntry / capability-evolution-governance semantic evidence v1 / workstation-dispatch-policy operational-practice / plan-dag-scheduler runtime v2 全部 code-aligned partial (详 wave-13 anchors); wave 14 task 01/02/03/04 (commits 00cbc1d/2e7789a/96842cd/338a3fb) 升级: file-first writer integration (三类 artifact 主路径) / PlanNodeStateChanged variant 已扩 + live EventRef 三层策略 / review-gate auto-create policy enum (manual|emit_question|off) + deterministic id / unified-entry pipeline v1 (file-first + review-gate + scheduler args 转发) 全部 code-aligned; wave 15 task 02/04/05 (commits 3f37d32/03513c0/615b249) 升级: 4 sections (capability-evolution-governance / unified-entry-pipeline workstation-dispatch-policy / action-instruction-actor plan-dag-scheduler / unified-entry-pipeline review-gate-policy + id-derivation) 物理 split 到 4 shard (intent-capability-governance / intent-workstation-policy / intent-plan-dag / intent-directive-artifacts), 本 pillar 留 stub; review-gate explicit resolution bridge v0 (review_decision approved|rejected|needs_changes + envelope validator 5 fail-fast 错误码; 接 directive approve/archive + plan approve/mark/supersede; workflow handler 接入 pending) + workstation-dispatch v0 opt-in (PLAN node :workstation-dispatch true → mission_task_delegate, agent-team literal 恰好一次, SafeDescriptor 不 fallback prompt) 全部 code-aligned-partial; 4 项 v0 non-goal 中 review-resolution + workstation-dispatch 已部分缓解 (workflow review-resolution / autonomous spawn 仍 surface 不实现); 高阶 semantic lifting / forge compiler / 完整 11-stage scheduler 仍 pending (详 wave-15 anchors via intent-pillar-source-index.lisp)"
