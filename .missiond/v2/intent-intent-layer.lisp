...
(pillar intent-layer
  :version "v0.4"
  :status "phase-B recursive architecture contract 2026-04-26 — stimulus/directive → reasoning → prescription/output; unified-entry pipeline (message → alignment → plan → execution → workflow) 持续演进: directive-compiler v0 / plan-compiler v0 / plan-runner v0 + auto-selection v1 / workflow-distiller v0 / methodology compiler v0 / generated flow loader / unified-entry pipeline v0 internal helper / evidence-collector typed EvidenceEntry / capability-evolution-governance semantic evidence v1 / workstation-dispatch-policy operational-practice / plan-dag-scheduler runtime v2 全部 code-aligned partial (详 wave-13 anchors); wave 14 task 01/02/03/04 (commits 00cbc1d/2e7789a/96842cd/338a3fb): file-first writer integration / PlanNodeStateChanged variant + live EventRef 三层策略 / review-gate auto-create policy enum + deterministic id / unified-entry pipeline v1 全部 code-aligned; wave 15 task 02/04/05 (commits 3f37d32/03513c0/615b249): 4 sections 物理 split 到 4 shard, 本 pillar 留 stub; review-gate explicit resolution bridge v0 + workstation-dispatch v0 opt-in 落地. wave 16 task 01-08 (commits 01708be/331d1c1/8ffa9b2/a51bc52/d8f8a6e/591d288/0e6ee63/a632a91) 升级: workflow handler 接入 review-resolution → review-gate-resolution-v0 升 code-aligned (3 surface 全接); review-gate QuestionEvent::Resolved subscriber listener v0 (spawn_review_resolution_sub 与 spawn_decision_sub 并行; observation-only consume answer); workstation-dispatch auto-inference v1 (5 inference rules for mission_task_delegate scoped node + workstation_dispatch_source 5 值) → 升 code-aligned; PLAN DAG paused 7th lifecycle + review-gate question-event trigger (deterministic 'review:plan:<plan_id>:v<v>:plan-node:<sha256(node_id)[..16]>' / aggregate_status='dag_paused'); per-node retry policy v0 (cap 3 attempts / cap 60s delay / SafeDescriptor 不 retry / 每 attempt 写自己 evidence); scoped commit handoff daemon enforce v0 opt-in (4 错误码 / gate 在 allocate_id 之前); evidence subscriber 三档 live/log/unavailable + cap 1024 FIFO cache; unified-entry e2e smoke v0 (deterministic 4 hand-off no-LLM regression). 4 项 v0 non-goal 中 review-resolution / workstation-dispatch / review listener 已分别部分缓解; auto_approve_directive / auto_approve_plan / autonomous_workstation_dispatch 完整版 + 完整 11-stage scheduler (claim-lease / rollback / acceptance / mark-plan-final) + paused-resume listener (paused 节点收到 Resolved 后 re-dispatch) + 高阶 semantic lifting / forge compiler 仍 pending (详 wave-16 anchors via intent-pillar-source-index.lisp)"
