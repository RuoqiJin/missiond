...
(pillar intent-layer
  :version "v0.4"
  :status "phase-B recursive architecture contract 2026-04-27 — stimulus/directive → reasoning → prescription/output; unified-entry pipeline (message → alignment → plan → execution → workflow) 持续演进: directive-compiler v0 / plan-compiler v0 / plan-runner v0 + auto-selection v1 / workflow-distiller v0 / methodology compiler v0 / generated flow loader / unified-entry pipeline v0 internal helper / evidence-collector typed EvidenceEntry / capability-evolution-governance semantic evidence v1 / workstation-dispatch-policy operational-practice / plan-dag-scheduler runtime v2 全部 code-aligned partial; wave 14 task 01-04: file-first writer / PlanNodeStateChanged + live ref / review-gate auto-create v1 / unified-entry pipeline v1 全部 code-aligned; wave 15-18: review-gate resolution v0 + workstation-dispatch v0 + auto-inference + plan paused 7th + retry v0 + scoped commit enforce v0 + evidence subscriber 三档 + e2e smoke + paused-resume hook + claim-lease v0 + acceptance evaluator + rollback policy + finalize/distill + cross-node acceptance fan-in + cascade rollback + cross-plan distill chain + autonomous PLAN field inference + review automation policy + scoped-commit worktree preflight + ExecutionEvent dispatch metadata v1. wave 19 task 02-10: machine-contract task SSOT 全闭环. wave 20 task 01-09 全闭环: task-scope-index-guard v1 + renderer scoped-commit guard v2 + execution preflight contract scope v1 + machine-driven dispatch v0 (Lisp 真成 dispatch SSOT, Markdown brief 不再 load-bearing) + unified-entry machine-loop smoke v2 + cross-plan distill auto-trigger v1 + LLM-augmented plan inference v0 sonnet_suggest + review auto-answer policy v0 + ExecutionEvent legacy metadata sweep v0 (11 variants 闭环). **wave 21 task 01-08 全闭环 propose+apply-gate (commits 44c74df/1335fa7/308426e/68b84f1/a18200b/e140773/4d494db/8ba8723)**: hooks-path installer v1 (scripts/install-missiond-hooks.mjs + scripts/check-missiond-hooks.mjs doctor + .githooks/pre-commit env-gated; **opt-in repo-local only — 不擅自 default-on**) + task-run verifier v1 (scripts/verify-task-run.mjs 三合一 + 14 forbidden git verb proof + dogfood self-verify) + execution report-verifier integration v1 (mission_execution(complete) 加 task_run_verifier_status / shared_memory_path / verifier_diagnostics / verified 4 字段 + verified=true 触发 daemon-internal sexp cross-check + 4 structured error codes + daemon_never_invokes_mutating_git unit-pinned) + autonomous workstation LLM proposal v0 (workstation_inference_mode=off|sonnet_suggest opt-in + WorkstationProposalGate 仅在 caller+PLAN 全空才 propose + 4 propose 字段 × 3 confidence × 4 safety status + workstation_proposals[] cap 6 + **propose only never auto-spawn — applied=false / auto_spawn=false 永钉死** + Sonnet unavailable surfaces llm_unavailable + DAG mode preflight refuse) + plan inference apply gate v1 (apply_inferred_fields=true opt-in apply gate + 6 道严格 gate caller-approval / master-flag / confidence ∈ {high, medium} / conflict / safety / slot availability + 8 skip reason canonical + **persisted plan.sexp_text 永不 mutate** persist_inference_applied=false 永钉死) + LLM auto-approve proposal v0 (auto_approve_mode=off|sonnet_suggest opt-in for directive/plan review surfaces + ORTHOGONAL to wave-18/07 + wave-20/08 三 knob 共存 + **5 invariants test-pinned** I1 never auto-reject / I2 destructive blocked WITHOUT LLM call / I3 applied=false + requires_human=true 永钉死 / I4 sonnet unavailable no fallback / I5 destructive_check ALWAYS from is_destructive_review_action helper + 22 unit tests + 10 dispatch branch sites) + sonnet distill chain auto-apply v1 (auto_sonnet=true + auto_sonnet_approved=true 双 opt-in + 7 重 gate + auto-promote inner distill from dry_run to sonnet + 8 status taxonomy + **7 invariants test-pinned** I1 default-off / I2 dual opt-in / I3 reuse wave-20 trigger never relax / I4 caller-already-sonnet refusal / I5 sonnet failure preserve inner / I6 review_required=true PINNED / I7 wave-19/20 blocks UNCHANGED purely additive) + machine-contract autonomous loop smoke v3 (15 cross-wave invariant tests + Markdown task_brief_preview NEVER 进 artifact_refs 二度钉死 + machine dispatch SSOT 钉死). **brief → preflight → staged guard → commit → verifier (post-commit) → execution complete verify (daemon-internal verified gate) 六段闭环**. **propose-only 与 explicit apply-gate 范式覆盖 4 路通道 (workstation dispatch / plan field inference / review gate / sonnet distill chain), persisted state (plan.sexp_text / review row / completion log) v0 永不被自动 mutate**. 4 项 v0 non-goal 中 review-resolution / workstation-dispatch / review listener / review automation / review auto-answer (deterministic_safe) / LLM auto-approve propose / sonnet distill chain auto-apply 已分别部分缓解; 完整 LLM auto-apply (review/directive/plan真正落) / autonomous workstation true spawn (无 hint+真 spawn) / persisted plan inference apply (sonnet_suggest 升级到 真改 plan.sexp_text) / hooks default-on / 高阶 semantic lifting / forge compiler 仍 pending (详 wave-13..21 anchors via intent-pillar-source-index.lisp)"
