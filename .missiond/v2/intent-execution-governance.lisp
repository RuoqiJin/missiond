...
(execution-governance-shard
  ...
  :status "code-aligned — 12-action manager + ExecutionEvent emission + dispatch metadata 全 11 variants + scoped commit daemon enforce v0 (wave 16-06 commit 591d288) + scoped-commit worktree preflight v0 (wave 18-08 commit f171ae6) + execution task-contract completion verification v0 (wave 19-08 commit 405d13b) + execution preflight task-contract scope v1 (wave 20-03 commit fe835e8 加 8 contract-scope 字段, 0 mutating git) + task-scope-index-guard v1 (wave 20-01 commit 1fc0fd6) + ExecutionEvent legacy metadata sweep v0 (wave 20-09 commit 6e01e3f, 11 variants 全闭环). **wave 21 task 01 (commit 44c74df) 加 hooks-path installer v1** — scripts/install-missiond-hooks.mjs (--check/--install/--json/--dry-fixture/--strict) + scripts/check-missiond-hooks.mjs read-only doctor alias; --install runs git config --local core.hooksPath .githooks once + no-op when aligned + never --global/--system; .githooks/pre-commit 保 MISSIOND_TASK_CONTRACT env-gated 不破现有 workflow; **opt-in repo-local only — 不擅自 default-on**. **wave 21 task 02 (commit 1335fa7) 加 task-run verifier v1** — scripts/verify-task-run.mjs 三合一 (task contract + report task_id + commit_hash + memory completion); --task/--report/--memory/--commit/--json/--dry-fixture/--allow-missing-memory; 14 forbidden git verb proof + dogfood self-verify + verify-task-contract.mjs main() gated by import.meta.url. **wave 21 task 03 (commit 308426e) 加 execution report-verifier integration v1** — mission_execution(complete) 加 4 新字段 (task_run_verifier_status enum passed|failed|skipped|unknown / shared_memory_path / verifier_diagnostics / verified bool) + verified=true 触发 daemon-internal enforce_verified_completion gate (要求 enforce_scoped_commit=true + task_contract_path + task_report_path + commit_hash + 解析 report file 通过 local sexp module 不引新 dep + 断言 :schema=missiond.report-contract.v1 + :task_id matches contract head id + :commit_hash matches supplied hash with short<->long sha prefix overlap); 4 structured error codes (VERIFIED_REQUIRES_ENFORCEMENT/TASK_CONTRACT/TASK_REPORT/COMMIT_HASH + TASK_REPORT_REQUIRED/MALFORMED + TASK_REPORT_TASK_ID_MISMATCH + TASK_REPORT_COMMIT_HASH_MISMATCH + TASK_CONTRACT_MALFORMED) — 全在 companion log mutation 之前 fail-fast; 持久化为 Lisp keyword (`:task-run-verifier-status` / `:shared-memory-path` / `:verifier-diagnostics` / `:verified`); CompletionRecord struct 加 4 Option fields; legacy companion logs 不带新字段 byte-identical preserved; preflight_commit 也 echoes task_report_path/shared_memory_path advisory hints; daemon NEVER spawns Node (wave21-02 verifier 仍 out-of-process 权威); **daemon_never_invokes_mutating_git unit-pinned** (Command::new(<git>) 在 agent_execution.rs returns ONE hit — wave18-08 status read; zero mutating git argv strings introduced). brief → preflight (含 contract scope) → staged guard → commit → verifier (post-commit) → execution complete verify (daemon-internal verified gate) 六段闭环. 完整 enforce-by-default + git config core.hooksPath .githooks 默认启用 (wave21-01 仍 opt-in repo-local only) + report-contract checker auto-invoke from daemon (wave21-03 仍 caller-supplied verified flag) 仍 future"
