...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-26 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial (详 wave-13 anchors); wave 14 task 01/03/04 (commits 00cbc1d/96842cd/338a3fb) 升级: mission_directive/plan/workflow 加 write_file/topic/overwrite_file/project/cwd/target_project + review_gate_policy/emit_review_question/review_question_id args + 6 file_* + 4 review_question_* response fields 全部 code-aligned; wave 15 task 02/04/05 (commits 3f37d32/03513c0/615b249) 升级: implemented-surface mission_execution / mission_capability_usage / mission_directive write_file+review_gate / mission_plan write_file+review_gate / mission_workflow write_file+review_gate 物理 split 到 intent-execution-governance / intent-capability-governance / intent-directive-artifacts shard, 本 pillar 留 stub; mission_directive (approve/archive) + mission_plan (approve/mark/supersede) 加 review_decision/review_actor/review_note args (review-resolution bridge v0); mission_plan (action=execute) 加 workstation_dispatch/workstation_dispatch_dry_run args + PLAN node :workstation-dispatch hint (workstation-dispatch v0 opt-in); tool count 仍 83 不变; mission_workflow review-resolution / autonomous workstation spawn / 完整 11-stage PLAN DAG / semantic lifting / forge compiler / scoped commit daemon enforce 仍 pending (详 wave-15 anchors via intent-pillar-source-index.lisp)"
