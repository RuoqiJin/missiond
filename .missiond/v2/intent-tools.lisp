...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-26 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial (详 wave-13 anchors); wave 14 task 01/03/04: mission_directive/plan/workflow 加 write_file + review_gate args + 6 file_* + 4 review_question_* response fields 全部 code-aligned; wave 15 task 02/04/05: 5 implemented-surface 物理 split + mission_directive/plan 加 review_decision/review_actor/review_note args + mission_plan execute 加 workstation_dispatch args. wave 16 task 01-08 (commits 01708be/331d1c1/8ffa9b2/a51bc52/d8f8a6e/591d288/0e6ee63/a632a91) 升级: mission_workflow(action='resolve_review') schema 加同 5 字段 (3 surface 全接) → tools.surface.review-resolution-args 升 code-aligned; mission_plan execute workstation_dispatch 加 5 inference rules + workstation_dispatch_source 5 值 → tools.surface.plan-workstation-dispatch-args 升 code-aligned; mission_plan execute 加 PLAN node :review-gate 'question-event' / :review-action / :review-text / :retry-count / :max-attempts / :retry-delay-ms hints (paused 7th lifecycle + retry policy v0); mission_execution 加 enforce_scoped_commit (opt-in default false) + scoped_commit_enforced/scoped_commit_validation response (4 错误码: COMMIT_HASH_REQUIRED / COMMIT_BLOCKER_REQUIRED / CLAIM_SCOPE_REQUIRED / SCOPED_COMMIT_VIOLATION); tool count 仍 83 不变 (全部 wave-16 升级在既有 action 上加 args, 不新增 tool). 完整 11-stage PLAN DAG / semantic lifting / forge compiler / paused-resume listener / autonomous workstation spawn (无 hint) / 4 项 v0 non-goal 完整自动化 / enforce-by-default scoped commit 仍 pending (详 wave-16 anchors via intent-pillar-source-index.lisp)"
