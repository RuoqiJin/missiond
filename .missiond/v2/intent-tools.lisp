...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-26 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 (schema 加 max_parallel_nodes) + unified-entry pipeline v0 internal helper (不新增 tool, 复用现有管理面) 全部 code-aligned partial (详 wave-13 anchors via intent-pillar-source-index.lisp); scoped commit handoff 设计为 mission_execution completion 字段扩展; 完整 11-stage PLAN DAG / file-first .lisp writer / auto QuestionEvent gate / semantic lifting / forge compiler / ExecutionEvent dispatch metadata / planner-class model alias / scoped commit daemon enforce / autonomous workstation dispatch (4 项 v0 non-goal) 仍 pending"
