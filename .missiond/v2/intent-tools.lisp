...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-27 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial; wave 14 task 01/03/04: mission_directive/plan/workflow 加 write_file + review_gate args 全部 code-aligned; wave 15 task 02/04/05: 5 implemented-surface 物理 split + review_decision/review_actor/review_note + workstation_dispatch args; wave 16-18: review-resolution 3 surface + workstation auto-inference 5 rules + plan paused 7th + retry v0 + scoped commit enforce v0 (4 错误码 COMMIT_HASH_REQUIRED / COMMIT_BLOCKER_REQUIRED / CLAIM_SCOPE_REQUIRED / SCOPED_COMMIT_VIOLATION) + paused-resume + claim-lease v0 + acceptance evaluator + rollback policy + finalize/distill + cross-node acceptance fan-in + cascade rollback + cross-plan distill chain + autonomous PLAN field inference deterministic + review automation policy v0 + scoped-commit worktree preflight v0; tool count 仍 83. wave 19 task 06/07/08: mission_plan execute 加 emit_task_contract / task_contract_mode opt-in args (eligible workstation nodes 写 .missiond/tasks/generated/<plan_id>/<node_id>.lisp); mission_execution(complete) 加 task_contract_path/task_report_path/verifier_status/verifier_notes 4 字段 + 2 新错误码 (TASK_CONTRACT_REQUIRED / MALFORMED_TASK_CONTRACT_PATH / COMMIT_HASH_REQUIRED_FOR_CONTRACT); wave 19 task 09: mission_workflow(distill) 加 auto_chain / auto_chain_name opt-in 字段; tool count 仍 83. **wave 20 task 03/04/06/07/08/09 (commits fe835e8/681c95d/3669ebc/6bb935a/8adb0a8/6e01e3f)**: mission_execution(preflight_commit) 加 8 contract-scope 字段 (task_contract_status / staged_out_of_scope / staged_forbidden / unstaged_in_scope / task_contract_scope / next_step / task_contract_error / task_contract_resolved_path; 0 mutating git); mission_plan execute 加 dispatch_contract_mode arg (allowlist 'rendered' / 'machine') + render_markdown shorthand → DispatchContractMode {Rendered (default), Machine} (machine 模式接通 wave-19/07 dormant consumer, **Lisp 真正成 dispatch SSOT**) + 加 task_contract_source_path response field; mission_workflow(distill) 加 auto_chain_trigger arg (default 'never' / 'deterministic_only', 6 trigger rule); mission_plan 加 infer_plan_fields=sonnet_suggest 新 mode (opt-in / suggest only / applied=false 钉死 / DAG mode 拒); review-gate 加 auto_answer_policy arg (off|deterministic_safe|dry_run, 3 hard invariants I1 never reject / I2 destructive never auto-promote / I3 no LLM + unified_entry forward + build_artifact_refs lift 5 outcome keys); ExecutionEvent 8 legacy variants (Heartbeat/Released/DeviationRecorded/DecisionRecorded/IssueRecorded/Audited/Repaired/StaleClaim) 全加 dispatch_strategy/target_project/requested_cwd serde-default skip-serializing fields (11 variants 全闭环). tool count 仍 83 不变 (全部 wave-19/wave-20 升级在既有 action 上加 args 或 fields, 不新增 tool). 完整 LLM auto-approve 全 4 v0 non-goal / autonomous workstation spawn 完全无 hint / git config core.hooksPath .githooks 默认启用 / sonnet 完全自动接 chain (5+ rule 全过) / LLM-augmented PLAN inference apply / semantic lifting / forge compiler 仍 pending (详 wave-13..20 anchors via intent-pillar-source-index.lisp)"
