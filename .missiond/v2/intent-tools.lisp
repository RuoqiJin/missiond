...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-27 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial; wave 14-18 持续闭环 (mission_directive/plan/workflow write_file + review_gate args + 5 implemented-surface split + review-resolution 3 surface + workstation auto-inference 5 rules + plan paused 7th + retry v0 + scoped commit enforce v0 (4 错误码) + paused-resume + claim-lease v0 + acceptance evaluator + rollback policy + finalize/distill + cross-node acceptance fan-in + cascade rollback + cross-plan distill chain + autonomous PLAN field inference deterministic + review automation policy v0 + scoped-commit worktree preflight v0); tool count 仍 83. wave 19 task 06/07/08/09: mission_plan execute 加 emit_task_contract / task_contract_mode + mission_execution(complete) 加 4 字段 + mission_workflow(distill) 加 auto_chain / auto_chain_name. wave 20 task 03/04/06/07/08/09: mission_execution(preflight_commit) 加 8 contract-scope 字段; mission_plan execute 加 dispatch_contract_mode + render_markdown + task_contract_source_path; mission_workflow(distill) 加 auto_chain_trigger; mission_plan 加 infer_plan_fields=sonnet_suggest; review-gate 加 auto_answer_policy; ExecutionEvent 11 variants 全闭环. wave 21 task 01-08 全闭环 propose+apply-gate: scripts/install-missiond-hooks.mjs + scripts/check-missiond-hooks.mjs + scripts/verify-task-run.mjs 三脚本上线; mission_execution(complete) 加 4 新字段 + verified=true daemon-internal sexp cross-check + 4 structured error codes; mission_plan execute 加 workstation_inference_mode opt-in (propose only / applied=false / auto_spawn=false 永钉死) + apply_inferred_fields + llm_caller_approved + persist_inference opt-in apply gate (persisted plan.sexp_text 永不 mutate persist_inference_applied=false 永钉死); mission_directive + mission_plan 加 auto_approve_mode (propose only / 5 invariants); mission_workflow(distill) + mission_plan(distill_chain) 加 auto_sonnet + auto_sonnet_approved 双 opt-in (7 重 gate / 7 invariants); 15 deterministic smoke. **wave 22 task 01-07 全闭环 explicit-gate-promotion + auto-verifier + smoke v4 (commits 49555c4/02ac627/4b55cb4/fee6567/162a303/2423d4b/6b2125c)**: scripts/install-missiond-hooks.mjs default mode 升级到 = --check 只读 doctor (wave22-01) + scripts/render-claudecode-task.mjs renderHooksDoctorPreflight() 块在每 :commit :required brief commit section 上方; mission_execution(complete) 加 daemon-internal `auto_run_task_run_verifier` 8 cross-checks in-process (verification_source='daemon-auto-verifier' 主路径 / legacy verified=true degrade 'legacy-caller-claim') + 3 新错误码 SHARED_MEMORY_REQUIRED / SHARED_MEMORY_MALFORMED / SHARED_MEMORY_NO_COMPLETION_FOR_TASK (wave22-02); mission_directive + mission_plan 加 apply_llm_auto_approve + proposal_hash + caller_approved 3 strict opt-in apply gate (legacy quiet `action=approve` 路径触发 DB transition / 6 道严格 gate G1-G6 / 3 新错误码 APPLY_GATE_MISSING_PROPOSAL_HASH / APPLY_GATE_PROPOSAL_HASH_MISMATCH / APPLY_GATE_INVALID_PARAM / response surface llm_auto_approve_proposal_hash + llm_approve_apply_gate block / explicit review_decision 路径 gate 仍 informational only / wave-21/06 5 invariants 全 preserved 5 dedicated tests, wave22-03); mission_plan execute 加 persist_inference + caller_approved + proposal_hash 4 opt-in 升级到 v2 persisted apply gate (compute_inference_proposal_hash 32-hex SHA-256 + plan_insert(version=max+1) + plan_supersede(old_id) **真改 plan.sexp_text rollback through predecessor** / 2 新错误码 PERSIST_APPLY_MISSING_PROPOSAL_HASH / PERSIST_APPLY_PROPOSAL_HASH_MISMATCH / attach_persisted_apply_block stable wire / wave-21/05 6 invariants 全 preserved 7 dedicated tests / **I6 v1 `apply_gate.persist_inference_applied=false` 仍硬钉死** v2 用 SEPARATE `persisted_apply` block surface, wave22-04); mission_plan execute 加 auto_spawn + workstation_caller_approved + preflight_acceptable + workstation_proposal_hash 4 opt-in autonomous workstation true spawn v1 (**12-rule gate matrix** G1-G12 + 3 新错误码 AUTO_SPAWN_INVALID_PARAM / AUTO_SPAWN_MISSING_PROPOSAL_HASH / AUTO_SPAWN_PROPOSAL_HASH_MISMATCH + 15 status taxonomy / 走 mission_task_delegate substrate **绝不 claude -p** / wave-21/04 4 invariants 全 preserved 4 dedicated tests, wave22-05); mission_workflow(distill) + mission_plan(distill_chain) 加 auto_sonnet_policy ∈ {off, safe_after_rules, dry_run} closed-enum policy-driven (**dual opt-in 移除** policy 选择即 attestation / legacy 双 opt-in 仍 back-compat additive coexists / 8 AUTO_SONNET_POLICY_STATUS_* wire constants / 10 typo strict reject / wave-21/07 7 invariants 全 preserved 7 dedicated tests, wave22-06); 5 handler files smoke +9 new deterministic apply-gate smoke tests (**22 cross-wave invariants pinned** wave21-04 4 + wave21-05 6 + wave21-06 5 + wave21-07 7 / no real LLM/spawn/mutating git, wave22-07). **tool count 仍 83 不变 — 全部 wave-22 升级仍在既有 action 上加 args 或 fields, 不新增 tool**. NO 新 MCP tool. 完整 LLM 自主无任何 caller opt-in / Sonnet 真无任何 attestation / hooks default-on real install (wave22-01 仍 doctor only) / 完全 autonomous workstation 无 PLAN hint 无 task_contract_path (wave22-05 仍要求 task_contract_path + 12-rule gate) / semantic lifting / forge compiler 仍 pending (详 wave-13..22 anchors via intent-pillar-source-index.lisp)"
  ...
  :wave-26-status-summary
    ["mission_plan execute 增加 router_backend_registry_path dry-run evidence input; absent/off 保持旧响应形状且不读文件"
  ...
  :historical-footprint-sources
    ["mcp-defs 头部 '67 tools' 是旧数, 当前 83 (含 mission_execution/capability_usage/directive/plan/workflow/global_instruction 等新增) / mission_minimax_process Deprecated → mission_sonnet_process / future surfaces backlog cleared / promoted: mission_execution (4ab7994) + mission_capability_usage (c55fd61) + 4 unified-entry surfaces 已入 MCP registry"]
  ...
  (section mcp-surface-lifecycle
    :status "mission_execution + mission_capability_usage + mission_directive/plan/workflow + mission_global_instruction promoted to implemented; future backlog empty; unified-entry-pipeline actor v0 + plan_dag runtime v2 + evidence-collector typed + unified-entry pipeline v0 internal helper 全部 code-aligned partial (详 wave-13 anchors via intent-pillar-source-index.lisp); workstation dispatch 全程复用 mission_pty_*/mission_compute_slot/mission_task_delegate, 不新增 tool; file-first .lisp writer / 完整 11-stage PLAN DAG / auto QuestionEvent / semantic lifting / forge compiler / ExecutionEvent dispatch metadata 仍 pending"
    :actual-count 83
    :future-count 0
    ...
    (implemented-surface mission_plan
      :status "code-aligned partial — manager surface + plan-compiler v0 (compiler_mode=sonnet) + plan-runner v0 + auto-selection v1 (sexp hint parsing, 单节点 dispatch); execute_mode=bridge 默认 next_call (向后兼容), execute_mode=internal dispatch + 写 sidecar + 推 FSM; plan_dag runtime v2 (max_parallel_nodes / wave-based / 6 lifecycle + 3 skip 子分类 / failure-policy fail-fast vs continue / per-node typed evidence; dry_run 返 DAG + concurrency_plan); record_evidence typed wrap; 完整 11-stage 协议 (load-plan-graph / validate-node-schema / resolve-target-project-root / build-ready-set / acquire-execution-claim / dispatch-ready-nodes / collect-node-evidence / update-node-state / handle-retry-failure-rollback / mark-plan-final / trigger-record-execution-distill-candidate) 仍 pending — 详 actor plan-dag-scheduler (anchor: intent-layer.plan-dag-runtime-v2); file-first .lisp 自动写入 / 任意语义解析 / planner-class model alias 切换 仍 pending"
      :actions ["compile" "list" "get" "by_task" "approve" "mark" "supersede" "execute" "record_evidence"]
      :owner-boundary "tools owns schema; intent-layer owns plan-compiler / plan-runner v0 / future plan-dag-scheduler; memory directive-layer owns plan store; board links task execution"
      ...
      :dag-scheduler-future "actor plan-dag-scheduler — runtime v2 已 code-aligned partial; 完整 11-stage 仍 architecture-designed (DAG + ready-set + per-node FSM 由 daemon 内持有); claim/lease 复用 mission_execution(action=claim) — D010 教训不自建 ID 池; 节点 schema 必填 :id/:objective/:target, 可选 :depends-on/:condition/:failure-policy/:rollback-policy/:timeout-ms/:acceptance/:review-gate; 单节点 fast-path 已保留 (max_parallel_nodes=1)"
      ...
      :implementation-targets ["crates/missiond-mcp/src/tools/knowledge/plan.rs (schema: compile + execute_mode + dispatch_strategy + max_parallel_nodes + auto-selection)"
                               "crates/missiond-daemon/src/handlers/knowledge/plan.rs (action=compile sonnet + action=execute internal + auto-selection v1 + sidecar; 多节点 → plan_dag runtime v2)"
                               "crates/missiond-daemon/src/handlers/knowledge/plan_dag.rs (runtime v2)"
                               "crates/missiond-daemon/src/handlers/knowledge/evidence_collector.rs (typed EvidenceEntry; record_evidence typed wrap)"
                               "crates/missiond-daemon/src/llm/sonnet_gateway.rs"
                               "crates/missiond-daemon/src/handlers/knowledge/agent_execution.rs (mission_execution forwarding; future claim-lease 接入)"
                               "crates/missiond-daemon/src/handlers/compute/task_delegate.rs (mission_task_delegate forwarding)"]
      :future-implementation-targets ["crates/missiond-daemon/src/intent_layer/plan_runner.rs (新文件 — 完整 11-stage scheduler 抽离, claim-lease/retry/rollback)"
                                      "crates/missiond-daemon/src/intent_layer/plan_dag.rs (新文件 — 完整 DAG + 10 状态 FSM; runtime v2 是 6+3 子集)"
                                      "crates/missiond-core/src/event/events/execution.rs (ExecutionEvent::PlanNodeStateChanged 字段扩展 — anchor: intent-layer.plan-dag-runtime-v2.execution-event-decision)"]
      :pending ["file-first .missiond/plans/<topic>/PLAN.lisp 自动写入/同步"
                "完整 11-stage PLAN DAG scheduler (claim-lease / per-node retry / acceptance / rollback / review-gate paused / mark-plan-final / trigger-record-execution-distill)"
                "arbitrary PLAN.lisp 语义解释"
      ...
      (logic-core
        ...
        (step s5 "execute returns next_call bridge descriptor for mission_execution / mission_task_delegate / mission_flow_run; rejects unknown targets and non-approved plans; execute_mode=internal v0 单节点 dispatch + auto-selection v1 已 code-aligned; execute_mode=internal v1 完整 PLAN DAG scheduler (11-stage 协议 + per-node FSM + claim-lease) architecture-designed pending — 详 intent-layer pillar :: actor plan-dag-scheduler 与 flow pillar :: F-intent-alignment-plan-execution-loop :: s6 :: dag-scheduler")
    ...
    (implemented-surface mission_workflow
      :status "code-aligned partial — manager surface + workflow-distiller v0 (distill_mode=sonnet) + methodology compiler v0 (compile_mode=deterministic) + run_methodology + generated flow loader (mission_flow_run discoverability); dry_run 模式默认 preview; file-first .lisp 自动写入 / 高阶 semantic lifting / forge compiler / 自动 record_execution-distill 联动 / longest-prefix cwd resolver 仍 pending"
  ...
  (section tool-governance
    ...
    (wave-13-unified-entry-no-new-tool-confirmation
      :desc "wave 13 task 03 (commit 9759675) 已落 unified-entry pipeline v0 — internal helper, 不新增 MCP tool"
      :code-anchor "crates/missiond-daemon/src/handlers/knowledge/unified_entry.rs"
      :tool-count-invariant "保持 83 不变"
      :rationale "复用 mission_directive(action=compile) + mission_plan(action=compile|approve|execute) + mission_workflow(action=distill|record_execution) 现有管理面; internal run_pipeline + 纯函数 plan_pipeline 提供 7 步 pipeline_stage 字符串映射, 通过 next_call descriptor 驱动 caller 调既有 tool"
      :v0-non-goals
        ["auto_approve_directive — 不允许 LLM 产物自动越过 alignment-review-gate"
         "auto_approve_plan — 不允许 LLM 产物自动越过 plan-review-gate"
         "auto_answer_review_question — 不替人/Codex 答 QuestionEvent"
         "autonomous_workstation_dispatch — 不自动 spawn ClaudeCode 工位执行 plan"]
      :enforcement "internal helper 在每 response meta.v0_non_goals 显式 surface; future actor 升级时若要打开任一 non-goal, 必须先扩 mcp-defs / mcp-dispatch / 加 tool count"
