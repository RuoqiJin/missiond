...
(pillar tools
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-27 — 83 actual endpoints (tool count invariant); project-root spawn cwd contract aligned; mission_pty_spawn/pty_send/compute_slot/task_delegate 标 preferred workstation dispatch substrate; mission_directive/plan/workflow actor v0 + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 + mission_plan record_evidence typed wrap + mission_plan execute plan_dag runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial; wave 14 task 01/03/04: mission_directive/plan/workflow 加 write_file + review_gate args 全部 code-aligned; wave 15 task 02/04/05: 5 implemented-surface 物理 split + review_decision/review_actor/review_note + workstation_dispatch args; wave 16-18: review-resolution 3 surface + workstation auto-inference 5 rules + plan paused 7th + retry v0 + scoped commit enforce v0 (4 错误码) + paused-resume + claim-lease v0 + acceptance evaluator + rollback policy + finalize/distill + cross-node acceptance fan-in + cascade rollback + cross-plan distill chain + autonomous PLAN field inference deterministic + review automation policy v0 + scoped-commit worktree preflight v0; tool count 仍 83. wave 19 task 06/07/08: mission_plan execute 加 emit_task_contract / task_contract_mode opt-in args; mission_execution(complete) 加 4 字段 + 2 新错误码; wave 19 task 09: mission_workflow(distill) 加 auto_chain / auto_chain_name opt-in 字段; tool count 仍 83. wave 20 task 03/04/06/07/08/09: mission_execution(preflight_commit) 加 8 contract-scope 字段; mission_plan execute 加 dispatch_contract_mode + render_markdown shorthand + task_contract_source_path response; mission_workflow(distill) 加 auto_chain_trigger; mission_plan 加 infer_plan_fields=sonnet_suggest; review-gate 加 auto_answer_policy; ExecutionEvent 11 variants 全闭环. **wave 21 task 01-08 全闭环 propose+apply-gate (commits 44c74df/1335fa7/308426e/68b84f1/a18200b/e140773/4d494db/8ba8723)**: scripts/install-missiond-hooks.mjs + scripts/check-missiond-hooks.mjs + scripts/verify-task-run.mjs 三脚本上线 (hooks-installer opt-in repo-local only / run-verifier 三合一 + 14 forbidden git verb proof); mission_execution(complete) 加 4 新字段 (task_run_verifier_status / shared_memory_path / verifier_diagnostics / verified) + verified=true 触发 daemon-internal sexp cross-check + 4 structured error codes (VERIFIED_REQUIRES_ENFORCEMENT/TASK_CONTRACT/TASK_REPORT/COMMIT_HASH + TASK_REPORT_REQUIRED/MALFORMED + TASK_REPORT_TASK_ID_MISMATCH + TASK_REPORT_COMMIT_HASH_MISMATCH + TASK_CONTRACT_MALFORMED) + daemon_never_invokes_mutating_git unit-pinned (wave21-03); mission_execution(preflight_commit) 也 echo task_report_path/shared_memory_path advisory hints; mission_plan execute 加 workstation_inference_mode=off|sonnet_suggest opt-in (4 propose 字段 × 3 confidence × 4 safety + workstation_proposals[] cap 6, **propose only never auto-spawn — applied=false / auto_spawn=false 永钉死**, DAG mode preflight rejects INVALID_PARAM, wave21-04); mission_plan execute 加 apply_inferred_fields=true + llm_caller_approved (object/array shape) + persist_inference (bool) opt-in apply gate (6 道严格 gate + 8 skip reason canonical + apply_gate block 在每 dispatch 分支 surface, persisted plan.sexp_text 永不 mutate, persist_inference_applied=false 永钉死, wave21-05); mission_directive 加 auto_approve_mode (approve/archive); mission_plan 加 auto_approve_mode (approve/mark/supersede) — propose-only, llm_auto_approve_proposal 块在 10 dispatch 分支 surface, 5 invariants pinned (I1 never reject / I2 destructive blocked WITHOUT LLM call / I3 applied=false + requires_human=true / I4 no fallback / I5 destructive_check from helper), 22 unit tests, ORTHOGONAL to wave-18/07 + wave-20/08 三 knob 共存 (wave21-06); mission_workflow(distill) + mission_plan(distill_chain) 加 auto_sonnet + auto_sonnet_approved 双 opt-in (7 重 gate + auto-promote inner distill from dry_run to sonnet + 8 status taxonomy + 7 invariants pinned I1-I7 + strict shape validation, wave21-07); 4 handler files smoke +15 deterministic e2e tests pinning cross-wave invariants (wave21-08). **tool count 仍 83 不变 — 全部 wave-21 升级仍在既有 action 上加 args 或 fields, 不新增 tool**. NO 新 MCP tool. 完整 LLM auto-approve 真正落 (4 v0 non-goal) / autonomous workstation true spawn (wave21-04 propose only) / persisted plan inference apply (wave21-05 persist_inference_applied=false 钉死) / hooks default-on (wave21-01 仍 opt-in repo-local only) / semantic lifting / forge compiler 仍 pending (详 wave-13..21 anchors via intent-pillar-source-index.lisp)"
