...
(pillar worker
  :version "v0.5"
  :status "phase-C recursive architecture contract 2026-04-26 — runtime path → ordered mechanics → explicit egress; project-root spawn cwd contract; claudecode workstation orchestration policy (resident-lisp / fresh-code-alignment / agent-team-hint / spawn-over-prompt-mode / project-root-cwd / scoped-commit-handoff) operational-practice + architecture-designed; mission_execution dispatch_strategy companion log meta + scoped commit handoff (durability plane) + 完整 PLAN DAG scheduler 设计完成 (详 wave-13 anchors); wave 14 task 02: ExecutionEvent::PlanNodeStateChanged variant 已扩 + live EventRef 三层策略 code-aligned; wave 15 task 02/05: claudecode-workstation-orchestration section 物理 split 到 intent-workstation-policy shard + workstation-dispatch v0 opt-in 落地. wave 16 task 03/06 (commits 8ffa9b2/591d288) 升级: workstation-dispatch auto-inference v1 — 5 inference rules (target=mission_task_delegate / strategy ∈ 4 strategies / objective 非空 / scoping signal / 非 explicit false) + workstation_dispatch_source 5 值 (explicit_arg / plan_hint / inferred / disabled / not_applicable) → workstation-dispatch-v0 升 code-aligned; scoped commit handoff daemon enforce v0 opt-in (mission_execution 接 enforce_scoped_commit default false 字节兼容; 4 错误码 COMMIT_HASH_REQUIRED / COMMIT_BLOCKER_REQUIRED / CLAIM_SCOPE_REQUIRED / SCOPED_COMMIT_VIOLATION; gate 在 allocate_id 之前 rejected 不 bump state; daemon 不跑 git, commit_hash 由 caller 提供; scope-overlap 与 audit/claim 同一 scopes_overlap helper). ExecutionEvent::Opened 等其他 variant 的 dispatch metadata / plan-runner v1 完整 11-stage / autonomous workstation spawn (无 PLAN hint 全自动) / enforce-by-default scoped commit + git 仓库挂钩 仍 pending (详 wave-16 anchors via intent-pillar-source-index.lisp)"
