...
(pillar worker
  :version "v0.5"
  :status "phase-C recursive architecture contract 2026-04-26 — runtime path → ordered mechanics → explicit egress; project-root spawn cwd contract; claudecode workstation orchestration policy (resident-lisp / fresh-code-alignment / agent-team-hint / spawn-over-prompt-mode / project-root-cwd / scoped-commit-handoff) operational-practice + architecture-designed; mission_execution dispatch_strategy companion log meta + scoped commit handoff (durability plane) + 完整 PLAN DAG scheduler 设计完成 (详 wave-13 anchors); wave 14 task 02 (commit 2e7789a) 升级: ExecutionEvent::PlanNodeStateChanged variant 已扩 (4 必 + 5 可选含 dispatch_strategy/target_project) + live EventRef 三层策略 已 code-aligned; wave 15 task 02/05 (commits 3f37d32/615b249) 升级: section claudecode-workstation-orchestration (含 6 policy + dispatch-decision-matrix + execution-strategy-record) 物理 split 到 .missiond/v2/intent-workstation-policy.lisp shard, 本 pillar 留 stub; workstation-dispatch v0 opt-in 已落 (PLAN node :workstation-dispatch true → mission_task_delegate transport, 不 claude -p; agent-team literal 恰好一次; SafeDescriptor 不 fallback prompt; handlers/knowledge/workstation_dispatch.rs 962 行); ExecutionEvent::Opened 等其他 variant 的 dispatch metadata / scoped commit daemon enforce / plan-runner v1 完整 11-stage / autonomous workstation spawn (从 PLAN.lisp 自动推断) 仍 pending (详 wave-15 anchors via intent-pillar-source-index.lisp)"
