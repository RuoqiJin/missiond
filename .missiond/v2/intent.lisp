...
(intent missiond-v2
  ...
  (navigation-assets
    (source-of-truth-index "intent-pillar-source-index.lisp"
      :desc "判真索引 — pillar code-truth registry; v0.7 (wave 16 task 09) 已覆盖 7 pillar baseline + 7 高变动语义区 + wave 13 task 01/02/03 + wave 14 task 01/02/03/04/05 + wave 15 task 01/02/03/04/05 + wave 16 task 01/02/03/04/05/06/07/08 (workflow review-resolution / review-gate listener QuestionEvent::Resolved / workstation auto-inference v1 / plan paused 7th + retry policy v0 / scoped commit enforce v0 / evidence subscriber 三档 live/log/unavailable / unified-entry e2e smoke) 共 ~115 entry; 主 Lisp 压缩/拆分 cross-ref 锚点统一走这里")
  ...
  (pillar worker
    ...
    :canonical-status "v0.5 phase-C 2026-04-26 — recursive contract + xjp-router provider + mission_execution manager + project-root spawn cwd + claudecode-workstation-orchestration policy + dual-plane scoped-commit handoff + dispatch_strategy/target_project/requested_cwd companion log + plan-runner v0 + auto-selection v1 + PLAN DAG runtime v2 全部 code-aligned partial; wave 14 task 02: ExecutionEvent::PlanNodeStateChanged variant 已扩 + live EventRef 三层策略; wave 15 task 02/05: claudecode-workstation-orchestration section 物理 split + workstation-dispatch v0 opt-in. wave 16 task 03/06 (commits 8ffa9b2/591d288): workstation-dispatch auto-inference v1 (5 inference rules + 5 source 值, mission_task_delegate scoped node 自动推断) → 升 code-aligned; scoped commit handoff daemon enforce v0 (mission_execution 接 enforce_scoped_commit opt-in default false; 4 错误码; gate 在 allocate_id 之前; daemon 不跑 git) 已 code-aligned. ExecutionEvent::Opened dispatch metadata / 完整 11-stage scheduler / autonomous workstation spawn (无任何 hint 全自动) / enforce-by-default scoped commit 仍 pending (详 wave-13 + wave-14 + wave-15 + wave-16 anchors)"
  ...
  (pillar flow
    ...
    :canonical-status "v0.7 phase-C 2026-04-26 — 83 actual tools indexed + F-intent-alignment-plan-execution-loop 8 stages 统一入口 + 双 review gate + plan-runner v0 + F-execution-log-governance + F-scoped-commit-handoff (control plane + durability plane) + F-methodology / F-capability-usage / F-workstation-dispatch + PLAN DAG runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial; wave 14 task 01-04: file-first writer integration / PlanNodeStateChanged variant + live ref / review-gate auto-create v1 / unified-entry pipeline v1 全部 code-aligned; wave 15 task 02/04/05: 4 flow shard 物理 split + review-gate explicit resolution bridge v0 + workstation-dispatch v0 opt-in. wave 16 task 01-08: workflow handler 接入 review-resolution + review-gate QuestionEvent::Resolved subscriber listener v0 (event bus live subscription) + workstation auto-inference v1 + PLAN DAG paused 7th + retry policy v0 + scoped commit daemon enforce v0 opt-in + evidence subscriber 三档 + e2e smoke deterministic 4 hand-off; 不新增 tool (仍 83). 完整 11-stage PLAN DAG (claim-lease / rollback / acceptance / mark-plan-final) / paused-resume listener / autonomous workstation spawn (无 hint 全自动) / 4 项 v0 non-goal 完整自动化 / semantic lifting / forge compiler / enforce-by-default scoped commit 仍 pending (详 wave-13 + wave-14 + wave-15 + wave-16 anchors)"
