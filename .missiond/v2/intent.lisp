...
(intent missiond-v2
  ...
  (navigation-assets
    ...
    (precompression-note
      :desc "wave 13 task 05: 已执行 L1 安全压缩; wave 14 task 07: 写 L2 shard split plan; wave 15 task 02: L2 物理 split 已执行 (5 shard 创建 + 28 source-index 重定向 + section-id 全保 R008); wave 15 task 03: shard-aware checker 已落; wave 16 task 09: 回填 wave 16 全部 8 task; wave 17 task 09 / wave 18 task 10 / wave 19 task 12 / wave 20 task 10: 持续回填 + 升级条件; 后续压缩仍按 compression-policy 走; 详 architecture-dsl.lisp :: judgement-now / intent-pillar-source-index.lisp :: judgement-now :: wave-20-status-summary")
    ...
    (extensible-domain-count-test
      ...
      :status code-aligned)
    ;; ── wave 19 + wave 20 machine-contract / dispatch SSOT additions ──
  ...
  (pillar memory
    ...
    :status "v0.5.8 — 9 modules + directive artifacts + agent-execution dual-plane handoff (control + durability) + capability-usage-read-model semantic evidence v1 + directive-layer actor v0 全部 code-aligned partial; plan-node-state-projection 顶层字段 + plan-evidence-sidecar typed 已 code-aligned partial; wave 14 task 01: file-first writer integration 三类 artifact 主路径 code-aligned; wave 15 task 02: file-first-artifacts + capability-usage-read-model 物理 split 到 shard; wave 16 task 01/04/05/06 (commits 01708be/a51bc52/d8f8a6e/591d288): plan-node-state-projection 加 paused 7th lifecycle + per-attempt evidence (每 retry attempt 写自己 entry); agent-execution-coordination 加 scoped commit handoff daemon enforce v0 (4 错误码 + gate 在 allocate_id 之前 + daemon 不跑 git); review-gate-resolution-v0 升 code-aligned (workflow handler 接入, 3 surface 全接 directive/plan/workflow). wave 19 task 04 (commit 77f1f2b 吸入) + task 08 (commit 405d13b): shared-memory v1 ledger (6 entry types + checker + seed) 落 .missiond/tasks/<wave>/shared-memory.lisp 作为并行工位协调 sub-strate; mission_execution(complete) 加 task_contract_path/task_report_path/verifier_status 与 claim scope ⊇ contract write-scope 协议. wave 20 task 03 (commit fe835e8) + task 08 (commit 8adb0a8): mission_execution preflight_commit 加 8 contract-scope 字段 (0 mutating git); review-gate auto-answer policy v0 加 deterministic_safe / dry_run 模式 + 3 hard invariants (I1 never reject / I2 destructive never auto-promote / I3 no LLM). 完整 11-stage scheduler 主线已 close; enforce-by-default scoped commit + git config core.hooksPath .githooks 默认启用 + LLM 自动 approve 全 4 v0 non-goal 仍 pending (详 wave-13/14/15/16/17/18/19/20 anchors via intent-pillar-source-index.lisp)"
  ...
  (pillar worker
    ...
    :canonical-status "v0.5 phase-C 2026-04-27 — recursive contract + xjp-router provider + mission_execution manager + project-root spawn cwd + claudecode-workstation-orchestration policy + dual-plane scoped-commit handoff + dispatch_strategy/target_project/requested_cwd companion log + plan-runner v0 + auto-selection v1 + PLAN DAG runtime v2 全部 code-aligned partial; wave 14 task 02: ExecutionEvent::PlanNodeStateChanged variant 已扩 + live EventRef 三层策略; wave 15 task 02/05: claudecode-workstation-orchestration section 物理 split + workstation-dispatch v0 opt-in. wave 16 task 03/06: workstation-dispatch auto-inference v1 + scoped commit handoff daemon enforce v0 已 code-aligned. wave 17 task 02/07: claim-lease v0 (Claimed 8th lifecycle) + workstation scoped-commit brief default v1. wave 18 task 02/06/08: ExecutionEvent dispatch metadata Claimed+Completed + autonomous PLAN field inference v0 deterministic + scoped-commit worktree preflight v0 (mission_execution preflight_commit 0 mutating git). wave 19 task 07/08: workstation task-contract consumer v0 (overlay_contract + MalformedTaskContract SafeDescriptor 绝不 fall back claude -p) + execution task-contract completion verification v0. **wave 20 task 03/04/09 (commits fe835e8/681c95d/6e01e3f) 闭环**: mission_execution preflight_commit 加 8 contract-scope 字段对账; machine-driven dispatch v0 (DispatchContractMode {Rendered, Machine} + dispatch_contract_mode arg, Lisp 真成 dispatch SSOT, Markdown brief 不再 load-bearing); ExecutionEvent legacy metadata sweep v0 (8 legacy variants 全加 dispatch trio, 11 variants 全闭环). 完整 autonomous workstation spawn (无任何 hint 全自动) / enforce-by-default scoped commit + git config core.hooksPath .githooks 默认启用 仍 pending (详 wave-13/14/15/16/17/18/19/20 anchors)"
  ...
  (pillar flow
    ...
    :canonical-status "v0.7 phase-C 2026-04-27 — 83 actual tools indexed + F-intent-alignment-plan-execution-loop 8 stages 统一入口 + 双 review gate + plan-runner v0 + F-execution-log-governance + F-scoped-commit-handoff (control plane + durability plane) + F-methodology / F-capability-usage / F-workstation-dispatch + PLAN DAG runtime v2 + unified-entry pipeline v0 internal helper 全部 code-aligned partial; wave 14 task 01-04: file-first writer integration / PlanNodeStateChanged variant + live ref / review-gate auto-create v1 / unified-entry pipeline v1 全部 code-aligned; wave 15 task 02/04/05: 4 flow shard 物理 split + review-gate explicit resolution bridge v0 + workstation-dispatch v0 opt-in. wave 16-18: workflow review-resolution + review-gate listener + workstation auto-inference v1 + PLAN DAG paused 7th + retry policy + scoped commit enforce v0 + evidence subscriber 三档 + e2e smoke + paused-resume hook + claim-lease v0 + acceptance evaluator + rollback policy + finalize/distill + cross-node acceptance fan-in + cascade rollback + cross-plan distill chain + autonomous PLAN field inference + review automation policy + scoped-commit worktree preflight + ExecutionEvent dispatch metadata v1 (Claimed+Completed). wave 19 task 02-10: machine-contract task SSOT 全闭环 (verifier + report + shared-memory + renderer + plan emit + workstation consume + execution complete verify + cross-plan distill auto-chain + forward compensate ref). **wave 20 task 01-09 全闭环 (commits 1fc0fd6/b36cf6c/fe835e8/681c95d/d308fae/3669ebc/6bb935a/8adb0a8/6e01e3f)**: task-scope-index-guard v1 + renderer scoped-commit guard v2 + execution preflight contract scope v1 + machine-driven dispatch v0 (Lisp 真成 dispatch SSOT) + unified-entry machine-loop smoke v2 + cross-plan distill auto-trigger v1 + LLM-augmented plan inference v0 sonnet_suggest + review auto-answer policy v0 (3 hard invariants) + ExecutionEvent legacy metadata sweep v0 (11 variants 闭环). 不新增 tool (仍 83). 完整 LLM auto-approve 全 4 v0 non-goal / autonomous workstation spawn 完全无 hint / git config core.hooksPath .githooks 默认启用 + worktree-level scope enforce / sonnet 完全自动接 chain (5+ rule 全过) / LLM-augmented PLAN inference apply / frontend Lisp 仍 pending (详 wave-13..20 anchors via intent-pillar-source-index.lisp)"
