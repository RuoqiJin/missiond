...
(intent missiond-v2
  ...
  (navigation-assets
    (source-of-truth-index "intent-pillar-source-index.lisp"
      :desc "判真索引 — 哪个旧图代表哪 pillar 的代码真相 (gptpro 2026-04-21 产出); v0.2 (2026-04-26) 增 stable section-id registry: 7 pillar baseline + status taxonomy + implements 路径; v0.3 (2026-04-26 wave 12 task 06) 扩 7 高变动语义区 +22 entry + :compression-safe?; v0.4 (2026-04-26 wave 13 task 04) 回填 wave 13 task 01/02/03 真实状态: evidence-collector typed helper / PLAN DAG runtime v2 / unified-entry pipeline v0 共 +11 entry, 升 3 现有 entry status; 主 Lisp 后续压缩/拆分的 cross-ref 锚点都走这里")
    ...
    (precompression-note
      :desc "本次 wave 11 决议: 主 Lisp 不压缩 — 等 file-first writer + review gate + PLAN DAG 最小闭环稳定后, 才按 compression-policy.allowed 批量压缩状态文本; 物理 split shard 是再下一步; 详见 architecture-dsl.lisp :: judgement-now / intent-pillar-source-index.lisp :: judgement-now (wave 13 task 04 已升 PLAN DAG runtime v2 至 code-aligned partial)")
    (plan-dag-scheduler-design
      :desc "wave 11 D 组完整 PLAN DAG scheduler Lisp 架构 + wave 13 task 02 runtime v2 落地: max_parallel_nodes 并发 + 6 lifecycle + 3 skip 子分类 + failure-policy fail-fast/continue + per-node evidence; runtime v2 已 code-aligned partial (commit 8bb6110); 完整 11-stage scheduler / claim-lease / per-node retry / rollback compensate / acceptance evaluator / review-gate paused 仍 architecture-designed pending"
      ...
      :runtime-v2-anchor ".missiond/v2/intent-pillar-source-index.lisp :: section-entry intent-layer.plan-dag-runtime-v2 (wave 13 task 02 commit 8bb6110)"
    ...
    (unified-entry-pipeline-v0
      :desc "wave 13 task 03 (commit 9759675): unified-entry pipeline v0 internal helper code-aligned partial — 不新增 MCP tool, 复用 mission_directive/mission_plan/mission_workflow 管理面 surface; 7 步 pipeline (s1_message_intake → s3_alignment_review_gate → s4_plan_authoring → s5_plan_review_gate → s6_execution_runner); 每 response 携带 pipeline_stage / next_step / flow_ref / expects_next_inputs / next_call (适用时); v0 non-goals 4 项 (auto_approve_directive / auto_approve_plan / auto_answer_review_question / autonomous_workstation_dispatch) 在每 response surface"
      :anchor ".missiond/v2/intent-pillar-source-index.lisp :: section-entry intent-layer.unified-entry-pipeline.run-pipeline-helper"
      :code "crates/missiond-daemon/src/handlers/knowledge/unified_entry.rs"
      :status code-aligned-partial)
    (evidence-collector-typed
      :desc "wave 13 task 01 (commit 88568a9): typed EvidenceEntry helper 已接入 plan.rs::action_execute_internal 与 plan_dag.rs 每节点 dispatch — canonical source/kind/schema_version + with_extra flat-top 保留 byte-for-byte 兼容; legacy mission_plan(action=record_evidence) wire 仍兼容; live ExecutionEvent ref 用 EventRef::unavailable 占位, bus subscription 仍 pending"
      :anchor ".missiond/v2/intent-pillar-source-index.lisp :: section-entry intent-layer.evidence-collector-typed-helper"
  ...
  (pillar memory
    ...
    :status "v0.5.8 — 9 modules + directive artifacts + agent-execution contract upgraded to dual-plane handoff (execution Lisp control plane + scoped git commit durability plane) + capability-usage-read-model semantic evidence v1 + directive-layer actor v0 all code-aligned partial; plan-node-state-projection 已升 code-aligned partial (wave 13 task 02 commit 8bb6110: per-node pending/ready/running/succeeded/failed/skipped + 3 skip 子分类已写 evidence sidecar 顶层 — sidecar nodes[<node_id>] block 子树仍 architecture-designed pending, 等完整 11-stage scheduler 落地); plan-evidence-sidecar 已升 code-aligned partial via typed EvidenceEntry (wave 13 task 01 commit 88568a9: plan.rs + plan_dag.rs 走 typed EvidenceEntry, legacy passthrough byte-for-byte 兼容); plan-runner v1 (full 11-stage PLAN DAG scheduler 复用 agent-execution-coordination claim/lease 协议) architecture-designed pending; scoped commit daemon enforce / file-first .lisp writer / 完整 PLAN DAG claim-lease/retry/rollback 仍 pending"
  ...
  (pillar worker
    ...
    :canonical-status "v0.5 phase-C 2026-04-26 (recursive contract + xjp-router provider + mission_execution manager + project-root spawn cwd design + claudecode-workstation-orchestration policy operational-practice + architecture-designed; shared execution log + scoped commit handoff 升级为默认并行工位协议; mission_execution dispatch_strategy/target_project/requested_cwd 已写入 companion log meta — code-aligned partial; plan-runner v0 + auto-selection v1 sexp hint parsing 已 code-aligned partial 单节点 dispatch; PLAN DAG runtime v2 已 code-aligned partial (wave 13 task 02 commit 8bb6110: max_parallel_nodes / 6 lifecycle 状态 / 3 skip 子分类 / failure-policy fail-fast vs continue / 节点 dispatch evidence 走 typed EvidenceEntry 写串行化避免文件 race); 完整 11-stage scheduler / claim-lease (复用 agent-execution-coordination D010 协议) / per-node retry-N / rollback compensate / acceptance evaluator / review-gate paused 仍 architecture-designed pending; ExecutionEvent dispatch metadata / PlanNodeStateChanged 扩展 (wave 13 task 02 决议: scheduler runtime 与 bus subscription 正交, 不扩 ExecutionEvent variant 直到 bus subscription / 完整 scheduler 落地) / scoped commit daemon enforce 仍 code-alignment pending)"
  ...
  (pillar tools
    ...
    :canonical-status "v0.7 phase-C 2026-04-26 (83 actual tools classified; mission_directive/plan/workflow actor v0 + plan-runner v0 + auto-selection v1 + methodology compiler v0 + generated flow loader + mission_execution dispatch_strategy companion log + mission_capability_usage semantic evidence v1 all code-aligned partial; mission_plan(action=record_evidence) typed wrap 已 code-aligned partial (wave 13 task 01 commit 88568a9: evidence_kind/source 缺省走 legacy passthrough wire 兼容, 任一出现走 typed EvidenceEntry); mission_execution complete future fields for scoped commit handoff architecture-designed; 不新增 tool — wave 13 task 03 (commit 9759675) unified-entry pipeline v0 用 internal helper 复用 mission_directive/mission_plan/mission_workflow 管理面, 不新增 mission_message/mission_invoke, tool count 保持 83 不变; scoped commit handoff 复用 mission_execution, agent-team 仍任务 .md 提示; file-first .lisp writer / 完整 11-stage PLAN DAG (claim-lease/retry/rollback) / auto QuestionEvent / semantic lifting / forge compiler / ExecutionEvent dispatch metadata / planner-class model alias / scoped commit daemon enforce 仍 code-alignment pending)"
  ...
  (pillar intent-layer
    ...
    :canonical-status "v0.4 phase-B 2026-04-26 (unified-entry-pipeline actor v0 全部 code-aligned partial: directive-compiler v0 / plan-compiler v0 / plan-runner v0 + auto-selection v1 (单节点) / workflow-distiller v0 / methodology compiler v0 / generated flow loader; unified-entry-pipeline v0 internal helper 已 code-aligned partial (wave 13 task 03 commit 9759675: handlers/knowledge/unified_entry.rs 891 行, run_pipeline + 纯函数 plan_pipeline; 7 步 pipeline s1_message_intake → s3_alignment_review_gate → s4_plan_authoring → s5_plan_review_gate → s6_execution_runner; 每 response 携带 pipeline_stage / next_step / flow_ref / expects_next_inputs / next_call; v0 non-goals 4 项 auto_approve_directive / auto_approve_plan / auto_answer_review_question / autonomous_workstation_dispatch 在每 response surface; +21 tests; 不新增 MCP tool); evidence-collector role 已升 typed EvidenceEntry helper (wave 13 task 01 commit 88568a9: plan.rs::action_execute_internal + plan_dag.rs 每节点 dispatch 走 typed EvidenceEntry, EventRef::unavailable 占位); section action-instruction-actor :: actor plan-dag-scheduler 已升 code-aligned partial (wave 13 task 02 commit 8bb6110 PLAN DAG runtime v2: max_parallel_nodes / 6 lifecycle / 3 skip 子分类 / failure-policy fail-fast vs continue 已实现; 完整 11-stage scheduler / claim-lease / per-node retry / rollback compensate 仍 architecture-designed pending); capability-evolution-governance semantic evidence v1 已 code-aligned partial; workstation-dispatch-policy operational-practice + companion log dispatch_strategy 已落; file-first .lisp writer / 自动 review gate (4 项 v0 non-goal) / 高阶 semantic lifting / forge compiler / ExecutionEvent dispatch metadata / planner-class model alias / 完整 11-stage scheduler 代码同构 / event bus live subscription 仍 code-alignment pending)"
  ...
  (pillar flow
    ...
    :canonical-status "v0.7 phase-C 2026-04-26 (83 actual tools indexed + F-intent-alignment-plan-execution-loop 8 stages canonical 统一入口 + 双 review gate + plan-runner 内部调度契约 + actor v0 code-aligned partial; F-execution-log-governance 增 F-scoped-commit-handoff: shared execution Lisp control plane + scoped git commit durability plane; F-methodology / F-capability-usage / F-workstation-dispatch 已 code-aligned partial; PLAN DAG scheduler runtime v2 已 code-aligned partial (wave 13 task 02 commit 8bb6110: F-intent-alignment-plan-execution-loop :: s6 :: dag-scheduler concurrency + 6 lifecycle + 3 skip 子分类 + failure-policy fail-fast/continue + per-node evidence; 完整 11-stage 协议 claim-lease/retry/rollback 仍 architecture-designed pending); unified-entry pipeline v0 已落 internal helper (wave 13 task 03 commit 9759675: 不新增 tool, run_pipeline 内部 helper 复用 mission_directive/plan/workflow 管理面; 每 response 带 pipeline_stage/next_step/flow_ref/v0_non_goals); 不引入新 tool — file-first .lisp writer / 完整 11-stage PLAN DAG 代码同构 / auto QuestionEvent / semantic lifting / forge compiler / ExecutionEvent dispatch metadata / scoped commit daemon enforce / autonomous workstation dispatch 仍 pending)"
