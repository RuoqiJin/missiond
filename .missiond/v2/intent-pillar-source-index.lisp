...
(source-of-truth missiond-v2
  ...
  (judgement-now
    ...
    :next-step
      ["条件全满足后 (11 条件全 code-aligned; 完整 11-stage PLAN DAG scheduler 已 close 主线; wave 19 加 machine-contract task SSOT 全闭环; wave 20 加 machine-driven dispatch + scoped-commit 全 5 段闭环 + ExecutionEvent metadata 11 variants 闭环 + review auto-answer deterministic_safe; **wave 21 加 propose+apply-gate 范式覆盖 4 路 LLM/inference 通道 (workstation dispatch / plan field inference / review gate / sonnet distill chain) + execution 端 daemon-internal verified gate 6 段闭环 + hooks-path installer + 三合一 run verifier** — 留 完整 LLM auto-apply 真正落 (wave21-06 propose only) / autonomous workstation true spawn (wave21-04 propose only) / persisted plan inference apply (wave21-05 persist_inference_applied=false 永钉死) / git config core.hooksPath .githooks 默认 default-on (wave21-01 仍 opt-in repo-local only) / sonnet 完全自动接 chain 不需 dual opt-in (wave21-07 仍 dual opt-in required) / report-contract checker auto-invoke from daemon (wave21-03 仍 caller-supplied verified flag) / frontend Lisp 仍 future), 由 lisp-review skill 牵头, 按 compression-policy.allowed 三类做批量压缩"
       "wave 21 propose+apply-gate 范式 — 后续可推: (a) workstation propose 升级到 真 spawn (caller approval-gated → 真 mission_task_delegate spawn); (b) plan inference apply gate 升级到 persist (persist_inference_applied=true 真改 plan.sexp_text 落盘); (c) LLM auto-approve propose 升级到 真落 review row state transition (closing wave 14-04 留下的 4 v0 non-goal 之 auto_approve_directive + auto_approve_plan + auto_answer_review_question); (d) sonnet distill chain 升级到 完全自动 (无需 caller dual opt-in); (e) hooks-path installer 升级到 default-on (git config core.hooksPath .githooks 默认安装); (f) report-contract checker auto-invoke from daemon (verified=true 时 daemon 主动 spawn verifier 而非 caller 提供 status)"
