...
(directive-artifacts-shard
  ...
  :status "code-aligned (per source-index entries); writer integration 主路径已接入 (wave 14 task 01); review-gate auto-create v1 已落 (wave 14 task 03); wave 15 task 04 (commit 03513c0) 加 review-gate explicit resolution bridge v0 — 显式 review_decision (approved|rejected|needs_changes) + review_actor + review_note + envelope validator 5 fail-fast 错误码; 接到 directive (approve/archive) + plan (approve/mark/supersede); wave 16 task 01 (commit 01708be) 升: workflow handler 接入 mission_workflow(action='resolve_review') 5 字段 — methodology YAML 不 fake DB (receipt only); workflow row 无 status/version 列 → 仅 stamp 不 DB transition; 升: code-aligned-partial → code-aligned (3 surface 全接). wave 16 task 02 (commit 331d1c1) 加 review-gate QuestionEvent::Resolved subscriber listener v0 (spawn_review_resolution_sub 与 spawn_decision_sub 并行; conservative vocabulary mapping; observation-only consume answer 不替人答; auto_answer_review_question 仍 surface). wave 16 task 04 (commit a51bc52) 加 plan paused 7th lifecycle review-gate question-event trigger (anchor: intent-layer.plan-dag-runtime-v2.paused-lifecycle). wave 16 task 08 (commit a632a91) 加 unified-entry e2e smoke v0 (anchor: intent-layer.unified-entry-pipeline.e2e-smoke-v0). wave 17 task 01 (commit a42f5fd) 升 review-gate listener — spawn_review_resolution_sub 扩 plan-node action 自动 route 到 paused-node resume helper (approved → 重 dispatch / rejected/needs_changes → keep paused); 同时 mission_plan(action=execute) 加 4 resume args (resume_review_question_id / resume_review_decision / resume_actor / resume_note); 升: review-gate listener code-aligned-partial → code-aligned (paused-resume round-trip 闭环); anchor: intent-layer.plan-dag-runtime-v2.paused-resume-hook-v0 + intent-layer.unified-entry-pipeline.review-gate-resolution-listener-v0. wave 17 task 08 (commit c7fbac0) 加 unified-entry paused-resume e2e smoke (build_plan_execute_args forward 7 个 wave-17 keys + 6 步 round-trip no LLM no spawn no shell; anchor: intent-layer.unified-entry-pipeline.paused-resume-smoke-v0)"
  ...
  ;; ──────────────────────────────────────────────────────────
    (file-first-artifacts
      ...
      (artifact plan-node-state-projection
        ...
        :wave-17-progress "wave 17 task 01 (a42f5fd) 加 paused→resume_{approved|rejected|needs_changes} evidence; wave 17 task 02 (8661fcb) 加 claim_id + Claimed 第 8 态 + pending→claimed→running→{succeeded|failed}→released 五段 evidence (released 含 claim_terminal_state); wave 17 task 03 (f572729) 加 acceptance evaluator succeeded→acceptance_{accepted|rejected|manual_required} evidence (rejected 后翻回 failed; manual_required 复用 paused); wave 17 task 04 (d4466c6) 加 rollback failed→rollback_{not_requested|descriptor_ready|dispatched|refused|failed} evidence + node_results[].rollback 块 (acceptance_commands_executed=false pinned); wave 17 task 05 (402fb82) 加 plan-level dag_finalized evidence + finalization 块映射 aggregate→plan_status; rollback_path / acceptance_pass 字段已落; cross-plan distill chain / 跨节点 acceptance fan-in 仍 future"
