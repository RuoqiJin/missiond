...
(directive-artifacts-shard
  ...
  :status "code-aligned (per source-index entries); writer integration 主路径已接入 (wave 14 task 01); review-gate auto-create v1 已落 (wave 14 task 03); wave 15 task 04 (commit 03513c0) 加 review-gate explicit resolution bridge v0 — 显式 review_decision (approved|rejected|needs_changes) + review_actor + review_note + envelope validator 5 fail-fast 错误码; 接到 directive (approve/archive) + plan (approve/mark/supersede); wave 16 task 01 (commit 01708be) 升: workflow handler 接入 mission_workflow(action='resolve_review') 5 字段; 升: code-aligned-partial → code-aligned (3 surface 全接). wave 16 task 02 (commit 331d1c1) 加 review-gate QuestionEvent::Resolved subscriber listener v0. wave 16 task 04 (commit a51bc52) 加 plan paused 7th lifecycle review-gate question-event trigger. wave 16 task 08 (commit a632a91) 加 unified-entry e2e smoke v0. wave 17 task 01 (commit a42f5fd) 升 review-gate listener — spawn_review_resolution_sub 扩 plan-node action 自动 route 到 paused-node resume helper. wave 17 task 08 (commit c7fbac0) 加 unified-entry paused-resume e2e smoke. wave 18 task 07 (commit 0f55892) 加 review automation policy v0 — review_automation_policy (manual|suggest|auto_safe) + 5 deterministic safety rule (deterministic_mode / no_file_write OR file_hash_match / no_protected / no_additional_blockers / caller_opt_in); default = manual byte-compat; existing explicit review-resolution-v0 仍 authoritative; **绝不 auto_reject** (auto_safe 永远 approve-or-defer-to-manual); destructive transitions (archive/supersede) 永不 auto-promote; LLM 自动 approve / 自动 answer review question 仍 v0 non-goal; anchor: intent-layer.unified-entry-pipeline.review-automation-policy-v0. wave 18 task 09 (commit d440087) 加 unified-entry autonomous loop smoke v1 (build_plan_execute_args forward 4 wave-18 keys + 5 stage no LLM no spawn no shell; anchor: intent-layer.unified-entry-pipeline.autonomous-loop-smoke-v1)"
