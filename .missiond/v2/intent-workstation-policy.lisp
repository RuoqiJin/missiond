...
(workstation-policy-shard
  ...
  :status "operational-practice + code-aligned (per source-index entries); wave 15 task 05 (commit 615b249) 加 workstation-dispatch v0 opt-in — PLAN 节点 :workstation-dispatch true 触发 mission_task_delegate (不 claude -p); task brief 含 objective/owned/forbidden/acceptance/commit-policy + agent-team literal '使用 agent-team提高效率' 恰好一次; 失败返 SafeDescriptor 不静默 fallback prompt. wave 16 task 03 (commit 8ffa9b2) 加 auto-inference v1 — 5 inference rules (target=mission_task_delegate / strategy ∈ 4 strategies {resident-lisp/fresh-code-alignment/agent-team/mixed} / objective 非空 / scoping signal 存在 / 节点未 explicit false); workstation_dispatch_source 5 值 (explicit_arg / plan_hint / inferred / disabled / not_applicable); 不为 mission_execution / mission_flow_run 推断; target/project root 未解析时不推断; agent-team literal 仍恰一次; 升: code-aligned-partial → code-aligned (auto-inference for mission_task_delegate scoped node 已落). wave 16 task 06 (commit 591d288) 加 scoped commit handoff daemon enforce v0 — mission_execution 接 enforce_scoped_commit (opt-in default false 字节兼容); 4 错误码 (COMMIT_HASH_REQUIRED / COMMIT_BLOCKER_REQUIRED / CLAIM_SCOPE_REQUIRED / SCOPED_COMMIT_VIOLATION); gate 在 allocate_id 之前 (rejected 不 bump state); daemon 不跑 git; anchor: intent-layer.scoped-commit-enforce-v0. wave 17 task 07 (commit a21b8ae) 加 scoped-commit handoff brief default v1 — workstation_dispatch.rs::build_task_brief 任务 brief 加 section 7 'Completion handoff (scoped commit)'; code task (owned_files 非空) → 要求 enforce_scoped_commit=true / commit_status=committed / commit_hash / staged_files; read-only task (owned_files 空) → commit_status=not-required + 解释; response 加 scoped_commit_required: true / scoped_commit_policy: 'enforced-on-complete'; daemon agent_execution.rs 完全未动 (legacy mission_execution(action=complete) callers 仍 enforce_scoped_commit default false 字节兼容); brief 与 daemon 双层契约清晰: brief 强制新派工 worker 显式 commit, daemon legacy 仍可被旧 caller bypass; agent-team literal 仍恰一次 (新加 section 不影响 idempotent invariant); anchor: intent-layer.scoped-commit-handoff-brief-default-v1. wave 17 task 02 (commit 8661fcb) 加 PLAN DAG claim-lease v0 把 worker pillar 的 :owned-files 概念抬到 PLAN 节点级 — claim scope 优先级 :owned-files → :scope → plan/<plan_id>/node/<node_id> fallback; scopes_overlap_pure 共享给 plan_dag claim / agent_execution audit / scoped-commit enforce 三处, 三处不会再 drift; anchor: intent-layer.plan-dag-runtime-v2.claim-lease-v0. 完全 autonomous spawn (无任何 PLAN hint, plan-runner 全局推断, 跨所有 target) + 后续 enforce-by-default + git pre-commit hook 执行 scope check 仍 surface 不实现"
