...
(workstation-policy-shard
  ...
  :status "operational-practice + code-aligned (per source-index entries); wave 15 task 05 (commit 615b249) 加 workstation-dispatch v0 opt-in — PLAN 节点 :workstation-dispatch true 触发 mission_task_delegate (不 claude -p); task brief 含 objective/owned/forbidden/acceptance/commit-policy + agent-team literal '使用 agent-team提高效率' 恰好一次; 失败返 SafeDescriptor 不静默 fallback prompt. wave 16 task 03 (commit 8ffa9b2) 加 auto-inference v1 — 5 inference rules (target=mission_task_delegate / strategy ∈ 4 strategies / objective 非空 / scoping signal 存在 / 节点未 explicit false); workstation_dispatch_source 5 值; 升: code-aligned-partial → code-aligned. wave 16 task 06 (commit 591d288) 加 scoped commit handoff daemon enforce v0 — mission_execution 接 enforce_scoped_commit (opt-in default false 字节兼容); 4 错误码; gate 在 allocate_id 之前; daemon 不跑 git; anchor: intent-layer.scoped-commit-enforce-v0. wave 17 task 07 (commit a21b8ae) 加 scoped-commit handoff brief default v1 — workstation_dispatch.rs::build_task_brief 任务 brief 加 section 7 'Completion handoff (scoped commit)'; brief → daemon 双层契约清晰; anchor: intent-layer.scoped-commit-handoff-brief-default-v1. wave 17 task 02 (commit 8661fcb) 加 PLAN DAG claim-lease v0 把 worker pillar 的 :owned-files 概念抬到 PLAN 节点级 — scopes_overlap_pure 共享给 plan_dag claim / agent_execution audit / scoped-commit enforce 三处; anchor: intent-layer.plan-dag-runtime-v2.claim-lease-v0. wave 18 task 06 (commit 91f8fa1) 加 autonomous PLAN field inference v0 — workstation_dispatch 是 6 个推断字段之一 (deterministic heuristics + 3 confidence + 仅 high apply_safe; conflict 永不静默 override; 完全无 LLM); anchor: intent-layer.plan-dag.autonomous-plan-field-inference-v0. wave 18 task 08 (commit f171ae6) 加 scoped commit worktree preflight v0 — mission_execution 加 action='preflight_commit' 只调 git status --porcelain=v1 (grep proof 0 mutating subcommand: 不 add / 不 commit / 不 stash / 不 reset / 不 checkout / 不 push / 不 merge / 不 rebase); 返结构化 ok / changed_files / staged_files / out_of_scope_files / expected_owned_files / expected_untouched_files / next_step (建议: ready_to_commit / stage_required / out_of_scope_review / claim_scope_required); preflight 设计为新派工 worker 完成前自检; daemon 仍不跑 git mutating commands; brief → preflight → complete enforce 三段闭环; anchor: intent-layer.scoped-commit-worktree-preflight-v0. wave 19 task 07 (commit bfc72b7) 加 workstation task-contract consumer v0 — parse_task_contract / load_task_contract / WorkstationDispatchHints::overlay_contract; run_workstation_dispatch_with_contract 加 task_contract_path 参数; SafeDescriptorReason::MalformedTaskContract 新 variant; legacy brief byte-identical 当 contract absent; **绝不 fall back claude -p**; anchor: intent-layer.unified-entry-pipeline.workstation-task-contract-consumer-v0. wave 19 task 08 (commit 405d13b) 加 execution task-contract completion verification v0 — mission_execution(complete) 加 4 字段 (task_contract_path/task_report_path/verifier_status/verifier_notes); 2 新错误码 (TASK_CONTRACT_REQUIRED/MALFORMED + COMMIT_HASH_REQUIRED_FOR_CONTRACT); enforce_scoped_commit + task_contract_path → require commit_hash + claim scope ⊇ contract write-scope; daemon 仍只 read-only; brief → preflight → complete enforce + contract verify 四段闭环; anchor: intent-layer.unified-entry-pipeline.task-contract-completion-verification-v0. 完全 autonomous spawn (无任何 PLAN hint, plan-runner 全局 LLM 推断, 跨所有 target) + 后续 enforce-by-default + git pre-commit hook 执行 scope check + LLM-augmented PLAN inference 仍 surface 不实现"
