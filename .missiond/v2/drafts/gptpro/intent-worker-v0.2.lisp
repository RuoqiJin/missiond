...
(pillar worker
  ...
  :target-path ".missiond/v2/drafts/gptpro/intent-worker-v0.2.lisp"
  ...
  (section pty
    ...
    :semantic-parser-files
      ["semantic-terminal/src/patterns.rs"
       "semantic-terminal/src/state.rs"
       "semantic-terminal/src/gemini_state.rs"
       "semantic-terminal/src/fingerprint.rs"
       "semantic-terminal/src/confirm.rs"
       "semantic-terminal/src/tool.rs"
       "semantic-terminal/src/status.rs"
       "semantic-terminal/src/title.rs"
    ...
    :need-more-ground-truth
      ["crates/missiond-daemon/src/slot_manager/ — phase-A package 未给出逐文件清单, v0.2 只把它当 runtime authority, phase-B 再做精确 file mapping"]
  ...
  (section worker-side-computation
    (path retrieval-fusion
      ...
      (ingress
        ...
        :entry-components
          ["crates/missiond-daemon/src/context/context_pipeline.rs"
           "crates/missiond-daemon/src/context/retrieval.rs"
      ...
      (egress
        ...
        :need-more-ground-truth "context/retrieval.rs 与实际 fusion ranker 实现路径需 phase-B file scan 复核"))
  ...
  (need-more-ground-truth
    (item 1 "slot_manager/ 精确文件清单与其在 pty / orchestration path 的绑定关系")
    (item 2 "workflow_executor.rs 的表级 R/W 契约")
    (item 3 "learning_engine 除 intent_analyst 外的 precise table contract")
    (item 4 "context/retrieval.rs 与 retrieval-fusion 的真实文件绑定")
