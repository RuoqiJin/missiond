...
(pillar flow
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-27 — trigger/state → ordered cross-pillar stages → egress; directive/incident/methodology/capability-usage flows + project-root spawn cwd contract designed; F-intent-alignment-plan-execution-loop + F-execution-log-governance + F-scoped-commit-handoff + F-workstation-dispatch-policy + F-methodology-to-executable-compile + F-capability-usage-monitoring + actor v0 + PLAN DAG runtime v2 + unified-entry pipeline v0 internal helper + evidence-collector typed helper 全部 code-aligned partial; wave 14-18 持续闭环 (file-first writer + PlanNodeStateChanged + review-gate v1 + workstation-dispatch v0 + auto-inference + scoped commit enforce v0 + claim-lease + paused-resume + acceptance + rollback + finalize/distill + cross-plan distill + autonomous PLAN field inference + review automation policy + scoped-commit worktree preflight + ExecutionEvent dispatch metadata v1). wave 19 task 02-10: machine-contract task SSOT 全闭环. wave 20 task 01-09 全闭环: task-scope-index-guard v1 + renderer scoped-commit guard v2 + execution preflight contract scope v1 + machine-driven dispatch v0 (Lisp 真成 dispatch SSOT, Markdown brief 不再 load-bearing) + unified-entry machine-loop smoke v2 + cross-plan distill auto-trigger v1 + LLM-augmented plan inference v0 sonnet_suggest + review auto-answer policy v0 + ExecutionEvent legacy metadata sweep v0 (11 variants 闭环). wave 21 task 01-08 全闭环 propose+apply-gate: hooks-path installer v1 (opt-in repo-local only) + task-run verifier v1 + execution report-verifier integration v1 + autonomous workstation LLM proposal v0 (propose only) + plan inference apply gate v1 (persisted plan.sexp_text 永不 mutate) + LLM auto-approve proposal v0 (propose only) + sonnet distill chain auto-apply v1 (双 opt-in) + machine-contract autonomous loop smoke v3 (15 cross-wave invariants pinned). **wave 22 task 01-07 全闭环 explicit-gate-promotion + auto-verifier + smoke v4 (commits 49555c4/02ac627/4b55cb4/fee6567/162a303/2423d4b/6b2125c)**: hooks default-on doctor v2 (install-missiond-hooks.mjs default mode = --check 只读 doctor 默认跑 / --install 仍唯一 mutation / 4 reason codes / renderer renderHooksDoctorPreflight() 块在 commit section 上方 / 11 dry-fixtures 8→11; **doctor default-on ≠ real install default-on**) + execution auto-run-verifier v2 (daemon-internal `auto_run_task_run_verifier` 8 cross-checks in-process / verification_source='daemon-auto-verifier' 主路径 / 3 新错误码 SHARED_MEMORY_REQUIRED / SHARED_MEMORY_MALFORMED / SHARED_MEMORY_NO_COMPLETION_FOR_TASK / legacy verified=true degrade 'legacy-caller-claim' / 8 new tests / 绝不 spawn Node-shell-mutating-git) + review LLM approve apply gate v1 (apply_llm_auto_approve + proposal_hash + caller_approved 3 strict opt-in + **6 道严格 gate** G1-G6 / 3 新错误码 APPLY_GATE_MISSING_PROPOSAL_HASH / APPLY_GATE_PROPOSAL_HASH_MISMATCH / APPLY_GATE_INVALID_PARAM / 只 legacy quiet `action=approve` 路径触发 DB transition / wave-21/06 5 invariants 全 preserved 5 dedicated tests / explicit review_decision 路径 gate 仍 informational only) + persisted plan inference apply v2 (persist_inference + caller_approved + proposal_hash 4 opt-in + compute_inference_proposal_hash 32-hex SHA-256 + **plan_insert(version=max+1) 加 (plan-inference-applied :inference-version v2 ...) annotation + plan_supersede(old) 真改 plan.sexp_text rollback handle through predecessor** / 2 新错误码 PERSIST_APPLY_MISSING_PROPOSAL_HASH / PERSIST_APPLY_PROPOSAL_HASH_MISMATCH / wave-21/05 6 invariants 全 preserved 7 dedicated tests / **I6 v1 `apply_gate.persist_inference_applied=false` 仍硬钉死, v2 用 SEPARATE `persisted_apply` block surface 状态**) + autonomous workstation true spawn v1 (auto_spawn + workstation_caller_approved + preflight_acceptable 4 opt-in + **12-rule gate matrix** G1-G12 + 3 新错误码 + 15 status taxonomy / 走 mission_task_delegate substrate **绝不 claude -p** / wave-21/04 4 invariants 全 preserved 4 dedicated tests) + distill chain policy auto-sonnet v2 (auto_sonnet_policy ∈ {off, safe_after_rules, dry_run} closed-enum / **dual opt-in 移除** policy 选择即 attestation / legacy 双 opt-in 仍 back-compat additive coexists / wave-21/07 7 invariants 全 preserved 7 dedicated tests I7 验证 4 块 coexistence) + autonomous loop apply smoke v4 (9 new deterministic smoke tests across 5 in-scope files / **22 cross-wave invariants pinned** wave21-04 4 + wave21-05 6 + wave21-06 5 + wave21-07 7 / no real LLM (synthesised structs) / no real spawn (gate evaluators 终止于 Spawned 不调 substrate) / no mutating git (tempfile fixtures)). 不新增 tool (仍 83). brief → preflight → staged guard → commit → verifier (post-commit) → execution complete verify (daemon-internal **auto_run_task_run_verifier** 4 路径全提供 主路径 'daemon-auto-verifier') 六段闭环 wave-22 强化. **explicit-apply-gate 范式覆盖 4 路通道**: review LLM approve / persisted plan inference (v2 真改 plan.sexp_text + plan_supersede rollback handle) / autonomous workstation true spawn (走 mission_task_delegate 真 spawn) / sonnet distill chain (policy-driven 单一 attestation). 完整 LLM 自主无任何 caller opt-in / Sonnet 真无任何 attestation / git hooks default-on real install (wave22-01 仍 doctor only) / autonomous workstation true spawn 完全无 PLAN hint (wave22-05 仍要求 task_contract_path + 12-rule gate) / frontend Lisp 仍 pending (详 wave-13..22 anchors via intent-pillar-source-index.lisp)"
