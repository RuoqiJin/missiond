      ...
      :stages
        ((s1 message-intake
         ...
         (s6 execution-runner
            ...
            :code-alignment "code-aligned partial — execute_mode=internal + dispatch_strategy + auto-selection v1 完成 (单节点 / single dispatch); explicit args 仍优先, 缺省时按保守 hint 推断, 无法安全推断时返回 MISSING_PARAM 结构化错误; 完整 PLAN DAG scheduler / 多节点 dependency 执行 / arbitrary semantic interpretation 仍 architecture-designed pending — 详 sub-section dag-scheduler"
            ...
            :pending ["完整 PLAN DAG scheduler — 多节点 dependency 执行 / 并发 dispatch (sub-section dag-scheduler architecture-designed)"
            ...
            (dag-scheduler
              ...
              :implementation-targets-future
                ["crates/missiond-daemon/src/handlers/knowledge/plan.rs (action=execute internal — 升级到 DAG 调度循环, 当前 v0 仅单节点 dispatch)"
                 "crates/missiond-daemon/src/intent_layer/plan_runner.rs (新文件 — 提取 scheduler 出 plan.rs handler, 含 DAG 加载 / ready-set / FSM)"
                 "crates/missiond-daemon/src/intent_layer/plan_dag.rs (新文件 — DAG 数据结构 + cycle 检测 + ready-set 计算 + 节点 FSM enum)"
                 "crates/missiond-daemon/src/handlers/knowledge/agent_execution.rs (claim/heartbeat/release 接入 — 已 code-aligned, 复用)"
                 "crates/missiond-core/src/event/events/execution.rs (ExecutionEvent::PlanNodeStateChanged 扩展 — 当前 dispatch metadata 仍 pending)"
                 "crates/missiond-core/src/db/pg/directive.rs (plan_update_status 已 code-aligned; 是否新增 plan_evidence JSONB 列待审)"]
              :checker-contract
