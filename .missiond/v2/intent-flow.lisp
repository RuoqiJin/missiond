...
(pillar flow
  :version "v0.7"
  :status "phase-C recursive architecture contract 2026-04-26 — trigger/state → ordered cross-pillar stages → egress; directive/incident/methodology/capability-usage flows + project-root spawn cwd contract designed; F-intent-alignment-plan-execution-loop + F-execution-log-governance + F-scoped-commit-handoff + F-workstation-dispatch-policy + F-methodology-to-executable-compile + F-capability-usage-monitoring + actor v0 + PLAN DAG runtime v2 + unified-entry pipeline v0 internal helper + evidence-collector typed helper 全部 code-aligned partial (详 wave-13 anchors); wave 14 task 01/02/03/04 (commits 00cbc1d/2e7789a/96842cd/338a3fb): file-first writer integration / PlanNodeStateChanged variant + live ref / review-gate auto-create v1 (policy enum manual|emit_question|off) / unified-entry pipeline v1 (file-first + review-gate + scheduler args 转发) 全部 code-aligned (详 wave-14 anchors via intent-pillar-source-index.lisp); wave 15 task 02/04/05 (commits 3f37d32/03513c0/615b249): F-execution-log-governance / F-scoped-commit-handoff / F-workstation-dispatch-policy / F-capability-usage-monitoring 已物理 split 到独立 shard, 本 pillar 留 stub; review-gate explicit resolution bridge v0 + workstation-dispatch v0 opt-in 已 code-aligned-partial (详 wave-15 anchors via intent-pillar-source-index.lisp); 完整 11-stage PLAN DAG / semantic compiler / scoped commit daemon enforce / event bus live subscription / autonomous workstation spawn / workflow review-resolution / 4 项 v0 non-goal 自动化 仍 pending"
