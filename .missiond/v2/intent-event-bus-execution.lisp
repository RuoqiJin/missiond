...
(execution-log
  ...
  (deviations
    ...
    (deviation :id D016 :phase "v1.3.0-wire-drift-fix" :date "2026-04-19" :agent "ws-drift-elimination-executor"
               :lisp-said "v1.3.0 §4.6 event_log read-ui-projection (本 commit 升级) 新增 :ssot-wire-mapper = crates/missiond-core/src/event/wire_format.rs, :target 扩为 (projection.rs, wire_format.rs, ws_bridge.rs), :drift-elimination 明确 live 与 catch-up 共用 v2_payload_to_v1_shape → event_type=v1 wire_type (如 'board_task_created'), payload=flat v1 shape."
               :actually-did
                 ("(1) 新文件 crates/missiond-core/src/event/wire_format.rs (~790 行) — SSOT v1-wire mapper: 把原 daemon 侧 bus/ws_bridge.rs 的 52-arm v2_payload_to_v1_shape + v2_logged_to_v1_wire_format + 辅助 payload_of/provider_as_engine_string/fresh_ts_ms 全搬到 core; 新增 v1_wire_type_to_v2_parts 反向映射表 (覆盖 40+ wire_type → (domain, kind?) , 多对一如 slot_state_changed/user_message 返回 domain-only 匹配)."
                  "(2) crates/missiond-core/src/event/mod.rs — 加 pub mod wire_format;"
                  "(3) crates/missiond-daemon/src/bus/ws_bridge.rs — 删除本地 mapper (约 660 行) + 辅助函数 + 迁入 core 的 import (use missiond_core::event::wire_format::v2_logged_to_v1_wire_format); 保留 spawn_ws_bridge 和 12 个 byte-equiv 单测 (测试 import 改为 core 导出,证明 SSOT 经过 daemon 仍字节相等)."
                  "(4) crates/missiond-core/src/event/projection.rs — RawEventLogRow::into_timeline_row 改调 v2_payload_to_v1_shape, event_type 改为 v1 wire_type, payload 改为 flat v1 shape JSON 字符串; summary 仍从 raw externally-tagged payload 用 project_payload 重命名 extract_summary 抽取 (先抽 summary 再 wire-map); parse_event_type_filter 升级: 支持 'domain::kind' (内部) + v1 wire_type (前端) + unknown (legacy best-effort)."
                  "(5) 新增 5 个 catch-up SSOT 单测在 projection::tests: catch_up_emits_v1_wire_type_not_domain_kind / catch_up_slot_state_changed_wire_type / catch_up_message_logged_routes_by_role / summary_still_extracted_from_raw_payload / parse_event_type_filter_handles_all_input_forms — 锁死 event_type 不再出现 '::' 分隔符, payload 无 externally-tagged 残留, summary 仍能抽.")
               :deviations-from-plan
                 ("AI-facing stats/stratified 接口 (query_timeline_stats.by_type, query_timeline_stratified.type_limits map key) 仍用 'domain::kind' 形式, 未改. 理由: 这两个接口是给 MCP AI 分析层 (mission_timeline_stats / mission_timeline_stratified) 用的,AI 的语义辨识度更高 (如 'slot::became_idle' 明显好过 'slot_state_changed' 的合并歧义). UI WS catch-up + live 已 100% 对齐 v1 wire_type,这才是本 deviation 关心的 drift 范围."
                  "v1_wire_type_to_v2_parts 对几个多对一 wire_type (如 slot_state_changed/user_message/task_lifecycle/assistant_message/system_message/thinking_message) 返回 kind=None, 在 filter 路径解读为 'WHERE domain = $1' 的 domain-only 匹配 — 对 catch-up 过滤是 superset (比精确过滤多返几条其他 kind 的事件),但避免了为 slot/message/task/question 单独加 role/kind 细分 arg 的 API breaking."
                  "parse_event_type_filter unknown 输入回退到 'WHERE kind = $1' 的 legacy best-effort (沿用 D015 代码已存在的逻辑), 未改 — 因为改成 fail-fast 会破坏部分已发布 MCP 调用者的调用契约.")
               :test-status "cargo build --workspace clean (0 errors, 54 warnings same baseline); cargo test -p missiond-core --lib --features postgres wire_format 12/12 pass; cargo test -p missiond-core --lib --features postgres projection 5/5 new pass; cargo test -p missiond-daemon --bins ws_bridge 12/12 byte-equiv pass; full daemon 96 pass; full core 272 pass (255+12 wire_format+5 projection)."
               :reason "D015 已把 catch-up 换到 projection 但 event_type 从 v1 wire_type 变成 'domain::kind' — 与 live ws_bridge 输出漂移, 浏览器 WebSocket client 收到两种格式的 event.type 字段会 silently mismatch (live 推 'board_task_created', catch-up 回 'board::task_created'). SSOT 原则要求只有一份真理: 把 52-arm 映射从 daemon 搬到 core wire_format.rs, live 和 catch-up 都 import 同一 pub fn, 保证 byte-identical 输出. payload 字段同样从 externally-tagged JSON 改为 flat v1 shape — 全链路一致. 反向映射 v1_wire_type_to_v2_parts 让前端 catch-up 过滤请求 (按 event_type='board_task_created' 查) 也能翻译回 (domain, kind) SQL 条件, 双向对称."
               :approved-by "user task (WS catch-up/live drift elimination)"))
