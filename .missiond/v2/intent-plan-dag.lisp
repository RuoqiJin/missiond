...
(plan-dag-shard
  ...
  :status "code-aligned partial — runtime v2 已落 (wave 13 task 02); wave 15 task 05 (commit 615b249) 加 workstation-dispatch v0 opt-in — plan_dag.rs::dispatch_node 接 :workstation-dispatch true 节点路由到 workstation_dispatch.rs; 经 mission_task_delegate. wave 16 task 03 (commit 8ffa9b2) 加 workstation-dispatch auto-inference v1 (5 inference rules for mission_task_delegate scoped node + workstation_dispatch_source 5 值; anchor: intent-layer.unified-entry-pipeline.workstation-dispatch-v0). wave 16 task 04 (commit a51bc52) 加 paused 7th lifecycle + review-gate question-event trigger (节点 :review-gate 'question-event' + 可选 :review-action / :review-text → paused; deterministic review id 'review:plan:<plan_id>:v<v>:plan-node:<sha256(node_id)[..16]>'; aggregate_status='dag_paused' / runner_status='review_gate_paused'; bus failure 仍 pause + warning; 不实现 auto-resume; anchor: intent-layer.plan-dag-runtime-v2.paused-lifecycle). wave 16 task 05 (commit d8f8a6e) 加 per-node retry policy v0 (:retry-count additional / :max-attempts total / :retry-delay-ms cap 60s + cap 3 attempts; 每 attempt 写自己 evidence with attempt number; SafeDescriptor refusals 不 retry; failure-policy 与 retry 正交 retry exhaust 后 propagate_taint; anchor: intent-layer.plan-dag-runtime-v2.retry-policy-v0). wave 16 task 07 (commit 0e6ee63) 加 evidence subscriber 三档 live/log/unavailable (passive cache cap 1024 FIFO key 严格匹配 deterministic id; anchor: intent-layer.evidence-collector-event-ref). wave 17 task 01 (commit a42f5fd) 加 paused-resume hook v0 — mission_plan(action=execute) 4 字段 (resume_review_question_id / resume_review_decision / resume_actor / resume_note); 3 decision (approved 重 dispatch fresh attempt 1 / rejected 不 dispatch / needs_changes paused); spawn_review_resolution_sub 扩 plan-node action 自动 route 到 resume helper; evidence paused→resume_*; anchor: intent-layer.plan-dag-runtime-v2.paused-resume-hook-v0. wave 17 task 02 (commit 8661fcb) 加 claim-lease v0 + Claimed 第 8 lifecycle — claim_lease_secs (default 1800 clamp [60,14400]) / claimer_name (default 'plan-dag-scheduler') / enforce_claims (default false byte-compat); claim scope :owned-files → :scope → plan/<plan_id>/node/<node_id> fallback; scopes_overlap_pure 共享给 claim/audit/scoped-commit 三处; pending → claimed → running → {succeeded|failed} → released 五段 evidence; anchor: intent-layer.plan-dag-runtime-v2.claim-lease-v0. wave 17 task 03 (commit f572729) 加 acceptance evaluator v0 — 三模式 (inner_status / evidence_keys / manual); 绝不 shell; manual_required 复用 paused 不新加态; id space 'acceptance:plan:<plan_id>:v<v>:<node_id>' 与 review-gate 'review:*' 隔离 (各自 routing); anchor: intent-layer.plan-dag-runtime-v2.acceptance-evaluator-v0. wave 17 task 04 (commit d4466c6) 加 rollback policy v0 — 三模式 (none/descriptor/workstation) + 5 safety gates (missing objective / empty owned-files / no project signal / unsafe strategy / SafeDescriptor refusal); workstation 复用 build_task_brief / run_workstation_dispatch; rollback 在 final fail 后 / downstream taint 前; anchor: intent-layer.plan-dag-runtime-v2.rollback-policy-v0. wave 17 task 05 (commit 402fb82) 加 finalize + distill trigger v0 — finalize_plan / distill_on_success / distill_mode (default false / false / dry_run); 4 finalization rule (succeeded → succeeded; failed → failed; partial → failed; paused → unchanged 不撒谎); distill 经现有 super::workflow::handle() 不动 workflow.rs; distill 失败 warning 仍保 plan final state; anchor: intent-layer.plan-dag-runtime-v2.finalize-distill-v0. 完整 11-stage 大量 close (s5 claim ✓ / s7 acceptance ✓ / s8 paused-resume ✓ / s9 rollback ✓ / s10 mark-final ✓ / s11 distill 触发 ✓); 仍 partial 因为 cross-plan distill chain / 跨节点 acceptance fan-in / 自动 compensate-node + cascade-up rollback / 切 enforce default 仍 future (anchor: intent-layer.actor.plan-dag-scheduler)"
