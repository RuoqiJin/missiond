...
(plan-dag-shard
  ...
  :status "code-aligned partial → code-aligned (wave 18 close 主线 + wave 19 加 task-contract emitter + auto-chain + forward compensate ref) — runtime v2 已落 (wave 13 task 02); wave 15 task 05 (commit 615b249) 加 workstation-dispatch v0 opt-in — plan_dag.rs::dispatch_node 接 :workstation-dispatch true 节点路由到 workstation_dispatch.rs; 经 mission_task_delegate. wave 16 task 03 (commit 8ffa9b2) 加 workstation-dispatch auto-inference v1. wave 16 task 04 (commit a51bc52) 加 paused 7th lifecycle + review-gate question-event trigger. wave 16 task 05 (commit d8f8a6e) 加 per-node retry policy v0. wave 16 task 07 (commit 0e6ee63) 加 evidence subscriber 三档 live/log/unavailable. wave 17 task 01 (commit a42f5fd) 加 paused-resume hook v0. wave 17 task 02 (commit 8661fcb) 加 claim-lease v0 + Claimed 第 8 lifecycle. wave 17 task 03 (commit f572729) 加 acceptance evaluator v0 三模式 (inner_status / evidence_keys / manual). wave 17 task 04 (commit d4466c6) 加 rollback policy v0 三模式 (none/descriptor/workstation) + 5 safety gates. wave 17 task 05 (commit 402fb82) 加 finalize + distill trigger v0. wave 18 task 02 (commit 2fbaa88) 加 ExecutionEvent dispatch metadata v1 — Claimed + Completed 加 dispatch_strategy / target_project / requested_cwd (serde-bw-compat 不要求 caller 重传); anchor: event-bus.section.execution-event.dispatch-metadata-v1. wave 18 task 03 (commit 76ada10) 加 cross-node acceptance fan-in v0 — :acceptance-depends-on / :acceptance-requires (all_succeeded|any_succeeded|evidence_keys) / :acceptance-source-node + 4 validator rejection (missing dep / requires missing / source invalid / not ancestor cycle guard); evidence 写 acceptance_fan_in 块; anchor: intent-layer.plan-dag-runtime-v2.cross-node-acceptance-fan-in-v0. wave 18 task 04 (commit 7de886c) 加 cascade rollback v0 — :compensates / :rollback-cascade (none|plan|dispatch-safe) / :rollback-after; Kahn 拓扑排序 + cycle-tolerant fallback; 5 safety triggers; 复用 wave 17-04 workstation_dispatch substrate; cascade 中 acceptance_commands_executed: false 钉死; anchor: intent-layer.plan-dag-runtime-v2.cascade-rollback-v0. wave 18 task 05 (commit 76d818d) 加 cross-plan distill chain v0 — distill_chain_id / distill_chain_mode (record_only|dry_run|sonnet) / distill_chain_name; chain metadata 写 evidence sidecar **无 SQL migration**; 5 skip 状态; append-only 3 层保证; sonnet 仅显式触发; anchor: intent-layer.plan-dag-runtime-v2.cross-plan-distill-chain-v0. wave 18 task 06 (commit 91f8fa1) 加 autonomous PLAN field inference v0 — infer_plan_fields (off|preview|apply_safe) + 6 字段 (target / dispatch_strategy / target_project / owned_files / acceptance_mode / workstation_dispatch) + 3 confidence; 仅 high apply_safe 落; 3 evidence sources; conflict 永不静默 override; 完全无 LLM (deterministic heuristics); anchor: intent-layer.plan-dag.autonomous-plan-field-inference-v0. wave 19 task 06 (commit 5d425e2) 加 plan task-contract emitter v0 — mission_plan(execute) 加 emit_task_contract / task_contract_mode opt-in args; eligible workstation nodes (target=mission_task_delegate ∨ workstation_dispatch=true) 写 .missiond/tasks/generated/<plan_id>/<node_id>.lisp 用 task-contract v1; response 加 task_contract_path + render_command; **emit before dispatch** 失败不进 inner handler; default off byte-compat; anchor: intent-layer.plan-dag.task-contract-emitter-v0. wave 19 task 09 (commit e57b56d) 加 cross-plan distill auto-chain v1 — mission_workflow(distill) 加 auto_chain / auto_chain_name opt-in; deterministic id 'chain:auto:wf-<sha256(project_root + plan_id + workflow_anchor + evidence_sha256)[..16]>' US 隔离防碰撞; sonnet 仍仅显式; append-only 无 migration; default off byte-compat; anchor: intent-layer.plan-dag-runtime-v2.cross-plan-distill-auto-chain-v1. wave 19 task 10 (commit 246a675) 加 PLAN DAG forward compensate ref v0 — :compensate-node / :compensate-ref 双拼写 + underscore alias; 4 拒绝条件 (self / unknown / empty / direction mismatch); compute_compensation_order union forward + reverse; wave 18-04 5 safety gates 完整保留; anchor: intent-layer.plan-dag-runtime-v2.forward-compensate-ref-v0. 完整 11-stage 几乎全部 close (s5 claim ✓ / s7 acceptance + cross-node fan-in ✓ / s8 paused-resume ✓ / s9 rollback + cascade + forward compensate ref ✓ / s10 mark-final ✓ / s11 distill 触发 + cross-plan chain + auto-chain ✓); 仍 pending 是: LLM auto-approve / 完全 autonomous spawn (无 hint) / git pre-commit hook / sonnet 自动接 chain (5 rule 全过) / LLM-augmented PLAN inference (anchor: intent-layer.actor.plan-dag-scheduler)"
