...
(intent memory
  (version "v0.5.8"
    :polish "v0.5.0 (2026-04-20) L1 语义压缩 — history/migration-log 外移到 intent-memory-history.lisp; table-catalog SSOT 合并到 module (保 count + 表名索引); 行数 2999 → 2629 (-12.7%)"
    :patch-history-summary "v0.5.1 agent-execution-coordination 协议升格 (D010 教训) → v0.5.2 recursive-contract alignment → v0.5.3 file-first implementation alignment (intent-alignment/PLAN/workflow .lisp 三段) → v0.5.4 capability-usage-read-model 新增 → v0.5.5 directive/plan/workflow 管理面 + unified-entry-pipeline file-first 加固 → v0.5.6 actor v0 status backfill + semantic evidence v1 → v0.5.7 wave 11 plan-dag-scheduler 设计 + artifact plan-node-state-projection → v0.5.8 execution handoff dual-plane (control + durability) + scoped-commit-contract"
    :v0.5.8-wave-13-note "wave 13 task 01/02/03 真实状态 (lisp 不增主版本, 仅 status 回填; 详 intent-pillar-source-index.lisp v0.4): plan-evidence-sidecar 已升 typed EvidenceEntry (wave 13 task 01 commit 88568a9, plan.rs + plan_dag.rs 接入, EventRef::unavailable 占位); plan-node-state-projection 顶层 sidecar 字段已升 code-aligned partial (wave 13 task 02 commit 8bb6110 plan_dag runtime v2: 6 lifecycle + 3 skip 子分类 upstream_failed/fail_fast_aborted/condition_gated); writer plan-runner-v0 单节点 + runtime v2 wave-based concurrency 子集 (max_parallel_nodes / failure-policy fail-fast vs continue); 完整 11-stage scheduler / file-first .lisp writer 仍 pending"
    :v0.5.8-wave-14-note "wave 14 task 01/02/03 真实状态 (lisp 不增主版本, 仅 status 回填; 详 intent-pillar-source-index.lisp v0.5): file-first writer integration (wave 14 task 01 commit 00cbc1d) — 三类 artifact (directive alignment / PLAN.lisp / workflow methodology) 全部走统一 helper file_artifacts::attempt_artifact_write → resolve_target_project_root → atomic_write_artifact; 严禁 process cwd fallback; partial 语义 (DB 已写, file 失败 → status=partial + file_write_error); 6 file_* 响应字段; foundation helper file_artifacts.rs dead_code 全清; PlanNodeStateChanged variant + live EventRef 三层策略 (wave 14 task 02 commit 2e7789a) — ExecutionEvent::PlanNodeStateChanged 4 必 + 5 可选 (含 dispatch_strategy/target_project); evidence collector 走 live id → deterministic id → unavailable 兜底; review-gate auto-create v1 (wave 14 task 03 commit 96842cd) — review_gate policy enum + deterministic id + file 写失败拒发 — anchor 详 intent-pillar-source-index.lisp :: memory.directive-layer.file-first-writer-integration / event-bus.section.execution-event.plan-node-state-changed / intent-layer.unified-entry-pipeline.review-gate-policy"
    :v0.5.8-wave-15-note "wave 15 task 02/04 真实状态 (lisp 不增主版本, 仅 status 回填; 详 intent-pillar-source-index.lisp v0.6): file-first-artifacts (5 artifact 详细字段 + plan-evidence-sidecar + plan-node-state-projection) + file-first-writer-integration anchor 已 物理 split 到 .missiond/v2/intent-directive-artifacts.lisp shard (wave 15 task 02 commit 3f37d32; section-id 全保 R008); review-gate explicit resolution bridge v0 (wave 15 task 04 commit 03513c0) — 显式 review_decision (approved|rejected|needs_changes) + review_actor + review_note + envelope validator 5 fail-fast 错误码; 接到 directive (approve/archive) + plan (approve/mark/supersede); workflow handler 接入仍 pending — anchor 详 intent-pillar-source-index.lisp :: intent-layer.unified-entry-pipeline.review-gate-resolution-v0 / intent-layer.l2-shard-split.executed"
    :v0.5.8-wave-16-note "wave 16 task 01/04/05 真实状态 (lisp 不增主版本, 仅 status 回填; 详 intent-pillar-source-index.lisp v0.7): workflow handler 接入 review-resolution (wave 16 task 01 commit 01708be) — mission_workflow(action='resolve_review') 5 字段; methodology YAML 不 fake DB (receipt only); workflow row 无 status/version 列 → 仅 stamp 不 DB transition; review-gate-resolution-v0 升 code-aligned (3 surface 全接 directive/plan/workflow). plan-node-state-projection 顶层 sidecar 字段加 paused 7th lifecycle (wave 16 task 04 commit a51bc52, 节点 :review-gate 'question-event' 触发 paused) + per-attempt evidence (wave 16 task 05 commit d8f8a6e, 每 retry attempt 写自己一条 evidence with attempt number); plan-evidence-sidecar shape 仍向后兼容 v0 plan_runner_dispatch entry — anchor 详 intent-pillar-source-index.lisp :: intent-layer.unified-entry-pipeline.review-gate-resolution-v0.workflow-handler / intent-layer.plan-dag-runtime-v2.paused-lifecycle / intent-layer.plan-dag-runtime-v2.retry-policy-v0")
  (parent "v2/intent.lisp :: pillar memory")
  (created "2026-04-19")
  (history
  ...
  (module directive-layer
    ...
    ;; ── file-first-artifacts moved to L2 shard ──
    ;; Full content moved to .missiond/v2/intent-directive-artifacts.lisp
    ;; (wave 15 task 02 L2 shard split per architecture-dsl.lisp ::
    ;;  l2-shard-split-plan :: shard intent-directive-artifacts).
    (file-first-artifacts
      :status "moved-to-shard (code-aligned partial)"
      :file-ref ".missiond/v2/intent-directive-artifacts.lisp"
      :shard-section "file-first-artifacts"
      ...
      :writer-integration-cross-ref "memory.directive-layer.file-first-writer-integration (also in shard)"
      :note "5 artifact (intent-alignment-lisp / plan-lisp / workflow-lisp / plan-evidence-sidecar / plan-node-state-projection) live in shard; section-ids stable per L2 plan rule-1")
    ...
    (module-ingress
      ...
      (writer directive-compiler-v0
        :binds-to [:mcp-surface]
        :status "code-aligned partial — mission_directive(compile, compiler_mode=sonnet) 走 SonnetGateway interactive lane 编译 directive sexp + 校验 fenced block/parens/allowed top-level head; persist=true 写 draft row; auto multi-round refine + file 自动写入仍 pending"
      ...
      (writer plan-compiler-v0
        :binds-to [:mcp-surface]
        :status "code-aligned partial — mission_plan(compile, compiler_mode=sonnet) 从 approved directive / board task 编译 plan sexp DAG/FSM; compiled_from=directive/<id>:<version> 或 board_task/<id>; persist=true 写 awaiting_approval (不自动 approve); planner-class model alias + file 自动写入仍 pending"
      ...
      (writer plan-runner-v0
        :binds-to [:mcp-surface]
        :status "code-aligned partial — mission_plan(execute internal) dispatch 到 mission_execution/task_delegate/flow_run + 写 evidence sidecar + 推 plan FSM executing (单节点); auto-selection v1 从 plan.sexp_text 保守解析 hints (explicit 优先, 无法推断 MISSING_PARAM); plan_dag runtime v2 (max_parallel_nodes / wave-based / 6 lifecycle + 3 skip / failure-policy fail-fast vs continue / per-node typed evidence); record_evidence typed wrap; 完整 11-stage scheduler / arbitrary semantic interpretation 仍 pending — 详 intent-layer pillar :: actor plan-dag-scheduler (anchor: intent-layer.plan-dag-runtime-v2)"
        :writes "plan FSM (status → executing) + .missiond/v2/plans/<plan_id>.evidence.json (plan_runner_dispatch + plan_dag_node_dispatch entry per node)"
        ...
        :v1-future-target "writer plan-dag-scheduler — actor 完整 11-stage 实现后, 此 writer 升级为 DAG 调度循环 + per-node FSM + claim/lease (复用 agent-execution-coordination); v0 单节点 fast-path 必须保留 (max_parallel_nodes=1)"
        :pending ["完整 11-stage PLAN DAG scheduler (claim-lease / per-node retry / acceptance / rollback / review-gate paused / mark-plan-final / trigger-record-execution-distill)"
                  "arbitrary PLAN.lisp 语义解释"
                  "PLAN file (.missiond/plans/<topic>/PLAN.lisp) 自动写入"
                  "auto QuestionEvent gate"
                  "ExecutionEvent dispatch metadata 扩展 (anchor: intent-layer.plan-dag-runtime-v2.execution-event-decision)"
      ...
      (writer workflow-distiller-v0
        :binds-to [:mcp-surface]
        :status "code-aligned partial — mission_workflow(distill, distill_mode=sonnet) 读 succeeded plan + evidence sidecar 生成 workflow sexp + match_rules JSON; persist=true 写 draft/template; methodology compiler v0 (compile_mode=deterministic, paren-validate + (step …) 抽取 + executable YAML); generated flow loader; 高阶 semantic lifting / forge compiler / 自动 record_execution-distill 联动 / file 自动写入仍 pending"
  ...
  (module system-support
    ...
    ;; ── derived-read-model capability-usage-read-model moved to L2 shard ──
    ;; Full content moved to .missiond/v2/intent-capability-governance.lisp
    (derived-read-model capability-usage-read-model
      :status "moved-to-shard (code-aligned v0.5.6 — semantic evidence v1)"
      :file-ref ".missiond/v2/intent-capability-governance.lisp"
      :shard-section "derived-read-model capability-usage-read-model"
      :section-id "memory.system-support.capability-usage-read-model"
      :owner-module "system-support"
      :note "truth-sources / ingress / logic-core / egress / pending live in shard; section-id stable per L2 plan rule-1")
