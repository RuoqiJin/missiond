  (workstation-config
    ...
    (chat-completions-policy jarvis-api
      ...
      :rule "OpenAI-compatible /v1/chat/completions routes to the explicit X-Slot-Id header when present; otherwise it uses this V3-projected default slot. Rust must not hardcode slot-jarvis. Jarvis broad requests MUST pass the strict intent/plan gate: emit intent_draft and await user confirmation, then emit plan_draft and await confirmation, then create BoardTask metadata with grounding_context_id, intent_artifact_id, and plan_artifact_id before any worker dispatch. Jarvis plan-confirmed dispatch classifies the user objective verb into review/design/implementation/verification via classify_jarvis_dispatch_verb: implementation verbs (implement, fix, create, build, add, refactor, migrate, 补齐, 接入, 实现, 修复, 重构, 提交, 推送) produce task_class=code with non-empty write_scope; when the objective explicitly asks for Codex, dispatch MUST use engine_hint=codex and pool_hint=codex-code-worker, otherwise scoped code work may use engine_hint=claude_code and pool_hint=claude-code-default. review/smoke/read-only verbs remain task_class=review, write_policy=read-only with empty write_scope. /jarvis/api/monitor/jarvis is read-only and may report auto_heal policy; /internal/jarvis/slot/ensure is a localhost-only deploy smoke control surface for restoring the default Exited/Error slot after blue-green restart. Chat requests and local ensure may attempt at most one restart of an Exited/Error default slot when MISSIOND_JARVIS_SLOT_AUTO_HEAL=1; failure returns JARVIS_SLOT_UNAVAILABLE or JARVIS_SLOT_AUTO_HEAL_FAILED typed diagnostic and never falls back to direct local code search or fake output.")
    ...
    :invariants
      ["code and research dynamic slots MUST NOT hardcode --model sonnet"
       ...
       "If pty.send returns an active/progress frame but the provider later records a durable final assistant message in claude-jsonl/codex-local-index/gemini-chat-file, or the master/worker later writes a durable BoardTask summary note, and the claimed slot is idle/exited/error, Autopilot watchdog MUST synthesize the missing BoardTask summary when needed and close the running BoardTask from durable provider-or-note evidence + idle-or-terminal slot diagnosis; it MUST NOT wait for the full timeout/grace or close from PTY idle alone. A terminal provider slot (Exited/Error) is not a busy state; after durable final lookup fails, it may be unclaimed by the same watchdog recovery policy instead of wedging the task indefinitely."]
    (prompt-tool-contract autopilot-claudecode-prompt
      ...
      :artifact-owned-close
        (:mode orchestrator-owned
         ...
         :close-owner "Autopilot/orchestrator closes after durable provider final settle and task-result-artifact validation."
    ...
    (execution-ownership delegated-boardtask
	 ...
	 :close-owner
        (:default "Autopilot is the close owner — when state.pty.send returns Complete, Autopilot transitions the BoardTask running→done and writes the summary note."
         :legacy-worker-self-close "If an older prompt or manually operated slot already drove the BoardTask to Done via mission_board_update before pty.send returns, Autopilot preserves the Done state but treats it as a legacy projection; Jarvis/external clients still require task-result-artifact validation before final streaming."
