  (workstation-config
    ...
    (interactive-provider-box
      ...
      :outputs [turn_id lease_id slot_id provider_conversation_id durable_source final_text artifact_hash diagnostics transcript_refs step_records screen_identity screen_usage usage_snapshot_ref model_catalog_ref router_export_ref model_switch_result]
      :http-adapter
        (:base "/provider-box/v1"
         ...
         :rule "The HTTP adapter is a thin MissionD-owned facade over provider-interaction-box. It validates text-only guards and auth, then calls the box; it does not operate provider CLIs itself. xjp-router MissionDAgy calls /provider-box/v1/text-only/completions through the managed proxy URL.")
      :public-commands
        ((command worker-turn
      ...
      :forbidden-headless-provider-cli [claude-print claude-p claude-prompt codex-exec codex-output-last-message agy-print agy-prompt gemini-prompt-stream-json stdin-closed-provider-subprocess]
      :mode-contracts
      ...
      :current-implementation-gaps
        ["mission_compute_slot(create) can spawn dynamic slots with spawn-time model/profile parameters, but it is lifecycle-oriented create/list/extend/terminate and does not yet expose provider-interaction-box worker-turn semantics or durable final return."
         ...
         "xjpcode/MissionD text-only provider lanes remain legacy migration adapters; the target is pure-text-single-turn through the box with enforcement and durable final correlation."]
      :legacy-headless-migration-targets
        ((target missiond-runner-claude-print
         ...
         (target jarvis-codex-author-exec
    ...
    (chat-completions-policy jarvis-api
      ...
      :direct_answer_provider xjpcode-text-only
      :direct_answer_default_text_provider codex_cli
      :direct_answer_provider_env MISSIOND_JARVIS_DIRECT_ANSWER_PROVIDER
      :direct_answer_endpoint_env [MISSIOND_XJPCODE_TEXT_ONLY_URL MISSIOND_XJPCODE_TEXT_ONLY_ENDPOINT MISSIOND_XJPCODE_BASE_URL]
