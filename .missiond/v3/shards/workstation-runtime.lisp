  (workstation-config
    ...
    (model-profile codex-gpt-5-5-default
      :applies-to [worker-turn direct-answer operator-teaching]
      :spawn-model-arg "gpt-5.5"
      :reasoning-effort nil
      :search true
      :sandbox workspace-write
      :approval-policy never
      :rule "Provider-box callers that ask for GPT-5.5 default reasoning pass model=gpt-5.5 and omit model_profile/model_reasoning_effort so Codex CLI keeps the model default.")
    ...
    (interactive-provider-box
      :schema "missiond.interactive-provider-box.v1"
      :status migration-target
      :purpose "Single provider-CLI interaction boundary for MissionD, xjpcode, runner, worker, router-adapters, Jarvis, and external app callers. Callers submit structured turn/control/catalog requests; the box owns lease validation, PTY spawn/reuse, provider-driver dispatch, durable provider-log final extraction, timeout/cancel, usage/model/catalog observations, and result artifact projection. Provider-specific key sequences, menu navigation, and CLI quirks live only behind driver capabilities; external callers never script PTYs directly."
      :public-contract "missiond.provider-interaction-turn.v1"
      :callers [jarvis-intent-author jarvis-grounding-worker jarvis-plan-author grounded-direct-answer missiond-runner board-worker-adapter codex-vision-worker gemini-router-cli provider-router-exporter xjpcode-text-provider-adapter external-apps]
      :inputs [provider engine model model_profile dangerously_bypass_approvals_and_sandbox cwd project_root prompt attachments mode timeout_secs timeout_cancel_policy correlation_id artifact_contract output_contract read_scope write_scope no_tools no_mcp no_shell no_file_access tool_policy desired_worker model_switch_policy single_turn_policy router_export_policy]
      :outputs [turn_id lease_id slot_id provider_conversation_id provider_session_identity durable_source final_text artifact_hash diagnostics transcript_refs step_records slot_status screen_identity screen_usage usage_snapshot_ref model_catalog_ref router_export_ref model_switch_result]
      :http-adapter
        (:base "/provider-box/v1"
         :auth "internal bearer token required; public traffic reaches it only through the self-built proxy deployment program"
         :endpoints ["GET /provider-box/v1/models"
                     "GET /provider-box/v1/usage"
                     "POST /provider-box/v1/usage/refresh"
                     "POST /provider-box/v1/turns"
                     "POST /provider-box/v1/text-only/completions"
                     "POST /provider-box/v1/slots/spawn"
                     "GET /provider-box/v1/slots/<slot_id>/status"
                     "GET /provider-box/v1/slots/<slot_id>/session"
                     "GET /provider-box/v1/slots/<slot_id>/capabilities"
                     "POST /provider-box/v1/slots/<slot_id>/restart"
                     "POST /provider-box/v1/slots/<slot_id>/input"
                     "POST /provider-box/v1/slots/<slot_id>/switch-model"
                     "POST /provider-box/v1/slots/<slot_id>/usage/refresh"
                     "POST /provider-box/v1/slots/<slot_id>/exit"
                     "POST /provider-box/v1/slots/<slot_id>/clear"
                     "POST /provider-box/v1/slots/<slot_id>/permissions/<mode>"
                     "POST /provider-box/v1/slots/<slot_id>/fast/<mode>"
                     "POST /provider-box/v1/slots/<slot_id>/completions"]
         :rule "The HTTP adapter is a thin MissionD-owned facade over provider-interaction-box. It validates text-only guards and auth, then calls the box; it does not operate provider CLIs itself. GET /models returns provider-model-catalog, provider-router-export, OpenAI-style data[], and provider_text_only_sources[] for each exportable observed AGY model, including logical slot_pool_id, provider-box-managed replica policy, and text-only completion request templates, but never public replica slot ids; GPT-OSS is intentionally not exported as a text-only provider source. GET /models may also list guarded Codex exec text sources for GPT-5.5 default and xhigh reasoning, and guarded ClaudeCode text sources for exactly claude-code-opus-4-8-xhigh, claude-code-opus-4-6-high, and claude-code-sonnet-4-6-high; claude-code-default is not exported because the default model can drift. Codex sources are routeable only after live marker-smoke tests prove the guarded lane returns no tool/function/MCP events; ClaudeCode text sources are routeable only when the provider-box lane can launch an interactive ClaudeCode PTY with a per-request session-id, fixed trusted runtime cwd, --model/--effort for the selected logical source, --tools '', --strict-mcp-config, an empty --mcp-config file, --disable-slash-commands, durable JSONL final extraction, fail-closed tool/MCP/file/shell/web-search violation scanning, and fail-closed provider API error detection from ClaudeCode JSONL system/assistant error events. xjp-router MissionDAgy calls /provider-box/v1/text-only/completions through the managed proxy URL; slot-scoped /provider-box/v1/slots/<slot_id>/completions is internal maintenance/debug surface and is not exported as a router source. Router-facing AGY text-only requests use fixed-model hidden slots by default: the box verifies the visible current model on the internally selected slot and MUST NOT run interactive /model switching in the request hot path unless allow_model_switch=true is explicitly set or the hidden replica was just cold-started after missing/exited/error/reclaimed state and needs its pinned model bootstrap. Hidden AGY text-only replica slots are cold-startable and may be reclaimed after two hours without requests; the next logical request respawns and re-verifies the pinned model. ClaudeCode text-only requests MUST NOT accept external slot_id; provider-box generates a private ephemeral slot and session-id per request, hides slot/session paths from HTTP responses, and exits/kills the ephemeral PTY after durable final extraction so no context is reused. Slot status/session/capabilities APIs attach provider_session_identity and a provider-box capability matrix when durable metadata can identify the active provider conversation: AGY uses exit resume text, cache/last_conversations.json, or history.jsonl workspace mappings; Codex uses rollout JSONL session_meta.payload.id with cwd matching, and both expose provider_session_ref such as AGY-<conversation_uuid> or CODEX-<session_uuid> plus the provider-native resume command. Codex slot spawn/status APIs accept model=gpt-5.5 plus either model_profile=xhigh/model_reasoning_effort=xhigh for the high-reasoning lane or no reasoning field for default model reasoning; trusted internal Codex lanes may set dangerously_bypass_approvals_and_sandbox=true, which maps to Codex CLI --dangerously-bypass-approvals-and-sandbox and suppresses separate sandbox/approval flags. Codex /exit is exposed as a verified control action: provider-box types /exit and Enter separately, then verifies the prior pid exited or the slot restarted. Explicit restart is exposed separately as POST /provider-box/v1/slots/<slot_id>/restart and MUST require confirm_destroy_context=true because it destroys potentially valuable context. Codex MCP reconnect is intentionally not advertised as hot-reconnect capable; the mcp/reconnect endpoint returns restart_required guidance instead of implicitly restarting the slot. Codex exec text-only requests are a separate guarded provider-box lane: they run codex exec --json --output-last-message in an isolated runtime workspace with --ignore-user-config, --ignore-rules, read-only sandbox, approval_policy=\"never\" config, shell/web/apps/view_image config disables, stdout JSONL tool-event inspection, and fail-closed output-last-message extraction; output-last-message alone is never treated as proof that no tools ran. Codex image-generation requests are a task lane, not a text-only export: they keep Codex user config/skills and the first-party image-generation tool surface enabled for imagegen, disable MCP/shell/web/view-image, allow only the observed node_repl bootstrap/continuation that reads or slices `$CODEX_HOME/skills/.system/imagegen/SKILL.md`, require Codex rollout JSONL `image_generation_end.saved_path`, validate the generated PNG under CODEX_HOME/generated_images, import the PNG into xjp-image-service through its authenticated /v1/images/import endpoint, keep `IMAGE_DONE <signed_url>` only as an internal marker, and expose media_artifact, image_artifact, imageUrl, and image content_parts from provider-box HTTP responses; import, signing, or artifact validation failure MUST fail closed without returning local paths. GPT-5.5 default text has max_concurrent=4, GPT-5.5 xhigh text has max_concurrent=2, Codex research plus Codex image-generation each have max_concurrent=1, and each ClaudeCode exported text source has max_concurrent=1 on its provider-box-owned logical queue; queue completion means durable JSONL/result extraction has finished and the private execution context is no longer serving another caller. GET /provider-box/v1/usage returns the last cached provider usage snapshot selected by engine/provider query without launching or touching provider PTYs; POST /provider-box/v1/usage/refresh owns a dedicated provider-box usage probe slot per engine, using slot-agy-usage-probe for AGY /usage and slot-codex-usage-probe for Codex /status, and MUST NOT use text-only export replica slots unless a slot-scoped internal debug endpoint explicitly names one. External apps may use slot-scoped spawn/status/session/capabilities/input/switch-model/usage/exit/restart/clear APIs only as internal operator controls; those APIs still dispatch to provider drivers and return step_records plus slot_status rather than exposing raw PTY scripting.")
      :public-commands
        ((command worker-turn
           :request "external app, runner, or worker asks for a provider worker with params and a concrete result"
           :result "provider-interaction-turn plus optional task-result-artifact/interaction-direct-answer")
         (command model-switch
           :request "caller asks to switch an existing lease/slot/provider conversation to a target model"
           :result "model_switch_result with verified/applied/unsupported/unknown status")
         (command usage-probe
           :request "caller asks what this provider slot/account can currently use"
           :result "provider-usage-snapshot with exact/estimated/blocked/unknown confidence")
         (command model-catalog-export
           :request "caller asks to discover provider models and publish routeable entries for router"
           :result "provider-model-catalog and provider-router-export when discovery and routeability checks pass")
         (command pure-text-single-turn
           :request "caller asks for plain text only with no CLI tools, no shell, no files, no MCP, and no hidden multi-step provider work"
           :result "provider-interaction-turn whose output_contract is text/plain, or a typed violation/unsupported diagnostic")
         (command slot-status
           :request "caller asks to observe whether a provider CLI slot exists, is running, is idle/running/blocked/exited, and what recognized screen_identity/screen_usage is visible"
           :result "provider-interaction-turn with slot_status, screen_identity/screen_usage when visible, diagnostics, and no semantic answer text")
         (command control-action
           :request "caller asks for a named interactive operation such as raw composer input, clear_screen, or exit"
           :result "provider-interaction-turn with verified observe-act-observe step_records and final slot_status"))
      :transport-rule "Provider CLIs MUST normally be launched or reused as interactive PTY sessions through missiond-pty/slot-orchestrator. Input is delivered by the same path a human operator would use: paste/type/enter and provider-specific attachment UI when supported. ClaudeCode text-only export remains an interactive PTY lane, not `claude -p`, `--print`, stdin-closed, or headless subprocess mode: provider-box creates a private ephemeral ClaudeCode PTY, submits text and Enter as separate actions, and reads the answer only from the per-request session JSONL cursor. The only approved MissionD-owned exception is provider-box codex_exec_text, which may run codex exec as a guarded text-source adapter only when sandbox/approval/tool-disabling flags are applied, stdout JSONL is scanned for tool/function/MCP events, output-last-message is treated only as the durable final carrier, and the source is routeable=false until live smoke validates the guard. The semantic answer is read only from durable provider conversation sources, such as ClaudeCode JSONL, Codex rollout JSONL, Agy transcript_full.jsonl/history.jsonl, Gemini session files, codex_exec_text output-last-message plus JSONL guard, or a canonical task/interact artifact; PTY screen text is diagnostic only. semantic-terminal/missiond-pty recognition is a screen-to-state parser boundary: it may classify identity, usage, running, blocked, complete, and command-surface states, but MissionD provider-box owns leases, API contracts, retries, control actions, prompt submission, model switching, usage refresh, durable final extraction, and router/xjpcode export."
      :operator-teaching-rule "CLI help/learning flows that need a shell prompt MUST use an operator-confirmed diagnostic PTY shell and MUST NOT repurpose provider-box task turns, headless provider subprocesses, or stdin-closed one-shot execution. The diagnostic shell is live-screen evidence only; exported LLM answers still come from provider-box durable final sources. AGY 1.0.5 help confirms --model is a launch-time interactive session selector, while --print/-p/--prompt are non-interactive one-shot modes and stay forbidden for provider-box lanes; --dangerously-skip-permissions and --sandbox are launch toggles only. Codex CLI help and OpenAI Codex docs confirm --model/-m, --config/-c, --sandbox, --ask-for-approval on the interactive command, and --dangerously-bypass-approvals-and-sandbox; codex exec on the local version does not accept --ask-for-approval, so provider-box codex_exec_text uses -c approval_policy=\"never\" instead. ClaudeCode MissionD-owned worker/workstation slots default to provider-box or slot-orchestrator launch/restart with `claude --dangerously-skip-permissions` plus the internal capability override; the skip-permissions capability is a spawn/restart policy, not an answer-generation shortcut or public router text source. Explicit bypass/skip request fields remain accepted for operator controls, but ClaudeCode text-only lanes do not inherit the worker default. ClaudeCode text-only lanes use launch flags rather than global profile mutation: --model, --effort, --tools '', --strict-mcp-config, --mcp-config <empty-file>, --disable-slash-commands, and --session-id <uuid>. model_reasoning_effort is a documented config key with minimal/low/medium/high/xhigh values, so provider-box uses model_reasoning_effort=xhigh for GPT-5.5 xhigh and omits that override for default reasoning."
      :driver-capabilities
        ((capability submit-turn
           :rule "Driver accepts a normalized prompt/attachment/output contract and performs provider-specific interactive input. The driver may know key sequences later, but the public box contract only observes submitted, running, complete, blocked, or unsupported.")
         (capability stepwise-operator
           :rule "Every provider driver MUST run as observe -> act -> observe -> verify -> record. A driver records the pre-action observation, the exact human-like input action, the post-action observation, the expected UI/state delta, and a verified/unchanged/ambiguous/failed status before any next action. Multi-key workflows such as model switching are never emitted as blind key bursts.")
         (capability provider-prompt-authorization
           :rule "Provider authorization prompts are first-class blocked states, not task failures. ClaudeCode MCP authorization screens such as `New MCP server found in this project: supabase` are recognized as startup_provider_authorization/provider_authorization. Provider-box may only auto-confirm when the target matches the V3-projected provider_authorization_allowlist or MISSIOND_PROVIDER_BOX_CLAUDE_CODE_MCP_AUTH_ALLOWLIST, and it MUST use observe-act-observe selected-option verification before pressing Enter. Unknown authorization targets, missing selected option, or inability to verify the allow option fail closed with DIAG_PROVIDER_CONTROL_ACTION_UNVERIFIED; PTY screen text remains diagnostic evidence only.")
         (capability screen-state-recognition
           :rule "Every observe snapshot MUST attach structured screen_identity when visible. For Agy this includes cli_version from Antigravity CLI version, logged-in account, plan, current model, selected model in model picker, and cwd. For ClaudeCode this includes cli_version from `Claude Code v...`, current_model from the visible model line (for example Opus 4.8), reasoning_effort from `xhigh effort`, plan from the account plan label (for example Claude Max), permission_mode from the footer (`auto`, `default`, `accept_edits`, `plan`, or launch-time `bypass_permissions`), login-success account from `Logged in as ...`, and cwd. The ClaudeCode default footer may appear only as `? for shortcuts`, so recognition MUST preserve that as permission_mode=default. ClaudeCode running recognition MUST treat upstream spinner rows as randomized `<verb>…` activity from spinnerVerbs.ts, including examples such as Whirlpooling, Puzzling, Combobulating, and Spelunking, but MUST decide current activity from observe context: active spinner plus current user prompt/footer evidence is running, while stale spinner scrollback followed by a ready prompt is idle. ClaudeCode `/logout` running recognition MUST report phase=logout, and may use reason claude_code:logout_running. ClaudeCode completion recognition MUST accept the upstream turnCompletionVerbs.ts forms such as Worked/Baked/Brewed/Churned/Cogitated/Cooked/Crunched/Sautéed for <duration>. ClaudeCode first-run theme selection is startup_config blocked state with selected option extraction, not idle. ClaudeCode login method selection is auth_missing blocked state with startup_prompt_kind=login_method plus selected option extraction, not idle. ClaudeCode OAuth browser/manual-code page is auth_code_required blocked state with startup_prompt_kind=oauth_authorization; OAuth URL challenge/state/client_id values are secrets and MUST NOT be exported as structured diagnostics. ClaudeCode login-success continue page is startup_continue blocked state with startup_prompt_kind=login_success_continue and MUST wait for a separate Enter action. ClaudeCode security-notes continue page is startup_continue blocked state with startup_prompt_kind=security_notes_continue and MUST wait for a separate Enter action. Agy active turn recognition treats bottom/recent Braille spinner status lines plus Generating.../Working.../Loading... or esc to cancel as running; a visible `>` prompt alone is not idle while that active indicator is present. Agy /usage Model Quota screens MUST attach screen_usage with visible model_quotas percent/status entries, so later switch-model, usage-probe, and model-catalog-export decisions can verify against observed state instead of blind PTY text.")
	         (capability slot-status-control
	           :rule "Driver status and named control actions are MissionD provider-box APIs, not semantic-terminal responsibilities. status observes the current slot without synthesizing an answer. input writes text to the composer with optional Enter but does not wait for a semantic final; pure-text-single-turn is the separate API that waits for durable JSONL final extraction. clear_screen, exit, explicit restart, and provider-specific settings controls such as Codex permissions mode switching, ClaudeCode permission mode switching, and Codex /fast service-tier enable/disable are idempotent interactive control flows verified from recognized post-action screen state. ClaudeCode permission switching MUST first observe screen_identity.permission_mode, compute the number of Shift+Tab steps in the taught cycle auto -> default -> accept_edits -> plan -> auto, send one Shift+Tab at a time, and verify the footer after every step; bypass_permissions is a spawn/restart policy via dangerously_skip_permissions and is not a hot Shift+Tab target. ClaudeCode logout is implemented only as a hidden taught internal flow and MUST NOT be exposed through normal provider-box slot routes/capabilities until a human explicitly enables it, because re-login is expensive and accidental logout is risky. If enabled manually, logout MUST type `/logout` and press Enter as separate observe-act-observe steps; success requires a recognized auth_missing or startup_config screen such as the first-run theme prompt, and ordinary composer idle is explicitly not success. Because Codex /fast is a toggle, provider-box MUST read the current screen_identity current_model first and only type /fast when the observed fast state differs from the requested state. Codex exit uses /exit typed separately from Enter and verifies the previous pid ended or a supervised restart occurred; restart requires confirm_destroy_context=true.")
	         (capability agy-clear-cleanup
	           :rule "Agy /clear cleanup MUST first observe that the clear slash command is selected or staged in the composer, then press Enter as separate observe-act-observe completion/execution actions. Cleanup may be verified by the home identity screen or by a clean agy:composer_idle screen only after that selected/pending /clear command was observed; raw composer idle without verified clear command staging is not cleanup success.")
	         (capability switch-model
           :rule "Driver declares switch behavior as launch_arg, interactive_ui, direct_slash_command, conversation_setting, or unsupported. launch_arg may require respawn; interactive_ui/direct_slash_command require a provider-specific driver; unsupported returns MODEL_SWITCH_UNSUPPORTED. Agy interactive_ui switching first observes the visible /model picker, derives the selected row and target row from the recognized model list, then sends the exact number of arrow-key steps with observe-act-observe verification before Enter; bounded scan is a fallback only when the target is not visible or the planned navigation fails verification. ClaudeCode direct_slash_command switching is taught for `/model claude-opus-4-8`, `/model claude-opus-4-6`, and `/model claude-sonnet-4-6`; the driver must type the command and press Enter as separate observe-act-observe steps, press Enter a second time only when the post-action observation still shows the exact staged slash command in the composer, verify screen_identity.current_model equals Opus 4.8, Opus 4.6, or Sonnet 4.6, and fail closed if the composer is not empty before typing. Every successful switch must be verified by durable metadata, recognized UI/footer, provider settings export, or the launch config that created a fresh session.")
         (capability usage-probe
           :rule "Driver may report exact remaining usage only from structured provider output, durable logs, a recognized usage meter, or a provider account surface. Agy usage-probe refresh is an interactive control flow owned by provider-box on a dedicated usage probe PTY: if a Model Quota screen is already open, press Esc to close it, type /usage, press Enter, then observe the refreshed screen_usage model_quotas. Codex usage-probe refresh is an interactive /status control flow on a dedicated Codex probe PTY; it types /status and Enter as separate observe-act-observe steps and returns only the current model/account `5h limit` and `Weekly limit` lines, ignoring secondary model sections such as GPT-5.3-Codex-Spark. These flows may lazily spawn or recover the dedicated probe CLI as part of the API work, but they must not use router/xjpcode text-only export slots. Hard auth/billing/quota screens are blocked diagnostics. If only vague text is available, remaining usage is unknown; MissionD must not invent counters.")
         (capability model-catalog
           :rule "Driver may export provider models only from stable provider catalog sources, recognized model picker entries, provider config, or documented account metadata. Agy catalog export uses the interactive /model picker as its discovery source: open /model, observe recognized entries/selected/current model, scroll until the visible catalog is exhausted, and write provider-model-catalog entries from those observed display names. Current Agy transcripts with model=nil are not a model catalog.")
         (capability pure-text-guard
          :rule "Driver declares whether no_tools/no_mcp/no_shell/no_file_access can be enforced by launch profile, tool policy, provider settings, or observation. Text-only prompt wrapping must be concise, carry correlation_id, and use ordinary no-tool/no-file/no-shell wording without XML wrappers or internal-rule phrasing so router users do not receive compliance narration. If the guard cannot be enforced, pure-text-single-turn fails closed instead of relying on prompt wording alone.")
         (capability timeout-cancel-retry
           :rule "Driver timeout_cancel_policy is historical state, not a single screen-frame classification. If a prompt turn remains active past running_timeout_secs, or screen hash/output progress stays unchanged past no_progress_grace_secs while active indicators remain, the driver records PROVIDER_TURN_STALLED, sends the configured human-like cancel key such as escape, observes until cancel_grace_secs, and verifies a ready/interrupted surface before retrying. For Agy, a verified cancel surface is Interrupted · What should Antigravity CLI do instead? plus ready footer such as ? for shortcuts and the current model. If active indicators remain after max_cancel_attempts, fail with PROVIDER_TURN_TIMEOUT_CANCEL_FAILED and do not release a semantic answer from PTY text."))
      :forbidden-headless-provider-cli [claude-print claude-p claude-prompt codex-exec codex-output-last-message agy-print agy-p agy-prompt gemini-prompt-stream-json stdin-closed-provider-subprocess]
      :mode-contracts
        ((mode semantic-authoring
           :uses [codex-intent-author codex-key-judgment-author codex-plan-author]
           :rule "Jarvis intent.lisp, interaction-key-judgment, and plan.lisp authoring uses dedicated read-only provider PTY lanes, waits for the durable provider final that matches the turn correlation id, validates the artifact body, and fails closed with JARVIS_INTENT_AUTHOR_FAILED, JARVIS_KEY_JUDGMENT_AUTHOR_FAILED, or JARVIS_PLAN_AUTHOR_FAILED instead of falling back to deterministic Rust.")
         (mode grounded-direct-answer
           :uses [codex-review-worker agy-research claude-code-default router-explicit]
           :rule "Direct answers after confirmed plan.lisp use the box as a no-mutation interaction lane. It writes missiond.interaction-result-artifact.v1 before terminal final and never calls xjpcode text-only or local code search as an invisible fallback.")
         (mode runner-one-shot
           :uses [claude-code-default codex-review-worker agy-research]
           :rule "Runner-style one-shot calls are compatibility adapters over the box. They may request isolated context and no_tools, but they still interact through a PTY and durable logs rather than `claude --print`, `codex exec`, or stdin-closed subprocesses.")
         (mode vision
           :uses [codex-review-worker]
           :rule "Vision requests must either attach images through a provider-supported interactive UI flow and read the durable final, or fail with PROVIDER_INTERACTIVE_ATTACHMENT_UNSUPPORTED. `codex exec --json -i` remains a legacy migration target, not the final architecture.")
         (mode external-worker-turn
           :uses [claude-code-default codex-review-worker agy-research gemini-router]
           :rule "External apps may request a worker with provider/model/profile/cwd/sandbox/output_contract, but they receive only a box lease and result artifact. The box validates capability scope, picks or spawns the slot, submits exactly one normalized turn or creates an async job, and returns durable final/artifact refs; external callers cannot create arbitrary provider subprocesses or parse PTY output.")
         (mode model-switch
           :uses [slot-lease provider-driver workstation-pool]
           :rule "Model switch is a control turn, not a prompt. The box evaluates provider driver capability and either applies a verified interactive setting, respawns a launch_arg-only session under the same lease, or returns MODEL_SWITCH_UNSUPPORTED/MODEL_SWITCH_UNVERIFIED. Slot state and router routeability update only after verification.")
         (mode usage-probe
           :uses [provider-driver pty-recognition durable-provider-log account-status-observation]
           :rule "Usage detection returns provider-usage-snapshot. Exact remaining quota requires a structured or recognized source. Current generic PTY recognition can classify auth/billing/quota blockage but cannot report remaining counters for every CLI; unknown remains a valid snapshot status.")
         (mode model-catalog-export
           :uses [provider-driver provider-router-exporter router-policy]
           :rule "Model export writes provider-model-catalog first, then provider-router-export only for entries with stable provider_model_id, switch/test support, usage status, and routeability policy. Agy models are not exported from transcripts; Agy export comes from interactive /model picker observations plus /usage status and emits xjp-router routes for provider kind MissionDAgy, never CcSource. The generated route provider_model_id is the MissionD provider-box base URL exposed by the self-built proxy deployment program, not a direct Mac mini/Tailscale address.")
         (mode pure-text-single-turn
           :uses [provider-driver pure-text-guard output-contract]
          :rule "Pure text output is a box mode, not a headless CLI shortcut. It uses interactive PTY input or an explicitly inventoried provider-box headless compatibility path, no_tools/no_mcp/no_shell/no_file_access guards, a text/plain output contract, durable final extraction, and violation detection for tool calls, file access, shell, hidden subagent, or multi-step provider workflow. The driver must inject or sidecar a correlation marker visible to its durable transcript source. The prompt injected into the CLI uses short ordinary wording for direct text answers under no-tool/no-file/no-shell constraints, but prompt wording is not the safety boundary; the durable transcript guard is. Router/provider routes are fixed-model slot leases: the box verifies the slot's current model before prompt submission, fails closed on mismatch, and relies on a separate maintenance/control action to switch or clear slots. Violations fail the turn instead of being cleaned up as text."))
      :current-implementation-gaps
        ["mission_compute_slot(create) can spawn dynamic slots with spawn-time model/profile parameters, but it is lifecycle-oriented create/list/extend/terminate and does not yet expose provider-interaction-box worker-turn semantics or durable final return."
         "Model parameters are mostly launch-time flags today; Agy and ClaudeCode now have provider-specific switch-model drivers, while other providers without taught controls must still return MODEL_SWITCH_UNSUPPORTED."
         "PTY recognition can classify auth/billing/usage-limit blockage, but reliable remaining-usage counters require provider-specific usage-probe drivers and must otherwise return unknown."
         "Agy JSONL ingestion still normalizes transcript messages with model=nil and usage=nil; provider-box pure-text turns therefore verify the requested model from PTY screen_identity and extract semantic finals by correlation_id from transcript_full.jsonl/history.jsonl instead of trusting transcript model metadata."
        "Agy Opus/Claude router lanes should be exported as pinned fixed-model replica slots, for example slot-agy-claude-opus-46-thinking-a and slot-agy-claude-opus-46-thinking-b. Hot-path text-only completions lease only slots already verified on the requested model; the only in-request model switch allowed without explicit allow_model_switch is cold-start bootstrap for a hidden replica that was absent/exited/error/reclaimed before the request. Other model switching and /clear are maintenance/control flows outside the customer request path."
         "Non-Jarvis xjpcode/MissionD text-only provider lanes remain legacy migration adapters; the target is pure-text-single-turn through the box with enforcement and durable final correlation."]
      :legacy-headless-migration-targets
        ((target missiond-runner-claude-print
           :current-files ["crates/missiond-runner/src/runner.rs" "crates/missiond-runner/src/lib.rs" "crates/missiond-runner/Cargo.toml"]
           :current-patterns ["claude --print --output-format stream-json" "\"--print\""]
           :replacement "runner adapter calls provider-interaction-box mode=runner-one-shot and receives missiond.provider-interaction-turn.v1")
         (target jarvis-author-text-provider-env
           :current-files ["crates/missiond-core/src/ws/server.rs"]
           :current-patterns ["jarvis_author_text_provider" "MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_PROVIDER" "semantic-authoring" "pure-text-single-turn"]
           :replacement "Jarvis authoring provider selection may be env-configured only inside provider-interaction-box semantic-authoring/pure-text-single-turn lanes; callers must not bypass the box or read PTY screen text as semantic output")
         ...
         (target jarvis-grounded-direct-answer-text-only
           ...
           :replacement "direct answer calls provider-interaction-box mode=grounded-direct-answer; xjpcode may expose an HTTP facade but cannot operate provider CLIs itself")
         (target codex-vision-exec
           :current-files ["crates/missiond-daemon/src/workers/codex/vision_worker.rs" "crates/missiond-daemon/src/llm/codex_cli.rs"]
           :current-patterns ["codex exec --json -i" "codex exec --json --ephemeral --skip-git-repo-check"]
           :replacement "vision worker calls provider-interaction-box mode=vision or returns typed unsupported diagnostic")
         (target gemini-router-subprocess
           :current-files ["crates/missiond-daemon/src/llm/gemini_cli.rs"]
           :current-patterns ["gemini -o stream-json -p" "cmd.args([\"-p\", prompt, \"-m\", model, \"-o\", \"stream-json\", \"--yolo\"])" "stdin(std::process::Stdio::null())"]
           :replacement "GeminiClient requires GeminiPtyTransport/provider-interaction-box for provider CLI access; router API calls stay separate because they are not provider CLI automation")
         (target search-center-missiond-text-only
           :current-files [".missiond/v3/shards/universe/service-runtime.lisp"]
           :current-patterns ["missiond-xjpcode-text-only" "SEARCH_CENTER_MISSIOND_TEXT_ONLY_URL"]
           :replacement "Search Center calls a MissionD interaction-box HTTP adapter that is backed by PTY + durable log final extraction"))
      :invariants
        ["No new MissionD-owned provider CLI path may start from `claude --print`, `claude -p`, `codex exec`, `agy --print`, Gemini `-p -o stream-json`, or stdin-closed provider subprocesses, except the explicitly guarded provider-box codex_exec_text lane defined in this shard."
         "The box owns provider-specific prompt submission, idle/running recognition, durable final correlation, timeout/cancel, and result artifact creation; callers must not parse provider PTY screens or provider-specific JSONL directly unless they are the box implementation."
         "The box and every provider driver MUST remember step-by-step PTY operation records. Each action must be preceded and followed by an observation, and the driver must verify the expected screen/state change before continuing."
         "Provider prompt authorization is allowlist-gated: ClaudeCode MCP allow prompts may be auto-confirmed only for recorded targets such as mcp:missiond and mcp:supabase, after selected option verification; all unknown targets fail closed."
         "AGY provider-box bypass and model pinning are launch-time controls where possible: dangerously_bypass_approvals_and_sandbox maps to agy --dangerously-skip-permissions for provider-box AGY slots, and request.model maps to agy --model <display name> on spawn/restart. Provider-box MUST still verify screen_identity.current_model after startup; reused slots or launch mismatch may use /model picker observe-act-observe bootstrap, and unknown targets fail closed."
         "Jarvis semantic author lanes must validate provider-slot compatibility after provider normalization: Codex authors may use only slot-codex-* slots, AGY communicators may use only slot-agy-* slots, ClaudeCode may use only slot-claude-* slots, and Gemini may use only slot-gemini-* slots. A stale MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_SLOT_ID from a previous AGY/Gemini migration must be ignored rather than binding codex_cli authoring to the wrong provider slot."
         "Every box turn carries correlation_id/task_id/interaction_id in the submitted prompt or sidecar so reused sessions can ignore stale durable finals."
         "A provider final without a matching durable log/artifact is an observation diagnostic, not a semantic answer."
         "semantic-terminal is the parser/recognizer boundary only; it must not own MissionD provider-box APIs, queueing, driver retries, model switch semantics, usage refresh semantics, durable final extraction, or router/xjpcode provider export."
         "Usage and model catalog surfaces are observational contracts with confidence; unsupported and unknown are correct results, while guessed remaining quota or inferred model catalogs are forbidden."
         "Router export consumes provider-model-catalog/provider-router-export artifacts from the box and never scrapes provider CLIs independently. For Agy, router export targets xjp-router ProviderKind=MissionDAgy, capabilities text=true/tools=false/vision=false, and a managed-proxy MissionD provider-box URL."
         "Pure-text compatibility is implemented as pure-text-single-turn over the box; any hidden tool, shell, file, MCP, subagent, or multi-step provider workflow is a contract violation."
         "Legacy headless implementations remain only as explicitly inventoried migration targets; adding another occurrence outside the inventory is a checker failure."]
      :checker "node scripts/check-v3-interactive-provider-box.mjs --json")
    (chat-completions-policy jarvis-api
      ...
      :provider_interaction_box provider-interaction-box
      :headless_legacy_status migration-only
      :execution_modes [grounded_direct_answer work_order investigation_only]
      :direct_answer_provider provider-interaction-box
      :direct_answer_default_text_provider codex_cli
      :direct_answer_provider_env MISSIOND_JARVIS_DIRECT_ANSWER_PROVIDER
      :direct_answer_endpoint_env []
      :direct_answer_provider_slot_env MISSIOND_JARVIS_DIRECT_ANSWER_SLOT_ID
      :direct_answer_timeout_env MISSIOND_JARVIS_DIRECT_ANSWER_TIMEOUT_SECS
      :grounding_worker_provider claude-code
      :grounding_worker_slot_env MISSIOND_JARVIS_GROUNDING_WORKER_SLOT_ID
      :grounding_worker_default_slot "slot-jarvis-grounding-claude"
      :grounding_worker_model_env MISSIOND_JARVIS_GROUNDING_WORKER_MODEL
      :grounding_worker_timeout_env MISSIOND_JARVIS_GROUNDING_WORKER_TIMEOUT_SECS
      :grounding_worker_report_dir "MISSIOND_RUNTIME_DIR/context-gather-worker"
      :grounding_worker_report_kind interaction-grounding-report
      :key_judgment_author_provider codex-cli
      :key_judgment_author_slot_env MISSIOND_JARVIS_KEY_JUDGMENT_AUTHOR_SLOT_ID
      :key_judgment_author_default_slot "slot-codex-key-judgment-author"
      :key_judgment_author_model_env MISSIOND_JARVIS_KEY_JUDGMENT_AUTHOR_MODEL
      :key_judgment_author_model "gpt-5.5"
      :key_judgment_author_reasoning "xhigh"
      :key_judgment_artifact_kind interaction-key-judgment
      :intent_plan_author_slot_env [MISSIOND_JARVIS_INTENT_AUTHOR_SLOT_ID MISSIOND_JARVIS_PLAN_AUTHOR_SLOT_ID]
      :intent_plan_author_provider_env MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_PROVIDER
      :intent_plan_author_break_glass_env MISSIOND_JARVIS_ALLOW_NON_CODEX_AUTHORS
      :intent_plan_author_provider_slot_env MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_SLOT_ID
      :direct_answer_rule "Intent/grounding/key-judgment/plan remains mandatory for every Jarvis user turn during this validation phase, including simple chat. Current grounded_direct_answer implementation first uses the Codex plan author's required direct_answer_draft as the fast terminal answer, emits answer_delta/result_artifact/final, writes an interaction-direct-answer artifact with schema missiond.interaction-result-artifact.v1 and answer_source=plan_direct_answer_draft, and returns terminal final without creating a BoardTask. If a legacy/confirmed plan lacks direct_answer_draft, it may call provider-interaction-box mode=grounded-direct-answer within MISSIOND_JARVIS_DIRECT_ANSWER_STREAM_BUDGET_SECS; budget expiry emits typed diagnostic + result_pending instead of hanging the public SSE stream. The provider selector is MISSIOND_JARVIS_DIRECT_ANSWER_PROVIDER; selected providers must be registered provider-box drivers. Missing provider-box adapter/auth, unavailable provider CLI, absent grounding report, or absent matched durable final is a typed fail-fast diagnostic, not a fallback to BoardTask, xjpcode text-only, local code search, or fabricated answers. work_order and investigation_only keep the existing BoardTask/task-result-artifact path."
      :rule "OpenAI-compatible /v1/chat/completions routes to the explicit X-Slot-Id header when present; otherwise it uses this V3-projected default slot. Rust must not hardcode slot-jarvis. Jarvis broad requests MUST pass the strict intent/grounding/key-judgment/plan gate: after identity and permission resolution, Codex CLI GPT-5.5 xhigh worker slot-codex-intent-author authors intent.lisp through provider-interaction-box mode=semantic-authoring over a dedicated interactive PTY lane, and the authored artifact is read from correlated durable provider logs, not PTY screen text. MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_PROVIDER is a migration knob and MUST resolve to codex_cli unless explicit break-glass MISSIOND_JARVIS_ALLOW_NON_CODEX_AUTHORS=1 is set; stale AGY/Gemini author env on mac mini MUST NOT silently replace Codex authoring. Default Jarvis/iOS/OpenAI flow archives intent.lisp and plan.lisp without user-blocking confirmation cards; explicit MISSIOND_JARVIS_REQUIRE_CONFIRMATION/manual-review mode still emits confirm_required and reuses persisted payloads. After intent.lisp is archived or confirmed, MissionD MUST call mission_context_gather(persist=true) for a seed context pack and then submit a ClaudeCode provider-box worker-turn on MISSIOND_JARVIS_GROUNDING_WORKER_SLOT_ID (default slot-jarvis-grounding-claude) with MissionD/XJP MCP enabled, launch/restart bypass permissions enabled, no shell, scoped file write to target grounding_report_file, and prompt contract: \"根据用户输入的内容，在 MissionD 中搜集相关上下游全链的信息，存入文件 XXX.md，把内容供给下一步分析\". The grounding worker MUST write an interaction-grounding-report Markdown artifact/file containing upstream/downstream evidence, permissions, related Board/session/artifact facts, unknowns, and plan-author recommendations; it MUST NOT create BoardTask, implement changes, deploy, or write other files. After that report exists and before plan.lisp authoring, Codex CLI GPT-5.5 xhigh MUST author an interaction-key-judgment artifact from the user input plus grounding report, identifying the decisive framing such as \"不是算力差异，是用量差异\", rejected hypotheses, evidence refs, and planning implications. Only after key judgment exists may slot-codex-plan-author author plan.lisp from confirmed/archived intent plus grounding_report_preview/context_pack_file plus key_judgment_preview. plan.lisp MUST decide execution_mode and requires_board_task: if the grounding report and key judgment are sufficient to answer, it MUST choose grounded_direct_answer with requires_board_task=false and include direct_answer_draft; otherwise work_order or investigation_only requires BoardTask. For work_order/investigation_only, plan.lisp MUST decompose into up to ten workstreams and up to ten atom tasks per workstream, marking serial/parallel dependencies and fixed assignment: query/code_change/deploy_ops atoms to ClaudeCode, judgment/acceptance atoms to Codex CLI GPT-5.5 xhigh. No Jarvis authoring path may route through xjpcode text-only paid CLI channels or any deterministic Rust fallback. Authoring MUST still return the requested artifact only. Confirmation turns MUST reuse the previously returned missiond_intent_artifact_id / missiond_key_judgment_artifact_id / missiond_plan_artifact_id, missiond_direct_answer_draft, and grounding fields (grounding_context_id, context_pack_file, grounding_report_file, grounding_report_hash, grounding_worker_slot_id, context_sufficiency, sources_used) and MUST NOT re-author intent.lisp, re-run grounding, re-author key judgment, or re-author plan.lisp during confirmation. Rust may schema-normalize the authored intent/plan fields but MUST NOT silently fall back to deterministic Rust intent understanding or deterministic Rust plan generation; if intent author execution is unavailable or returns an invalid contract, the gate emits JARVIS_INTENT_AUTHOR_FAILED and stops; if grounding worker execution is unavailable or fails to write a report, the gate emits a typed grounding diagnostic and stops before key judgment/plan authoring; if key judgment author execution is unavailable or returns an invalid contract, the gate emits JARVIS_KEY_JUDGMENT_AUTHOR_FAILED and stops before plan authoring; if plan-author execution is unavailable or returns an invalid contract, the gate emits JARVIS_PLAN_AUTHOR_FAILED and stops before BoardTask creation. The OpenAI-compatible adapter MUST persist intent_archived, key_judgment_draft, and plan_archived for replay; legacy/manual-review mode may additionally mirror intent_draft/key_judgment_draft/plan_draft as missiond.openai-artifact-projection.v1 delta chunks. The gate MUST emit missiond.jarvis-progress.v1 through MissionD EventBus first as SystemEvent::ExternalServiceEvent service_id=missiond-jarvis event_kind=jarvis_progress and then read the matching event_id from the frontend EventBus bridge before projecting mobile/SSE status and OpenAI-compatible delta chunks for permission, grounding, key_judgment_authoring, plan_authoring, artifact write, and BoardTask creation; long authoring/grounding MUST heartbeat while running, and failures MUST visibly name the failed phase rather than leaving iOS on a generic spinner or HTTP 504. Jarvis plan-confirmed dispatch classifies the user objective verb into review/design/implementation/verification via classify_jarvis_dispatch_verb: implementation verbs (implement, fix, create, build, add, refactor, migrate, 补齐, 接入, 实现, 修复, 重构, 提交, 推送) produce task_class=code with non-empty write_scope; when the objective explicitly asks for Codex, dispatch MUST use engine_hint=codex and pool_hint=codex-code-worker, otherwise scoped code work may use engine_hint=claude_code and pool_hint=claude-code-default. review/smoke/read-only verbs remain task_class=review, write_policy=read-only with empty write_scope. /jarvis/api/monitor/jarvis is read-only and may report auto_heal policy; /internal/jarvis/slot/ensure is a localhost-only deploy smoke control surface for restoring the default Exited/Error slot after blue-green restart. Chat requests, local ensure, and deploy-daemon post-switch smoke may attempt at most one restart of an Exited/Error default slot when MISSIOND_JARVIS_SLOT_AUTO_HEAL=1; failure returns JARVIS_SLOT_UNAVAILABLE or JARVIS_SLOT_AUTO_HEAL_FAILED typed diagnostic and never falls back to direct local code search or fake output. MissionD deploys MUST run the local ensure endpoint in auto mode when launchd/current env enables Jarvis auto-heal, so Mac mini Jarvis readiness is restored before public monitor smoke is treated as healthy.")
    ...
    (portable-worker-runtime-registry
      ...
      (worker :id xjpcode-readonly-worker
        ...
        :dispatch-surface "mission_task_delegate detects engine_hint=xjpcode/xjpcode-readonly-worker, requires MISSIOND_XJPCODE_WORKER_URL or a loopback-only xjpcode_worker_url smoke override, POSTs the scoped WorkOrder, reads SSE only until the worker final frame instead of waiting for connection teardown, persists every frame to shared_events stream_id=worker-runtime:xjpcode, writes xjpcode_worker.telemetry, writes task_result_put_typed, then closes through worker_settle."
    ...
    (text-only-paid-cli-provider-registry
      ...
      :missiond-default-provider codex_cli
      :forbidden [mcp tool_use tool_call function_call slash_command file_read file_write apply_patch shell_exec approval_prompt]
      :rule "xjpcode owns the legacy text-only provider implementation during migration. It currently runs paid CLI channels in an isolated temporary cwd with stdin closed, no MCP config, no tool schema, no work-order write authority, and a bounded timeout. ClaudeCode uses print/text mode with slash commands and MCP disabled; legacy Codex uses exec in read-only sandbox; Agy uses its print/sandbox mode. This registry is migration-only under interactive-provider-box: new MissionD callers MUST NOT use these provider CLI modes directly, and xjpcode may keep an HTTP facade only if its provider CLI backend delegates to MissionD provider-interaction-box. MissionD provider-box owns the new codex_exec_text guarded lane, where Codex exec is allowed only with --json, output-last-message final extraction, isolated runtime cwd, --ignore-user-config, --ignore-rules, read-only sandbox, approval_policy=\"never\" config, explicit tool-disabling config, and JSONL tool-event violation detection; the lane is exported routeable=false until live smoke validates no hidden tool activity. Router model calls may share the same response contract only when tool schemas are empty, but MissionD grounded_direct_answer defaults to codex_cli only while the legacy adapter remains enabled. MissionD pins this registry so future xjpcode provider expansions cannot silently reintroduce CLI tool execution through the text-only path.")
  ...
  (workstation-pool
    ...
    (worker agy-research
      ...
      :accepts-boardtask true
      :write-allowed false)
    (worker agy-gemini-communicator
      :engine agy
      :role communicator
      :slot-id "slot-agy-gemini-31-pro-high-jarvis-communicator-a"
      :task-type jarvis_communicator
      :model-profile nil
      :model "Gemini 3.1 Pro (High)"
      :task-classes [user-communication result-summary plan-status]
      :capabilities [read-only user-facing-summary evidence-bound-summary communication-preference-read]
      :max-concurrency 1
      :timeout-secs 120
      :default-use jarvis-communication-officer
    ...
    (worker codex-intent-author
      ...
      :accepts-boardtask false
      :write-allowed false)
    (worker codex-key-judgment-author
      :engine codex
      :role key-judgment-author
      :slot-id "slot-codex-key-judgment-author"
      :task-type codex_key_judgment_author
      :model-profile codex-master-gpt-5-5-xhigh
      :model nil
      :reasoning-effort xhigh
      :search true
      :sandbox read-only
      :approval-policy never
      :task-classes [key-judgment-authoring planning-premise]
      :capabilities [judgment evidence-review code-read search]
      :max-concurrency 1
      :timeout-secs 180
      :default-use jarvis-key-judgment-authoring
      :accepts-boardtask false
      :write-allowed false)
    ...
    :invariants
      ["Claude coding workers use coding-default-opus-4-7, which means no Claude Code --model override; Sonnet cannot be the coding default."
       ...
       "Deployment/Ops work MUST route to the explicit claude-code-deploy-ops lane when context_intent=deploy-ops or task_class=deploy-ops is present; generic claude-code-default is not the default deployment lane. Deploy-ops is observation/planning first: it may query deploy-center, provenance, skills, and logs, but production mutation requires deploy-center policy or explicit Board approval. Deploy-ops output contracts are fixed to one of preflight-report, release-evidence-review, closure-verdict-review, rollback-plan, or postmortem. CI/build waiting uses deploy-center events/provenance or bounded XJP MCP wait/watch tools; raw gh api polling loops are forbidden."
       ...
       "Jarvis communication officer uses the AGY lane pinned to the AGY visible model name Gemini 3.1 Pro (High) as a read-only user-facing summarizer. MissionD launches the private communicator slot with --model 'Gemini 3.1 Pro (High)' plus --dangerously-skip-permissions and --sandbox when allowed, then provider-box verifies screen_identity.current_model or selects the model through the AGY /model control surface if a reused slot drifted. It consumes archived intent/key-judgment/plan artifacts plus BoardTask/task-result evidence and the MissionD-maintained communication preference Lisp at MISSIOND_JARVIS_COMMUNICATION_PREFERENCES_FILE or MISSIOND_RUNTIME_DIR/jarvis/communication-preferences.lisp. Communication preferences are candidate preference-observation forms appended by MissionD after redaction when the user explicitly asks for a communication style change; AGY may read them as style context but cannot write that file, alter plan, create BoardTasks, mark work complete, or become a worker execution authority."
       ...
       "Codex code/review worker lanes are ordinary BoardTask candidates and are separate from codex-master-control; codex-code-worker uses workspace-write rather than danger-full-access, codex-review-worker uses read-only, codex-intent-author, codex-key-judgment-author, codex-plan-author, and agy-gemini-communicator are non-BoardTask semantic/user-communication lanes for Jarvis artifacts, and the resident master cannot consume ordinary shard work."
       "Read-only Gemini pool workers MUST project to Gemini CLI `--approval-mode plan --policy .missiond/v3/policies/gemini-readonly-policy.toml`; workstation-pool registration MUST NOT set Gemini/Codex/Agy YOLO-style broad bypass for ordinary BoardTask workers, while MissionD-owned ClaudeCode worker/workstation slots intentionally default to launch/restart bypass through the internal capability override. The Gemini read-only policy MUST deny subagent delegation and write/shell tools."
