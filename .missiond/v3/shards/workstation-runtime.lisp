    ...
    (interactive-provider-box
      ...
      :mode-contracts
        ((mode semantic-authoring
           :uses [codex-intent-author codex-plan-author]
           :rule "Jarvis intent.lisp and plan.lisp authoring uses dedicated read-only provider PTY lanes, waits for the durable provider final that matches the turn correlation id, validates the Lisp artifact body, and fails closed with JARVIS_INTENT_AUTHOR_FAILED or JARVIS_PLAN_AUTHOR_FAILED instead of falling back to deterministic Rust.")
  ...
  (workstation-pool
    ...
    :invariants
      ["Claude coding workers use coding-default-opus-4-7, which means no Claude Code --model override; Sonnet cannot be the coding default."
       ...
       "Codex code/review worker lanes are ordinary BoardTask candidates and are separate from codex-master-control; codex-code-worker uses workspace-write rather than danger-full-access, codex-review-worker uses read-only, codex-intent-author and codex-plan-author are non-BoardTask semantic authoring lanes for Jarvis review artifacts, and the resident master cannot consume ordinary shard work."
