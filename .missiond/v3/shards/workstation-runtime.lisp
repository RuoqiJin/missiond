    ...
    (interactive-provider-box
      ...
      :invariants
        ["No new MissionD-owned provider CLI path may start from `claude --print`, `claude -p`, `codex exec`, `agy --print`, Gemini `-p -o stream-json`, or stdin-closed provider subprocesses, except the explicitly guarded provider-box codex_exec_text lane defined in this shard."
         ...
         "AGY provider-box bypass is launch-only: dangerously_bypass_approvals_and_sandbox maps to agy --dangerously-skip-permissions for provider-box AGY slots; AGY 1.0.4 has no stable launch-time model selector, so model pinning uses /model picker observe-act-observe after spawn."
  ...
  (workstation-pool
    ...
    :invariants
      ["Claude coding workers use coding-default-opus-4-7, which means no Claude Code --model override; Sonnet cannot be the coding default."
       ...
       "Jarvis communication officer uses the AGY lane pinned to the AGY 1.0.x visible model name Gemini 3.1 Pro (High) as a read-only user-facing summarizer. AGY does not expose a stable public --model launch flag; MissionD launches the private communicator slot with --dangerously-skip-permissions and --sandbox when allowed, then provider-box verifies or selects the model through the AGY /model control surface. It consumes archived intent/key-judgment/plan artifacts plus BoardTask/task-result evidence, but it cannot alter plan, create BoardTasks, mark work complete, or become a worker execution authority."
