  (workstation-config
    ...
    (text-only-paid-cli-provider-registry
      ...
      :rule "xjpcode owns the text-only provider implementation. It must run paid CLI channels in an isolated temporary cwd with stdin closed, no MCP config, no tool schema, no work-order write authority, and a bounded timeout. ClaudeCode uses print/text mode with slash commands and MCP disabled; Codex uses exec in read-only sandbox; Agy uses its print/sandbox mode. Router model calls may share the same response contract only when tool schemas are empty, but MissionD grounded_direct_answer defaults to codex_cli to prefer existing paid CLI lanes and avoid router spend unless the operator explicitly selects router. MissionD pins this registry so future xjpcode provider expansions cannot silently reintroduce CLI tool execution through the text-only path.")
