  (workstation-config
    ...
    (interactive-provider-box
      ...
      :outputs [turn_id lease_id slot_id provider_conversation_id durable_source final_text artifact_hash diagnostics transcript_refs step_records slot_status screen_identity screen_usage usage_snapshot_ref model_catalog_ref router_export_ref model_switch_result]
      :http-adapter
      ...
      :current-implementation-gaps
        ["mission_compute_slot(create) can spawn dynamic slots with spawn-time model/profile parameters, but it is lifecycle-oriented create/list/extend/terminate and does not yet expose provider-interaction-box worker-turn semantics or durable final return."
         "Model parameters are mostly launch-time flags today; Agy explicitly ignores model override because no stable flag is known, and there is no unified model-switch command."
