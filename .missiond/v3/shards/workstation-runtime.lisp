      ...
      :mode-contracts
        ((mode semantic-authoring
         ...
         (mode pure-text-single-turn
           ...
           :rule "Pure text output is a box mode, not a headless CLI shortcut. It uses interactive PTY input, no_tools/no_mcp/no_shell/no_file_access guards, a text/plain output contract, durable final extraction, and violation detection for tool calls, file access, shell, hidden subagent, or multi-step provider workflow. Router/provider routes are fixed-model slot leases: the box verifies the slot's current model before prompt submission, fails closed on mismatch, and relies on a separate maintenance/control action to switch or clear slots. Violations fail the turn instead of being cleaned up as text."))
