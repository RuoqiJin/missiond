    ...
    (interactive-provider-box
      ...
      :http-adapter
        (:base "/provider-box/v1"
         ...
         :rule "The HTTP adapter is a thin MissionD-owned facade over provider-interaction-box. It validates text-only guards and auth, then calls the box; it does not operate provider CLIs itself. GET /models returns provider-model-catalog, provider-router-export, OpenAI-style data[], and provider_text_only_sources[] for each observed AGY model, including logical slot_pool_id, provider-box-managed replica policy, and text-only completion request templates, but never public replica slot ids. xjp-router MissionDAgy calls /provider-box/v1/text-only/completions through the managed proxy URL; slot-scoped /provider-box/v1/slots/<slot_id>/completions is internal maintenance/debug surface and is not exported as a router source. Router-facing text-only requests use fixed-model hidden slots by default: the box verifies the visible current model on the internally selected slot and MUST NOT run interactive /model switching in the request hot path unless allow_model_switch=true is explicitly set. External apps may use slot-scoped spawn/status/input/switch-model/usage/exit/clear APIs only as internal operator controls; those APIs still dispatch to provider drivers and return step_records plus slot_status rather than exposing raw PTY scripting.")
      :public-commands
        ((command worker-turn
      ...
      :driver-capabilities
        ((capability submit-turn
         ...
         (capability pure-text-guard
           :rule "Driver declares whether no_tools/no_mcp/no_shell/no_file_access can be enforced by launch profile, tool policy, provider settings, or observation. If the guard cannot be enforced, pure-text-single-turn fails closed instead of relying on prompt wording alone.")
      ...
      :current-implementation-gaps
        ["mission_compute_slot(create) can spawn dynamic slots with spawn-time model/profile parameters, but it is lifecycle-oriented create/list/extend/terminate and does not yet expose provider-interaction-box worker-turn semantics or durable final return."
         ...
         "Agy Opus/Claude router lanes should be exported as pinned fixed-model replica slots, for example slot-agy-claude-opus-46-thinking-a and slot-agy-claude-opus-46-thinking-b. Hot-path text-only completions lease only slots already verified on the requested model; model switching and /clear are maintenance/control flows outside the customer request path."
