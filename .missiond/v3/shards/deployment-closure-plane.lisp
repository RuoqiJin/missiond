(deployment-closure-plane
    ...
    :records
      ((record DeploymentIntent
       ...
       (record DbMigrationAuthority
         :fields [service migration_mode source_migrations_root source_latest_version source_checksums live_ledger_table live_latest_version live_checksums reconciliation_status evidence_ref]
         :modes [auto strict skip external]
         :invariant "MIGRATION_MODE=skip means the running binary will not mutate schema at startup; it never means schema drift may be ignored. Closure for schema-owning services MUST compare the image/repo migration source against the live _sqlx_migrations ledger and emit typed diagnostics before declaring success.")
       ...
       (record ReleaseEvidence
         :fields [deployment_intent release_candidate release_lease runtime_observation smoke migration_authority migration_adoption secret_availability rollback_artifact_refs])
    ...
    :compiled-policy
      (:fields [manifest_required immutable_image_required runtime_digest_required smoke_required db_adoption_required migration_ledger_required release_lease_required artifact_lane target_side_build_allowed approval_policy diagnostic_profiles]
    ...
    :invariants
      ["MissionD MUST NOT infer production closure from GitHub success, curl probes, local git state, or PTY logs when Deploy Center closure evidence exists."
       ...
       "Deploy Center MUST expose closure evidence through stable discoverable ABI paths: GET /api/deploy/closure/:project, GET /api/deploy/evidence/:release_id, POST /api/deploy/releases/:release_id/evidence, and POST /api/deploy/releases/:release_id/close. These paths are the operator/API surface for ReleaseEvidence and ClosureVerdict review."
       ...
       "Codebase Service owns canonical Git hosting and /api/codebase source-authority APIs. Deploy Center /api/deploy/codebase/* is legacy-read-through/native-workflow compatibility only, MUST publish compatibility metadata with migration_expiry, and MUST NOT become the canonical Git remote/source authority."
       ...
       "Deploy-chain self-update preflight MUST expose credential presence and Secret Store refs only; read-token refs may default to secret-store://missiond/production/MISSIOND_DEPLOY_CENTER_READ_TOKEN, write-token refs may default to deploy/ci_deploy_api_key, and write-token values must never be emitted. Secret Store ref resolution requires Secret Store-owned env such as SECRET_STORE_API_KEY; Auth/XJP tokens such as XJP_API_KEY MUST NOT be treated as Secret Store access keys."
       "Deploy-chain self-update source freshness MUST be path-aware: Deploy Center direct paths are blocking, but workspace root files such as Cargo.toml/Cargo.lock block only when their diff mentions Deploy Center identity or services/deploy-center; unrelated workspace changes are reported as ignoredDirtyLines evidence, not source blockers."
       ...
       "Schema-owning services MUST publish a DbMigrationAuthority contract in manifest/deploy config: migration_mode env, image or repo migrations root, live ledger table, skip-mode semantics, and whether closure requires migration ledger reconciliation. Deploy Center self-update MUST treat MIGRATION_MODE=skip plus missing reconciliation as migration_mode_skip_without_reconciliation, not as success."
       "Deploy Center closure MUST record migration_authority evidence before final ClosureVerdict for schema-owning services. The evidence compares source migration versions/checksums from the deployed artifact with live _sqlx_migrations versions/checksums; missing ledger, lower live version, checksum mismatch, or unknown migration mode are typed blockers unless an explicit external migration authority evidence_ref is present."
    ...
    :diagnostics [reported_digest_missing runtime_digest_mismatch provenance_partial db_adoption_required migration_ledger_required migration_mode_skip_without_reconciliation migration_ledger_unavailable migration_source_unavailable migration_latest_version_behind migration_checksum_mismatch abi_freshness_mismatch release_lease_conflict deployment_lane_mismatch release_owner_executor_project_mismatch release_scoped_evidence_missing runtime_preflight_evidence_missing deploy_blocked_by_secret_store target_digest_missing final_runtime_digest_missing final_source_commit_missing source_commit_mismatch release_plan_missing release_plan_blocked build_runner_unavailable gcp_build_forbidden target_side_build_forbidden macmini_lane_forbidden runner_required_env_missing secret_availability_missing compiled_project_universe_unavailable service_runtime_config_missing deploy_center_slug_mismatch build_runner_role_missing runtime_runner_role_missing target_side_build_not_prohibited caddy_proxy_intent_missing domain_management_binding_missing dns_record_missing deploy_center_source_relevance_ignored generic_linux_build_pinned_to_single_runner codebase_source_authority_drift compatibility_fallback_without_expiry artifact_builder_identity_mismatch]
    :surfaces [deployment-event-ingest m6-deployment-confirmation deployment-evidence-preflight deploy-chain-closure-audit project-universe-domain-route-config deploy-agent-self-update-governance migration-authority-reconciliation mission_project mission_timeline.wait compiled-deployment-policy scripts/deploy-daemon.sh deploy-center-release-closure-ledger]
