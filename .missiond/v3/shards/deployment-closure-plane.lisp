(deployment-closure-plane
    ...
    :invariants
      ["MissionD MUST NOT infer production closure from GitHub success, curl probes, local git state, or PTY logs when Deploy Center closure evidence exists."
       ...
       "Deploy-chain self-update preflight MUST expose credential presence and Secret Store refs only; read-token refs may default to secret-store://missiond/production/MISSIOND_DEPLOY_CENTER_READ_TOKEN, write-token refs may default to deploy/ci_deploy_api_key, and write-token values must never be emitted."
