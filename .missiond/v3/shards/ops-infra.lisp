  (ops-infra
    ...
    :invariants
      ["Daemon redeploy MUST stay one command: build -> candidate release -> manifest -> active symlink -> launchctl kickstart -> socket wait -> IPC smoke."
       ...
       "Deploy scripts MUST serialize active-switch and apply-cleanup mutations through a deploy ownership lock, defaulting to $MISSIOND_INSTALL_ROOT/deploy.lock.d; concurrent deploys must fail with lock owner evidence instead of racing active."
