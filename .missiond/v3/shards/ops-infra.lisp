  (ops-infra
    ...
    :invariants
      ["Daemon redeploy MUST stay one command: build -> candidate release -> manifest -> active symlink -> launchctl kickstart -> socket wait -> IPC smoke."
       ...
       "Deploy scripts MUST serialize active-switch and apply-cleanup mutations through a deploy ownership lock, defaulting to $MISSIOND_INSTALL_ROOT/deploy.lock.d; concurrent deploys must fail with lock owner evidence instead of racing active, and metadata-less locks may be recovered only after MISSIOND_DEPLOY_LOCK_STALE_SECS."
       ...
       "Deploy scripts MUST verify after active switch, launchd restart, smoke, and cleanup that the active symlink, release-manifest.json, stable entrypoints, launchd WorkingDirectory, MISSIOND_PROJECT_ROOT, MISSIOND_RUNTIME_DIR, and MISSIOND_COMPILED_RUNTIME_DIR still belong to the candidate release before reporting success."
       "Infra skill evidence classifiers MUST treat deployment closure facts as infra evidence candidates, including service.manifest.toml, manifest gate, canary smoke, migration/relation failures, compose, entrypoint, binary marker, and volume override lines; query/project scoped ranking decides whether they are returned."
