...
(service-runtime-universe
    ...
    (deployment-channel-plane
      ...
      :invariants
        ["MissionD V3 SSOT owns declared deployment-channel intent; deploy-center, GitHub Actions, Vercel, and GCP facts annotate observed state and drift but must not silently overwrite declared channels."
         ...
         "Generic Linux native_workflow build jobs must use runner_labels plus required_capabilities for the 10900KF/12900KF build pool; runner_agent_id is a singleton affinity override and requires runner_pin_rationale evidence."
    ...
    (service :id code-center
      ...
      :codebase-boundary (:schema "missiond.codebase-boundary.v1" :source-authority xjp-codebase :canonical-api-prefix "/api/codebase" :legacy-code-api-prefix "/api/code" :git-ssh-endpoint "ssh://git@codebase.xiaojins.com:2222/<repo>.git" :push-trigger-source codebase-push :push-success-authority "ReleasePlan+ReleaseEvidence+ClosureVerdict" :deploy-center-legacy-prefix "/api/deploy/codebase" :deploy-center-legacy-status legacy-read-through :legacy-migration-expiry "2026-06-30")
