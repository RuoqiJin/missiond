...
(project-maturity-registry
    ...
    (maturity :id xjp-project-universe :current M5 :target M6 :gap [live-authority-source-smoke forge-catalog-live-source final-m6-report])
    (maturity :id xjp-domain-service :current M5 :target M6 :gap [live-dns-readiness multi-zone-runtime-smoke final-m6-report])
    (maturity :id xjp-image-service :current M5 :target M6 :gap [media-domain-readiness object-storage-runtime-smoke final-m6-report])
    (maturity :id xjp-video-service :current M5 :target M6 :gap [media-domain-readiness transcode-runner-integration live-video-smoke final-m6-report])
    (maturity :id xjp-video-transcode-runner :current M5 :target M6 :gap [runner-live-registration transcode-runner-smoke final-m6-report])
    ...
    (maturity :id good-things-daily :current M2 :target M6 :gap [presentation-layer-regression feedback-event-regression feed-browser-smoke scheduled-job-runner final-m6-report])
