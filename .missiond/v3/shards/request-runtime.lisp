      ...
      ((function context-gather-artifact
       ...
       (function provider-box-usage-and-catalog-export
         ...
         :core ((step s1 :logic "ask the provider driver for a usage snapshot; classify exact, estimated, blocked, or unknown with source and confidence")
                ...
                (step s5 :logic "for Agy, emit MissionDAgy TOML route entries only for observed model-picker entries with a provider-box-managed logical slot_pool_id, hidden replica policy, pure_text=true, usage status not blocked, and provider_model_id equal to the managed proxy URL for MissionD provider-box; do not infer all models from transcript JSONL and do not export private replica slot ids"))
