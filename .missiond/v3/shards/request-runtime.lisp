  ...
  (grounded-dispatch-policy
      ...
      ((function context-gather-artifact
       ...
       (function provider-interaction-box-turn
         ...
         :core ((step s1 :logic "normalize every non-BoardTask provider CLI request into missiond.provider-interaction-turn.v1 with provider, mode, prompt_hash, attachment_refs, timeout, cwd/project_root, and correlation_id")
                ...
                (step s3 :logic "select or spawn an interactive PTY slot from workstation-pool; direct `claude --print`, ad-hoc `codex exec`, Agy print/prompt modes, Gemini `-p -o stream-json`, and stdin-closed provider subprocesses are forbidden outside the legacy migration inventory and the explicit provider-box codex_exec_text guarded lane")
                (step s4 :logic "dispatch by mode: prompt turns call driver submit-turn, slot-status calls driver status/observe, control-action calls named driver input/clear/exit operations, model-switch calls driver switch-model or respawn, usage-probe calls driver usage-probe, model-catalog-export calls driver model-catalog and router exporter, pure-text-single-turn additionally installs the pure-text guard")
       ...
       (function provider-box-slot-control
         ...
         :core ((step s1 :logic "normalize spawn/status/screen/observe/input/pty-step/clear/exit requests into provider-interaction-turn rather than direct PTY writes by callers")
       ...
       (function provider-box-usage-and-catalog-export
         ...
         :core ((step s1 :logic "read cached provider usage snapshots through GET /provider-box/v1/usage without touching provider PTYs; refresh only through POST /provider-box/v1/usage/refresh")
                (step s1b :logic "ask the provider driver for a usage snapshot; classify exact, estimated, blocked, or unknown with source and confidence")
    ...
    :invariants
      ["All non-exact worker dispatch must carry grounding_context_id before a provider PTY receives the prompt."
       ...
       "Provider CLI access outside normal BoardTask worker dispatch MUST route through interactive-provider-box and persist missiond.provider-interaction-turn.v1; direct `claude --print`, ad-hoc `codex exec`, Agy print/prompt modes, Gemini `-p -o stream-json`, and stdin-closed provider subprocesses are allowed only as explicitly inventoried legacy migration targets, except provider-box codex_exec_text which is a guarded no-tools automation lane and fails closed on JSONL tool/function evidence."
       "Provider CLI spawn/status/input/clear/exit/model-switch/usage/completion APIs belong to MissionD provider-box. semantic-terminal only parses visible terminal state for those drivers and MUST NOT become the API or orchestration boundary."
