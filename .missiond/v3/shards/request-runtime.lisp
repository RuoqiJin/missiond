  (grounding-search-aggregate
    ...
    :invariants
      ["mission_context_gather MUST expose source_profile, source_summaries, evidence_lanes, evidence_items, support_catalog, authority_order, noise_diagnostics, raw_sources_omitted, context_noise_metrics, and navigation_profile."
       "mission_context_gather MUST emit navigation_profile with selected_profiles, rejected_profiles, known_surfaces, required_questions, next_reads, verification_plan, evidence_status, and risk_flags so agents get a profile-aware evidence navigation package rather than only generic lane facts."
       ...
       "mission_context_gather conversation_logs compact summaries MUST preserve non-raw search diagnostics such as query, mode, count, totalHits, ftsHits, vecHits, filteredSemanticHits, and context_gather filter_context project/time_range/query_mode/limit so no-hit bounded conversation searches are distinguishable from disabled or unexecuted conversation lanes without exposing raw messages."
       ...
       "mission_context_gather evidence_lanes.skill_evidence.item_count MUST count compact injectable skill metadata/project links/infra items, not raw skill operational_facts; raw operational_fact_count remains diagnostic-only in source_summaries."
       "mission_context_gather evidence_lanes.support_refs.item_count MUST count compact injectable support evidence items, including support_catalog plus identity-bearing deployment_closure_policy and redacted credential refs, not only source keys."
       "mission_context_gather deployment_closure_policy support_refs MUST be gated to deploy_ops/full_debug, explicit infra target, or deployment-closure query anchors; non-deploy intent_default/conversation_audit requests may keep base support_catalog but MUST filter deployment_closure_policy from current projection and persisted evidence_item search, exposing deployment_closure_profile_filtered_count."
       "mission_context_gather compact responses and worker context packs MUST omit the full support_catalog.deployment_closure payload unless source_profile=deploy_ops/full_debug, include_raw_sources=true, explicit infra target, or deployment-closure query anchors opt in; compact support_catalog may expose deployment_closure_omitted diagnostics plus base project/service/support identity."
       "mission_context_gather source_profile=deploy_ops MUST query bounded MissionD EventBridge deployment_events from event_log domain system::external_service_event, keep only deploy-center events matching the resolved project/service/deploy slug before response/context injection, expose source_summaries.deployment_events.status and a top-level deployment_events summary alias when none are available, and never fall back to conversation/tool timeline hits for deploy evidence."
       "mission_context_gather deployment_events MUST treat deploy-center native workflow and artifact provenance kinds (workflow_run_created, workflow_job_started/succeeded/failed/cancelled/lease_expired, artifact_recorded) as scoped deploy evidence when the event carries deploy-center authority and project/service scope."
       "mission_context_gather deployment_events MUST expose drop_reason_counts and sample_dropped_events diagnostics for filtered EventBridge rows while never injecting dropped rows as evidence."
       "mission_context_gather deployment_events sample_dropped_events MUST omit low-value irrelevant_event_kind samples such as shared-memory worker telemetry, preserve their counts in drop_reason_counts/drop_sample_omitted_counts, and reserve samples for high-signal scope/authority/envelope failures."
       "mission_context_gather deployment_events MUST expose accepted_event_kinds and observed_candidates producer/event/source/authority counts so operators can distinguish relay/config absence from query filter mismatch without inspecting raw event_log rows."
       "mission_context_gather deployment_events MUST expose relay_diagnostics with inferred_gap values such as deploy_center_relay_absent_or_disabled and missiond_webhook_ingest_ok_deploy_center_relay_absent plus expected Deploy Center relay env/webhook fields when scoped Deploy Center events are unavailable. observed_candidates MUST distinguish deploy-center producer-shaped webhook probes from authoritative deploy-center.deploy_events rows via observed_webhook_ingest_probe, observed_authoritative_deploy_center_source, webhook_ingest_probe_candidate_count, and authoritative_deploy_center_candidate_count; non-authoritative manual_probe/codex-local-probe events may prove MissionD webhook ingest health but MUST NOT be treated as deploy evidence or as proof that Deploy Center relay emitted durable deploy_events rows. relay_diagnostics MUST include ingest_health plus a local_config_probe that uses the resolved deploy-center support_catalog root to check only file presence and relay env variable names in compose/services.yaml/service.manifest/code refs, never secret values, plus typed next_actions for relay enablement. local_config_probe MUST include secret_store_key_probe with expected namespace xjp-deploy-center, required key names DEPLOY_EVENT_RELAY_ENABLED/MISSIOND_EVENTBRIDGE_URL/MISSIOND_EXTERNAL_WEBHOOK_TOKEN, and the authority handoff to Deploy Center config-health manifest_secret_store_env plus manifest-verify. When local_config_probe shows relay env names are present but deploy-center events remain absent, next_actions MUST switch from static config repair to Secret Store key adoption, runtime env value, webhook reachability, and token-match verification, deploy_events cursor inspection, and deploy_logs-to-deploy_events write-path verification without printing secret values. deployment-event unknowns synthesize evidence_gap from source_summaries.deployment_events/relay_diagnostics instead of leaving obvious deploy_events/webhook/relay questions as generic needs_synthesis. source_profile=deploy_ops top-level next_action MUST surface deployment_events authority gap remediation from relay_diagnostics.next_actions before generic intent/plan synthesis when no authoritative deploy-center.deploy_events rows are available."
       "mission_context_gather source_profile=deploy_ops MUST distinguish no infra evidence from infra-os feature_disabled by returning source_summaries.infra.status=feature_disabled and diagnostics when MISSIOND_FEATURE_INFRA_OS_ENABLE/MISSIOND_FULL_OS_ENABLE is not enabled; optional feature_disabled diagnostics MUST NOT set the top-level ok=false by themselves. If scoped support_catalog/deployment_closure evidence exists, source_summaries.infra MUST include fallback_status=support_catalog_available and compact fallback_items so deploy facts are not hidden by the optional infra-os gate."
       ...
       "mission_context_gather MUST NOT project or persist support_catalog evidence for unresolved empty structural catalogs; support_catalog evidence requires real project/service/deploy/runtime/domain/endpoint/manifest/database/agent/deployment-closure content, not only schema/authority/secret_policy or empty object shells."
       "mission_context_gather context_noise_metrics.evidence_item_read_model MUST expose hit_count, raw_hit_count, deduplicated_count, and truncated_count so repeated context_gather/backfill projections and limit truncation are visible as separate retrieval noise classes."
       "mission_context_gather MUST skip persisted evidence_items read-model search for unresolved unscoped requests unless source_profile=full_debug; context_noise_metrics.evidence_item_read_model MUST expose scope_skipped so project-scoped support_refs cannot leak into unscoped conversation_audit/default contexts."
       ...
       "mission_context_gather MUST assign stable evidence_item IDs for volatile compact projections runtime_environment, project_resolution, project_registry, ssot, support_catalog, and deployment_closure_policy using profile/project/task/source identity rather than release path/hash-bearing summary text, so upsert overwrites the current projection instead of accumulating stale per-deploy content-hash rows."
  ...
  (grounded-dispatch-policy
    ...
    :authority [mission_context_gather mission_task_delegate mission_swarm_run autopilot-runtime shared-memory interaction-grounding-report task-result-artifact]
    :bypass-policy
      ((case exact-shard
    ...
    :functions
      ...
      (function plan-atomization-compiler
       ...
       (function provider-interaction-box-turn
         ...
         :core ((step s1 :logic "normalize every non-BoardTask provider CLI request into missiond.provider-interaction-turn.v1 with provider, mode, prompt_hash, attachment_refs, timeout, cwd/project_root, and correlation_id")
                ...
                (step s8 :logic "write provider-interaction-turn plus interaction-direct-answer, task-result-artifact, provider-usage-snapshot, provider-model-catalog, or provider-router-export projection only after matched evidence exists; Jarvis grounded direct answers may instead use plan.lisp direct_answer_draft as canonical terminal content with answer_source=plan_direct_answer_draft before entering provider fallback")
                (step s9 :logic "on timeout, PROVIDER_TURN_TIMEOUT_CANCEL_FAILED, provider auth/billing/quota block, unsupported driver capability, missing durable final, stale final mismatch, model-switch unverified, usage unknown, step verification failure, pure-text guard violation, or MISSIOND_JARVIS_DIRECT_ANSWER_STREAM_BUDGET_SECS expiry, emit typed diagnostic + result_pending without synthesizing an answer from PTY screen text"))
      ...
      (function jarvis-result-artifact-gate
       ...
       (function jarvis-grounded-direct-answer
         :entry [JarvisSSE confirmed-plan grounding_context_id grounding_report_file intent_artifact_id plan_artifact_id provider-interaction-box]
         :core ((step s1 :logic "read execution_mode and requires_board_task from confirmed plan metadata")
                ...
                (step s3 :logic "load grounding_report_preview from grounding_report_file first, then bounded context preview and source refs from context_pack_file/shared artifact")
                ...
                (step s7 :logic "if grounding_report_file is absent/unreadable, provider-interaction-box auth/driver/CLI readiness fails, or the provider cannot produce a matched durable final, fail fast with typed diagnostic and no fallback BoardTask"))
    ...
    :invariants
      ["All non-exact worker dispatch must carry grounding_context_id before a provider PTY receives the prompt."
       "Jarvis strict gate MUST NOT run full MissionD context collection before intent.lisp confirmation; the initial turn authors intent.lisp only, and the intent-confirmed turn runs mission_context_gather plus ClaudeCode MCP interaction-grounding-report before plan.lisp."
       "Jarvis plan confirmation MUST reuse grounding fields carried in confirm payload (grounding_context_id, context_pack_file, grounding_report_file/hash, grounding_worker_slot_id/turn_id, context_sufficiency, sources_used) and MUST NOT re-run mission_context_gather or the ClaudeCode grounding worker."
       "Jarvis plan authoring MUST receive grounding_report_preview; it may choose grounded_direct_answer with requires_board_task=false when the report is sufficient, otherwise work_order/investigation_only must create BoardTask."
       "Jarvis direct-answer and BoardTask runtime_metadata MUST carry interaction-grounding-report references so archive/replay can reconstruct the evidence chain."
