    ...
    (pillar communication
      ...
      (function interaction-gateway
        ...
        :core ((step s1 :logic "normalize Web, iOS, Jarvis, WeChat bridge, and service-trigger input into missiond.interaction-envelope.v1; legacy OpenAI-compatible /v1/chat/completions must use handle_chat_completions_interaction_adapter/openai_request_to_interaction_envelope and must not directly write provider PTYs")
               (step s2 :logic "resolve identity through Auth: bearer JWT for Web/iOS/Jarvis MUST call Auth /oidc/userinfo or configured MISSIOND_INTERACTION_AUTH_USERINFO_URL; service tokens use a separate internal service context, target Auth client_credentials with missiond:interact, and migration MISSIOND_INTERACTION_SERVICE_TOKEN/MISSIOND_API_TOKEN must be configured or explicitly non-production compatible through MISSIOND_ALLOW_UNCONFIGURED_API_TOKEN; missing service token fails closed with MISSIOND_SERVICE_TOKEN_UNCONFIGURED and monitor emits missiond.jarvis-auth-secret-readiness.v1. Smoke automation must inject MISSIOND_INTERACTION_SERVICE_TOKEN from secret-store ref missiond.jarvis-smoke/INTERACTION_SERVICE_TOKEN or let scripts/smoke-jarvis-interaction.mjs read that same ref through MISSIOND_JARVIS_SMOKE_SECRET_REF; deploy persists interaction auth env keys into launchd only when supplied by the operator environment; wechat_openid/unionid without binding returns identity_binding_required; Auth timeout or non-2xx response returns INTERACTION_AUTH_UNAVAILABLE and must not create BoardTask side effects")
               ...
               (step s5 :logic "write intent_archived by default for Jarvis/iOS/OpenAI replay analysis; emit intent_draft + confirm_required only for explicit legacy/manual-review mode")
               (step s6 :logic "write key_judgment_draft and plan_archived by default; plan_draft + confirm_required is legacy/manual-review only, and plan_archived must include execution_mode, requires_board_task, and direct_answer_draft when execution_mode=grounded_direct_answer")
               (step s7 :logic "when plan_archived selects grounded_direct_answer with requires_board_task=false, stream answer_delta/result_artifact/final from the plan direct_answer_draft or bounded provider fallback; otherwise create BoardTask metadata with interaction_id, PermissionContext, grounding_context_id, intent_artifact_id, plan_artifact_id, and dispatch contract")
               (step s8 :logic "stream response sink events for status, intent_archived, key_judgment_draft, plan_archived, BoardTask, worker, task-result-artifact, final, and typed diagnostics; PTY output remains diagnostic only"))
