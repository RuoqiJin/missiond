...
(surface ops-infra
      ...
      :note "ops-infra owns deploy-daemon.sh, bootstrap-managed-mac-node.sh, formatter-converged Rust hygiene, and restart-time background CPU policy. bootstrap-managed-mac-node.sh is the default managed Mac node bootstrap action: install Homebrew when absent, install/link libpq, add Homebrew/libpq to the managed shell PATH, and verify psql. deploy-daemon.sh builds paired missiond/mission-mcp releases under ~/.xjp-mission/releases/<release-id>, writes release-manifest.json, records release_owner_root for ownership checks, rejects cross-project-root active mutation unless MISSIOND_DEPLOY_ALLOW_PROJECT_ROOT_TAKEOVER=1 is explicitly set against MISSIOND_DEPLOY_EXPECTED_ACTIVE_ROOT, captures the active release generation before build, rejects pre-switch overwrite when active changed unless MISSIOND_DEPLOY_ALLOW_ACTIVE_RELEASE_RACE=1 is explicit, rejects same-owner stale commit deploys unless MISSIOND_DEPLOY_ALLOW_COMMIT_REGRESSION=1 is explicit, compiles V3 runtime projections into a candidate release-local compiled-runtime directory, switches active, writes a release-local source snapshot, rewrites launchd WorkingDirectory plus MISSIOND_PROJECT_ROOT/MISSIOND_ORCHESTRATOR_ROOT to that immutable snapshot, points MISSIOND_COMPILED_RUNTIME_DIR at the active release-local compiled-runtime artifact while keeping MISSIOND_RUNTIME_DIR under the owner-root runtime namespace, reloads launchd through bootout/bootstrap before kickstart, runs MCP smoke, rolls back, cleans releases, discovers managed-node Node.js/OCaml/Homebrew libpq Postgres client paths, and defaults CARGO_INCREMENTAL=0 for disk-bounded deploys. Managed Mac node bootstrap must install Homebrew plus libpq/psql before the node is considered operational, because psql is the standard Postgres diagnostic and backfill client for Board runtime metadata and interaction-ledger repair. rustfmt-missiond.sh is the M6 formatter gate; cargo-fmt-touched.sh remains a scoped fallback. AST startup full sync is opt-in, and ast_sync_worker skips topology KB rewrites when no stale files were synced.")
...
(surface missiond-blue-green-self-update
      ...
      :code ["scripts/deploy-daemon.sh"
             ...
             "scripts/check-v3-ops-infra-isomorphism.mjs"
             "scripts/check-v3-sysinfra-control-isomorphism.mjs"]
      :note "MissionD self-update is owned as a blue-green release workflow. Release candidates are immutable directories under ~/.xjp-mission/releases/<release-id>; the active symlink is the only switch; daemon and MCP entrypoints both resolve through active so they share one release-manifest.json. The deploy path separates release_owner_root from launchd_project_root: owner root gates active mutation, while launchd_project_root points at the release-local source snapshot that matches compiled runtime source_units. Active mutation is rejected when the active release owner or legacy launchd root belongs to a different project root unless MISSIOND_DEPLOY_ALLOW_PROJECT_ROOT_TAKEOVER=1 is explicitly set; the active release generation observed at script start is also checked before switch, and concurrent active changes fail closed unless MISSIOND_DEPLOY_ALLOW_ACTIVE_RELEASE_RACE=1 is explicit. Same-owner stale commit deploys are rejected when the candidate git_full_sha is not a descendant of the current active release commit, unless MISSIOND_DEPLOY_ALLOW_COMMIT_REGRESSION=1 is explicit for rollback or branch divergence. Compiled runtime projections are release artifacts: deploy compiles them under the candidate release and switches MISSIOND_COMPILED_RUNTIME_DIR with active, while MISSIOND_RUNTIME_DIR remains the owner-root runtime namespace for mutable runtime state. Generated V3 contract ABI is verified with node scripts/project-v3-contracts.mjs --check --json, it may refresh only under explicit MISSIOND_DEPLOY_REFRESH_CONTRACTS=1 developer mode, compiles typed Lisp runtime projections via node scripts/compile-v3-runtime.mjs --json before building binaries, records compiled projection schema/source/file hashes in typed_lisp_runtime, supports legacy direct-binary migration, verified linker signature acceptance before force-sign fallback, pre-switch MCP smoke, launchd runtime-root rewrite and reload, post-switch daemon IPC smoke, previous-release rollback, cleanup-only dry-run/apply, removal of incomplete release dirs, retention of active/previous/newest releases, and CARGO_INCREMENTAL=0 by default to keep self-update disk-bounded.")
)
