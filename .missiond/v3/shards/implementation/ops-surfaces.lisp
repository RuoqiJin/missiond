...
(surface ops-infra
      ...
      :note "ops-infra owns deploy-daemon.sh, bootstrap-managed-mac-node.sh, formatter-converged Rust hygiene, restart-time background CPU policy, managed-node tool discovery, launchd reload, smoke/rollback/cleanup, and disk-bounded release defaults. bootstrap-managed-mac-node.sh establishes Homebrew/libpq/psql before a managed Mac node is operational. deploy-daemon.sh builds paired missiond/mission-mcp blue-green releases and enforces owner-root/active-generation/runtime-projection safety. Detailed deploy-daemon and formatter behavior lives in blueprint-notes note-024.")
...
(surface missiond-blue-green-self-update
      ...
      :code ["scripts/deploy-daemon.sh"
             ...
             "scripts/check-v3-ops-infra-isomorphism.mjs"
             "scripts/check-v3-sysinfra-control-isomorphism.mjs"]
      :note "MissionD self-update is a blue-green release workflow: immutable candidates under ~/.xjp-mission/releases/<release-id>, active symlink as the only switch, paired daemon/MCP entrypoints, release-local compiled runtime artifacts, owner-root runtime namespace, guarded takeover/race/regression overrides, launchd rewrite/reload, smoke, rollback, cleanup, and disk-bounded build defaults. Detailed self-update safety and artifact behavior lives in blueprint-notes note-025.")
)
