...
(surface mission-shared-memory
      ...
      :note "MissionD shared memory is the Rust/Postgres durable coordination substrate. shared_events/shared_artifacts/shared_claims/agent_cursors plus jobs/job_attempts/work_leases/capability_grants/capability_audit_events/board_task_views hold typed runtime truth. Implementation workers need canonical task_contracts, active subject-bound capability grants, sandbox_profile, current job_attempts, and write-scope work_leases. work_leases acquisition is DB-atomic per (scope_kind, scope_key): expire stale leases, SELECT FOR UPDATE, then insert while holding a transaction advisory lock plus active-scope DB constraint; shared_claims is a compatibility mirror/projection. Conflicts return typed CLAIM_CONFLICT, never prose parsing. evidence_view projects Memory/KB + Logs/Timeline/Conversation; task_result_artifacts are canonical worker outputs, while BoardTask state is a coordination projection.")
