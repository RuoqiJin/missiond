...
(surface interaction-gateway
      ...
      :legacy-adapter (:route "/v1/chat/completions"
                       ...
                       :rule "OpenAI-compatible Jarvis/iOS clients are wire-compatible only at the edge. The route must normalize into InteractionEnvelope and pass through Auth PermissionContext, grounding, Codex CLI GPT-5.5 xhigh headless intent.lisp authoring, intent/plan confirmation, BoardTask metadata, and task-result-artifact; it must not directly write provider PTYs for execution. Every intent_draft MUST be semantically authored by the codex-intent-author worker lane via `codex exec --json --output-last-message` and carry author=codex-cli-gpt-5.5-xhigh, review_text, artifact_body, and artifact_language; Rust may validate and schema-normalize but MUST NOT silently fall back to deterministic intent generation or parse interactive TUI screen text as semantic output. Every plan_draft semantic event MUST carry review_text plus artifact_body/artifact_language so the user can inspect the review artifact before confirmation. The adapter MUST also emit an OpenAI-compatible delta projection carrying human-readable artifact summary plus missiond_projection payload so legacy clients do not hide review artifacts. confirm_required payloads MUST be persisted as missiond.jarvis-pending-confirmation.v1 conversation control markers, so legacy clients that later send conservative confirmation text such as 确认/确认意图/确认 plan can resume the pending intent/plan gate without losing the original missiond_confirm payload.")
