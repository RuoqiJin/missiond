...
(surface interaction-gateway
      ...
      :public-tools [mission_interaction]
	      :auth-boundary (:authority auth
	                      ...
	                      :service-token-policy "Auth client_credentials service tokens are the target authority; legacy MISSIOND_API_TOKEN/MISSIOND_INTERACTION_SERVICE_TOKEN is a migration binding that must be configured or explicitly disabled through non-production compatibility, never arbitrary bearer acceptance."
	                      :monitor-schema "missiond.jarvis-auth-secret-readiness.v1"
	                      :timeout-env MISSIOND_INTERACTION_AUTH_TIMEOUT_MS
	                      :failure-code INTERACTION_AUTH_UNAVAILABLE
	                      :rule "Web/iOS/Jarvis bearer tokens must be resolved through Auth userinfo before PermissionContext is accepted. Service callers use Auth client_credentials JWTs with missiond:interact or the configured migration service-token binding; MissionD fails closed with MISSIOND_SERVICE_TOKEN_UNCONFIGURED when no service token is configured and MISSIOND_ALLOW_UNCONFIGURED_API_TOKEN is not explicitly enabled for non-production. Automated Jarvis smoke uses MISSIOND_INTERACTION_SERVICE_TOKEN injected from secret-store ref missiond.jarvis-smoke/INTERACTION_SERVICE_TOKEN, or scripts/smoke-jarvis-interaction.mjs reads that same ref via MISSIOND_JARVIS_SMOKE_SECRET_REF; never use a fake token or repo-stored value. Deploy persists interaction auth env keys into launchd only when supplied by the operator environment. Metadata roles/tenant/application fields are hints only; MissionD must fail fast when Auth is unavailable instead of creating BoardTask side effects.")
	      :media-attachments (:schema "missiond.interaction-media-context.v1"
	                          :attachment-schema "missiond.interaction-media-attachment.v1"
	                          :binary-owner xjp-image-service
	                          :client-flow "xiaojinproIOS uploads bytes to xjp backend image service, then calls /jarvis/v1/chat/completions or /interactions/v1/messages with image_service_ref/artifact_id metadata"
	                          :normalizers [openai_request_attachments normalize_interaction_attachments normalize_interaction_attachment_ref interaction_media_context]
	                          :fields [attachment_refs image_service_ref artifact_id artifact_hash media_type sha256 size_bytes width height source_url_redacted signed_url_present]
	                          :rule "Jarvis image input is reference-only. OpenAI/iOS content parts image_url/imageUrl/media_artifact and native InteractionEnvelope.attachments normalize into missiond.interaction-media-attachment.v1 refs. data:image base64 is rejected with IMAGE_INLINE_DATA_URL_REQUIRES_XJP_UPLOAD and must not be written to /tmp/missiond_media, event logs, artifacts, or provider prompts. Signed URL values are never persisted or streamed with query/token material; only redacted URL presence/path and xjp-image-service refs are carried through received, intent.lisp, grounding request/report, key judgment, plan.lisp, direct answer, BoardTask runtime_metadata, archive, and replay.")
	      :legacy-adapter (:route "/v1/chat/completions"
                       ...
                       :projection "missiond.openai-artifact-projection.v1"
                       :pending-confirmation-schema "missiond.jarvis-pending-confirmation.v1"
                       :execution-mode-schema "missiond.jarvis-plan-execution-mode.v1"
                       :execution-modes [grounded-direct-answer work-order investigation-only]
                       :direct-answer-provider "provider-interaction-box"
                       :direct-answer-default-text-provider "codex_cli"
                       :direct-answer-provider-env MISSIOND_JARVIS_DIRECT_ANSWER_PROVIDER
                       :direct-answer-endpoint-env []
                       :intent-plan-author-slot-env [MISSIOND_JARVIS_INTENT_AUTHOR_SLOT_ID MISSIOND_JARVIS_PLAN_AUTHOR_SLOT_ID]
                       :interactive-provider-box "provider-interaction-box"
                       :headless-legacy-status retired-for-jarvis
                       :semantic-authoring-anchors ["Codex CLI GPT-5.5 xhigh provider_box intent.lisp authoring"
                                                    "Codex CLI GPT-5.5 xhigh provider_box plan.lisp authoring"
                                                    "codex-intent-author"
                                                    "codex-plan-author"
                                                    "author=codex-cli-gpt-5.5-xhigh"]
                       :direct-answer-artifact-schema "missiond.interaction-result-artifact.v1"
                       :communication-preferences-file "MISSIOND_JARVIS_COMMUNICATION_PREFERENCES_FILE or MISSIOND_RUNTIME_DIR/jarvis/communication-preferences.lisp"
                       :direct-answer-rule "Plan confirmation does not always create a BoardTask. Every plan_draft/plan_archived MUST carry execution_mode, requires_board_task, answer_policy, provider_hint, and direct_answer_draft when execution_mode=grounded_direct_answer. grounded_direct_answer is the mandatory path for simple Q&A, identity/status/explanation, and other requests whose answer can be produced from persisted grounding context without repository mutation or worker investigation; plan authoring MUST receive Auth-derived PermissionContext and set requires_board_task=false. Current grounded_direct_answer first accepts the Codex-authored plan direct_answer_draft as the canonical fast terminal answer, emits answer_delta/result_artifact/final, and writes interaction-direct-answer as missiond.interaction-result-artifact.v1 with answer_source=plan_direct_answer_draft. If no draft is available, it may call provider-interaction-box mode=grounded-direct-answer within MISSIOND_JARVIS_DIRECT_ANSWER_STREAM_BUDGET_SECS; budget expiry emits typed diagnostic + result_pending instead of hanging. If provider-box auth, provider CLI availability, or matched durable final extraction fails, the adapter MUST fail fast with a typed diagnostic and MUST NOT call xjpcode text-only, create a fallback BoardTask, or fabricate an answer. work_order and investigation_only MUST set requires_board_task=true and continue through BoardTask/work-order dispatch after plan confirmation."
	                       :rule "OpenAI-compatible Jarvis/iOS clients are wire-compatible only at the edge. The route must normalize into InteractionEnvelope and pass through Auth PermissionContext, semantic intent.lisp authoring, grounding, key_judgment, semantic plan.lisp authoring, optional legacy/manual-review confirmation, BoardTask metadata, and task-result-artifact; it must not directly write provider PTYs for execution. Default Jarvis/iOS/OpenAI flow archives intent.lisp and plan.lisp as intent_archived/plan_archived without user-blocking confirmation cards; confirm_required is emitted only when MISSIOND_JARVIS_REQUIRE_CONFIRMATION or legacy/manual-review mode explicitly asks for it. Image uploads from xiaojinproIOS MUST use xjp-image-service as the binary owner: OpenAI image_url/imageUrl/media_artifact parts and native attachments become media_context refs, image-only messages are accepted with a conservative default objective, data:image base64 is rejected, and signed URL query/token values are redacted before any SSE/artifact/conversation persistence. Every intent_draft/intent_archived MUST be semantically authored through provider-interaction-box mode=semantic-authoring over an interactive Codex PTY lane, carry author metadata, review_text, artifact_body/artifact_language, media_context, and read semantic output from correlated durable provider logs. Rust may validate and schema-normalize but MUST NOT silently fall back to deterministic intent generation, `codex exec`, xjpcode text-only, or interactive TUI screen text as semantic output. Every plan_draft/plan_archived MUST be semantically authored by the same provider-box backend and carry author metadata, review_text, artifact_body/artifact_language, plan_author_slot_id, concrete steps, media_context, and direct_answer_draft when execution_mode=grounded_direct_answer; Rust may validate and schema-normalize but MUST NOT silently fall back to deterministic plan generation. Confirmation turns MUST reuse the prior artifact ids from confirm_required payloads and MUST NOT re-author intent.lisp or plan.lisp during confirmation. If plan-author execution is unavailable or returns an invalid contract, the route emits JARVIS_PLAN_AUTHOR_FAILED and stops before BoardTask creation. The AGY Gemini 3.1 Pro communication officer receives communication_preferences_lisp from MISSIOND_JARVIS_COMMUNICATION_PREFERENCES_FILE or MISSIOND_RUNTIME_DIR/jarvis/communication-preferences.lisp; MissionD appends redacted preference-observation forms when user text explicitly asks for communication style changes, but provider-box AGY turns remain no_file_access and may not write preference files. The adapter MAY emit OpenAI-compatible delta projections for reviewable artifacts in legacy/manual-review mode; default archive-only mode must persist artifacts for replay without showing confirmation cards. During grounding, semantic authoring, artifact writes, and BoardTask creation, progress MUST be bus-first: write missiond.jarvis-progress.v1 as SystemEvent::ExternalServiceEvent service_id=missiond-jarvis event_kind=jarvis_progress, then read back the matching event_id from the frontend EventBus bridge before projecting a status event and OpenAI-compatible progress delta; direct projection is allowed only as a visible event_bus_projection fallback when the bus write/read path is unavailable. Heartbeat progress MUST continue while semantic authoring/grounding is still running, and any failure MUST name the failed phase such as grounding, intent_authoring_failed, key_judgment_authoring_failed, plan_authoring_failed, intent_artifact, plan_artifact, direct_answer, or board_task_create. confirm_required payloads MUST be persisted as missiond.jarvis-pending-confirmation.v1 conversation control markers only for legacy/manual-review mode so legacy clients can resume without losing the original missiond_confirm payload.")
      ...
      :events [received authenticated permission_resolved grounding intent_archived intent_draft key_judgment_draft plan_archived plan_draft confirm_required communicator_status communicator_final board_task_created worker_status result_pending result_artifact diagnostic final]
      :note "Unified external channel entry for Web, iOS, Jarvis, WeChat bridge, and service triggers. The HTTP adapter converts every external message to InteractionEnvelope, resolves Auth-derived PermissionContext, writes archived intent/grounding/key-judgment/plan artifacts by default, creates BoardTasks only when plan execution_mode requires them, and returns status/result through channel response sinks. mission_interaction is the MCP facade for receive/confirm_intent/confirm_plan/follow/status; legacy Jarvis chat remains wire-compatible but is treated as a channel adapter, not a direct PTY path.")
