...
(surface interaction-gateway
      ...
      :legacy-adapter (:route "/v1/chat/completions"
                       ...
                       :direct-answer-endpoint-env [MISSIOND_XJPCODE_TEXT_ONLY_URL MISSIOND_XJPCODE_TEXT_ONLY_ENDPOINT MISSIOND_XJPCODE_BASE_URL]
                       :intent-plan-author-provider-env MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_PROVIDER
                       :interactive-provider-box "provider-interaction-box"
                       :headless-legacy-status migration-only
                       :semantic-authoring-anchors ["Codex CLI GPT-5.5 xhigh headless intent.lisp authoring"
                       ...
                       :direct-answer-rule "Plan confirmation does not always create a BoardTask. Every plan_draft MUST carry execution_mode, requires_board_task, answer_policy, and provider_hint. grounded_direct_answer is the mandatory path for simple Q&A, identity/status/explanation, and other requests whose answer can be produced from the persisted grounding context without repository mutation or worker investigation; plan authoring MUST receive the Auth-derived PermissionContext, it MUST set requires_board_task=false, and direct-answer materialization MUST reuse the same PermissionContext instead of a legacy placeholder. Current grounded_direct_answer calls xjpcode text-only through MISSIOND_XJPCODE_TEXT_ONLY_URL or MISSIOND_XJPCODE_BASE_URL/provider/v1/text-only/completions, defaults to codex_cli via MISSIOND_JARVIS_DIRECT_ANSWER_PROVIDER, emits answer_delta/result_artifact/final, and writes interaction-direct-answer as missiond.interaction-result-artifact.v1; this path is migration-only and must be replaced by provider-interaction-box mode=grounded-direct-answer while preserving the same artifact and streaming surface. If the xjpcode text-only endpoint is unavailable on the legacy path, the adapter MUST fail fast with a typed diagnostic and MUST NOT create a fallback BoardTask or fabricate an answer. work_order and investigation_only MUST set requires_board_task=true and continue through BoardTask/work-order dispatch after plan confirmation."
                       :rule "OpenAI-compatible Jarvis/iOS clients are wire-compatible only at the edge. The route must normalize into InteractionEnvelope and pass through Auth PermissionContext, grounding, semantic intent.lisp authoring, semantic plan.lisp authoring, intent/plan confirmation, BoardTask metadata, and task-result-artifact; it must not directly write provider PTYs for execution. Every intent_draft MUST be semantically authored by the configured author backend: current legacy default codex-intent-author via `codex exec --json --output-last-message`, or xjpcode text-only paid CLI when MISSIOND_JARVIS_AUTHOR_TEXT_ONLY_PROVIDER is set, and carry author metadata, review_text, and artifact_body/artifact_language; this headless/text-only backend is migration-only and the target backend is provider-interaction-box mode=semantic-authoring over interactive PTY plus durable provider logs. Rust may validate and schema-normalize but MUST NOT silently fall back to deterministic intent generation or parse interactive TUI screen text as semantic output. Every plan_draft MUST be semantically authored by the same configured author backend and carry author metadata, review_text, artifact_body/artifact_language, plan_author_slot_id, and concrete steps; Rust may validate and schema-normalize but MUST NOT silently fall back to deterministic plan generation. Confirmation turns MUST reuse the prior artifact ids from confirm_required payloads and MUST NOT re-author intent.lisp or plan.lisp during confirmation. If plan-author execution is unavailable or returns an invalid contract, the route emits JARVIS_PLAN_AUTHOR_FAILED and stops before BoardTask creation. The adapter MUST also emit an OpenAI-compatible delta projection carrying human-readable artifact summary plus missiond_projection payload so legacy clients that only render chat deltas still show reviewable artifacts. During grounding, semantic authoring, artifact writes, and BoardTask creation, progress MUST be bus-first: write missiond.jarvis-progress.v1 as SystemEvent::ExternalServiceEvent service_id=missiond-jarvis event_kind=jarvis_progress, then read back the matching event_id from the frontend EventBus bridge before projecting a status event and OpenAI-compatible progress delta; direct projection is allowed only as a visible event_bus_projection fallback when the bus write/read path is unavailable. Heartbeat progress MUST continue while semantic authoring is still running, and any failure MUST name the failed phase such as grounding, intent_authoring_failed, plan_authoring_failed, intent_artifact, plan_artifact, or board_task_create. confirm_required payloads MUST be persisted as missiond.jarvis-pending-confirmation.v1 conversation control markers, so legacy clients that later send conservative confirmation text such as 确认/确认意图/确认 plan can resume the pending intent/plan gate without losing the original missiond_confirm payload.")
