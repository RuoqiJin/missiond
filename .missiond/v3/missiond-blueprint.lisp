...
(missiond-blueprint
  ...
  (implementation-map
    ...
    (surface workstation-dispatch
      ...
      :anchors [run_workstation_dispatch_with_contract_and_trace classify_task_kind build_task_brief "proposal model label projects from router-runtime-policy queued_sonnet_model" extract_inner_board_task_id dry_run_no_dispatch DryRun Dispatched ParsedTaskContract InferenceContext SafeDescriptorReason BriefTaskKind parse_task_contract evaluate_dispatch_decision outcome_to_response_fields]
      :note "workstation-dispatch is the substrate called by mission_plan execute_internal after target=mission_task_delegate is selected; workstation-config owns slot/model/prompt setup, while this surface owns the WorkstationDispatchOutcome response vocabulary and the handoff contract. [details: .missiond/v3/evidence/blueprint-notes.lisp#note-013]")
