    ...
    (surface evidence-collector
      ...
      :note "EVIDENCE_SCHEMA_VERSION pins the evidence wire shape for verification-receipt consumers. EventRefStatus is the closed status enum live | log | unavailable describing whether an event ref is live from publish, recovered from the event log, or unavailable. EventRefProvenance is the closed provenance enum live | passive_cache | event_log_query | unavailable so consumers can distinguish immediate publish results, the bounded in-memory passive cache, and the bounded event_log_query recovery path. EVENT_REF_CACHE_CAP = 1024 fixes the passive cache capacity. wrap_legacy_record_evidence lifts caller-supplied JSON evidence into the typed EvidenceEntry envelope without losing prior fields, keeping plan.rs compatibility while making the receipt payload Lisp-addressable. plan/execution_runtime.rs owns the mission_plan internal dispatch evidence append path through evidence_collector::append, while plan/evidence_sidecar.rs owns manual record_evidence compatibility wrapping.")
	    ...
	    (surface mission_execution-log
	      ...
	      :note "mission_execution-log is the durable companion-log and live-projection surface for mission_execution. agent_execution/lisp_syntax.rs owns the shared S-expression parser, Node/NodeKind, parse, and check_balance delimiter audit. agent_execution/log_store.rs owns COMPANION_DIR .missiond/v2, LogFile/read_log_file/write_log_file, Lisp quoting, canonical template rendering, key/value mutation, block append, and metadata update helpers so the Lisp companion log stays authoritative. agent_execution/log_counters.rs owns ID counters, allocate_id, scan_max_id, and insert_id_counters_block for claims/governance/completion writes and repair backfills. The facade agent_execution.rs keeps the public MCP action router, while agent_execution/log_surface.rs owns normalize_dispatch_strategy, VALID_DISPATCH_STRATEGIES, open/list action wiring, build_opened_event, read_dispatch_metadata_from_log, and emit_execution_event after durable writes succeed. agent_execution/log_governance.rs owns action_deviate, action_decide, action_issue, and their DeviationRecorded / DecisionRecorded / IssueRecorded live event projection. agent_execution/log_status.rs owns action_status plus the status read-model projection for active claims, open issues, unresolved deviations, latest decisions, completed_phases, and durability. agent_execution/session_trace.rs owns append_session_trace_event, resolve_session_trace_path, resolve_trace_task_id, sanitize_trace_backend, TraceEvent, TraceKind, and TraceWarning for the optional task session-trace projection used by open/preflight_commit/complete; trace write failures surface trace_warning and never abort the primary action.")
	    ...
	    (surface mission_execution-completion-audit
	      ...
	      :code ["crates/missiond-daemon/src/handlers/knowledge/agent_execution.rs"
	             ...
	             "crates/missiond-mcp/src/tools/knowledge/agent_execution.rs"
	             "scripts/check-v3-mission-execution-isomorphism.mjs"]
	      :note "mission_execution-completion-audit owns the completion durability gate. action_complete records completion facts from the facade, while agent_execution/completion_records.rs owns VALID_COMMIT_STATUSES, VALID_VERIFIER_STATUSES, VALID_TASK_RUN_VERIFIER_STATUSES, normalize_commit_status, normalize_verifier_status, normalize_task_run_verifier_status, collect_string_list, render_string_list, parse_string_list, CompletionRecord, parse_completions, summarize_durability, and the commit-status-without-hash / commit-status-blocked-without-blocker / scoped-commit-violation finding constants. agent_execution/completion_audit.rs owns action_complete only. agent_execution/completion_entry.rs owns CompletionEntryFields and render_completion_entry for companion-log Lisp entry projection. agent_execution/completion_response.rs owns CompletionResponseFields and build_completion_response for JSON egress projection. agent_execution/completion_indexes.rs owns rebuild_derived_indexes for durable-slot-derived cache reconstruction. agent_execution/completion_maintenance.rs owns action_audit, action_repair, ExecutionEvent::Audited, ExecutionEvent::StaleClaim, and ExecutionEvent::Repaired. agent_execution/completion_gates.rs owns check_id_monotonic, audit_scoped_commit_handoff, enforce_scoped_commit_completion, and enforce_task_contract_completion for scoped commit and task-contract evidence gates. agent_execution/completion_trace.rs owns append_completion_trace_if_requested and the complete/failure session-trace projection for completion responses. agent_execution/completion_verification.rs owns CompletionVerificationOutcome and evaluate_completion_verification for daemon-auto-verifier versus legacy-caller-claim decisioning. action_audit reports findings, and action_repair marks stale claims. agent_execution/task_verifier_inputs.rs owns ReportSummary, SharedMemorySummary, read_report_summary, read_task_contract_id, read_shared_memory_ledger, and read_completion_task_id for report-contract/task-contract/shared-memory Lisp projection. agent_execution/task_verifier_auto.rs owns auto_run_task_run_verifier for the in-process task-run verifier over task-contract/report/shared-memory artifacts. agent_execution/task_verifier.rs owns enforce_verified_completion for the legacy verified=true gate. agent_execution/preflight.rs owns preflight_commit action wiring and session-trace observation before a writer commits. agent_execution/preflight_patterns.rs owns pattern_matches_path plus repo-relative glob normalization. agent_execution/preflight_porcelain.rs owns PorcelainEntry, parse_porcelain_status, and read-only git status. agent_execution/preflight_scope.rs owns build_preflight_summary, claim-scope projection, and task-contract write-scope / must-not-touch projection.")
