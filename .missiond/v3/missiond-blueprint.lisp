...
(missiond-blueprint
  ...
  (implementation-map
    ...
    (surface skill-runtime
      ...
      :note "Code-aligned V3 destination for skill registry and skill execution behavior. skill.rs is the thin mission_skill facade for consolidated mission_skill_query, mission_skill_context, mission_skill_mutate, and direct legacy skill tool names; skill/query.rs owns list/search/topics/actions/stats, FTS/vector ranking, topic hit recording, workflow action projection, composite registry/topic/action/embedding/execution statistics, and project-skill-link readiness; skill/context.rs owns context build/resolve, project-aware context resolution, skill dependency expansion, infra and KB dependency aggregation, and optional BoardTask context projection; skill/mutate.rs owns upsert/record/render/rollback, topic auto-create, block writes, materialization, skill version rollback, and embedding refresh through ProcessSkillTopic; skill/exec.rs owns mission_skill_exec and execute_workflow result/error egress. Additional skill policy knobs must land as dedicated V3 policy sections with checker pins before runtime code reads them.")
