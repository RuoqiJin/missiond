...
(missiond-blueprint
  ...
  (implementation-map
    ...
    (surface workstation-dispatch
      ...
      :note "workstation-dispatch is the substrate called by mission_plan execute_internal after target=mission_task_delegate is selected; workstation-config owns slot/model/prompt setup, while this surface owns the WorkstationDispatchOutcome response vocabulary and the handoff contract. WorkstationDispatchOutcome has DryRun and Dispatched terminal success branches plus InnerError and SafeDescriptor safety branches; run_workstation_dispatch_with_contract_and_trace builds the scoped task brief, outcome_to_response_fields projects runner_status=workstation_dispatch_v0, workstation_dispatch_status=dry_run_no_dispatch|dispatched|inner_returned_error, target_tool=mission_task_delegate, task_brief_preview, inner_result, and delegated_board_task_id. workstation_dispatch/descriptor.rs owns the task-contract v1 Lisp projection boundary: ParsedTaskContract, TaskContractParseError, parse_task_contract, load_task_contract, and resolve_contract_path_public. workstation_dispatch/decision.rs owns the entry decision gate: WorkstationDispatchSource, DispatchDecision, InferenceContext, INFERABLE_DISPATCH_STRATEGIES, opt_in_requested, explicit_workstation_dispatch_flag, and evaluate_dispatch_decision. extract_inner_board_task_id is the stable adapter from mission_task_delegate inner_result to request-flow smoke output. The --execute-dry-run audit must reach this substrate and return dry_run_no_dispatch rather than bridge_only; the --execute-real-dispatch audit must return dispatched with delegated_board_task_id without synchronously waiting for the worker.")
