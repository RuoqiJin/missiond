    ...
    (master-decision-loop
      ...
      :core
        ((step s1 :logic "probe codex MCP server readiness from codex mcp list and unattended approval readiness from ~/.codex/config.toml tool approval_mode entries")
         ...
         (step s2c2 :logic "require MissionD MCP narrowly: active objectives may use only the MCP surfaces needed for that BoardTask. do not call mission_kb_query, mission_conversation_query, provider-log tools, mission_intent, or mission_convergence_status unless the active task explicitly opts them in. A decision of create/update BoardTask or close_or_backfill MUST execute the matching MissionD MCP mutation such as mission_board_note_add, mission_board_update, or mission_board_create before the resident slot returns its final decision; if mutation is unavailable, return blocked")
