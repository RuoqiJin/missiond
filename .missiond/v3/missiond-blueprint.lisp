...
(missiond-blueprint
    ...
    (master-decision-loop
      ...
      :core
        ((step s1 :logic "probe codex MCP server readiness from codex mcp list and unattended approval readiness from ~/.codex/config.toml tool approval_mode entries")
         ...
         (step s9 :logic "defer long work to BoardTask/Autopilot; resident master heartbeat must not inspect provider durable logs, KB, Board backlog, historical conversation, or recent commits unless an explicit active BoardTask or manual mission_nightly_evolution workflow opts in"))
  ...
  (project-maturity-registry
    ...
    (maturity :id jarvis :current M9 :target M10 :gap [final-convergence])
    (maturity :id jarvis-forge :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id jarvis-mechanic :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id xjpcode :current M9 :target M10 :gap [final-convergence])
    (maturity :id neural-codegen :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id semantic-terminal :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id xiaojinpro-backend :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id deploy-center :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id deploy-agent :current M9 :target M10 :gap [final-convergence])
    (maturity :id auth :current M9 :target M10 :gap [final-convergence])
    (maturity :id router :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id payments :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    ...
    (maturity :id pcea :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id secret-store :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id xiaojin-blog :current M7 :target M10 :gap [event-bus commit-backfill worker-operational final-convergence])
    (maturity :id cuthub :current M9 :target M10 :gap [final-convergence]))
