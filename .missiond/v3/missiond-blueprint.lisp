...
(missiond-blueprint
  ...
  (implementation-map
    ...
    (surface mission_workflow
      ...
      :note "distill dry_run emits workflow-draft Lisp; sonnet distiller requires JSON workflow_sexp + object match_rules and validates balanced workflow_sexp before persisting a Workflow row. distill persist+write_file writes an enriched V3 workflow artifact with :workflow_id, :source_plans, :match_rules, :steps, :status, and :body workflow_sexp under ArtifactKind::Workflow at .missiond/workflows/<topic>.lisp, preserving partial-on-file-failure semantics. compile_methodology reads methodology Lisp from .missiond/workflows/<name>.lisp or workflow_path, then deterministic mode validates Lisp and emits executable YAML through the same workflow surface; its write_file path still canonicalizes methodology source because that branch has no workflow_id row. run_methodology dispatches that compiled YAML. Review gates remain receipt-only, and auto_sonnet_policy={off|safe_after_rules|dry_run} is a closed enum.")
    ...
    (surface task-runner-cli
      ...
      :note "Current task-runner code keeps the task-scoped compatibility lifecycle ledger while task-runner-append-event can also project each append into request-local one-event files at .missiond/requests/<request_id>/events/<seq>.event.lisp when request-id/events-dir are supplied. task-runner-dispatch and task-runner-submit-dispatch pass request-local lifecycle projection args through their real dispatch-event paths, via task-runner-next-action, instead of leaving them as append-helper-only knobs. task-runner-finalize-report can project the same finalized lineage into the V3 request-local final-report artifact at .missiond/requests/<request_id>/reports/final.lisp when request-id/request-reports-dir are supplied; the legacy report-contract remains the compatibility report. task-runner-append-event is the only cooperative mutation helper for task-lifecycle-event-log: it uses a sibling lock, rereads under lock, validates the candidate ledger and optional request-local lifecycle-event file, then atomically renames/creates outputs. task-runner-next-action prioritizes finalize_report before dispatch_task and emits dispatch events through appendLifecycleEvent. task-runner-parent-hotfix is read-only by default and projects parent patches through task-runner-finalize-report, preserving worker commit as :agent_commit_hash while :commit_hash/:final_commit_hash/:verified_commit_hash move to the final commit, and can also write the V3 request-local final-report projection. project-task-lifecycle-ledger backfills shared-memory/session-trace compatibility facts from lifecycle events. verify-task-runner-batch imports lifecycle validation, append-event, and parent-hotfix projections so batch smoke covers the lifecycle/final-report/receipt path.")
