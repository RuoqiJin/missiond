    ...
    :invariants
      ["code and research dynamic slots MUST NOT hardcode --model sonnet"
       ...
       "model=\"default\" and model_profile=coding-default-opus-4-7 both mean no CLI --model override"
	       ...
	       "Autopilot MUST honor explicit engine_hint/pool_hint when a matching V3 workstation-pool candidate is available. If it must fall back to a non-matching worker, it MUST record a durable reroute_reason as a BoardTask note before dispatching so the operator can see why the requested engine/pool was not used."
    ...
    (master-decision-loop
      ...
      :core
        ((step s1 :logic "probe codex MCP server readiness from codex mcp list and unattended approval readiness from ~/.codex/config.toml tool approval_mode entries")
         ...
         (step s2b :logic "build an architectural SSOT review prompt that asks what is logically inconsistent, missing, or optimizable from Lisp/Board/KB/event/provider durable evidence; keep the prompt open-ended for the resident agent while hard workflow contracts stay enforced by MissionD runtime, workflow.lisp, Board schema, and checkers")
         (step s2c :logic "require MissionD MCP first (mission_intent, mission_board_query, mission_conversation_query, mission_kb_query, mission_convergence_status, mission_slots), query the active BoardTask by id before deciding, treat PTY recognition as diagnostic only, avoid recursive delegation for BoardTasks that already contain ## Swarm metadata or task_class=context-pack/code, and only use mission_swarm_run or mission_task_delegate when the next shard is concrete enough")
