(codex-boot-context
    ...
    ((layer L0-always-on
       :rules
         ["Lisp is the SSOT: change Lisp first, then make code/checkers/runtime isomorphic."
          ...
          "For Codex App sessions that are not under MissionD-controlled prompt/tool injection, run node scripts/mission-context-pack.mjs --json --message \"$CODEX_USER_REQUEST\" before broad repo search; it defaults to live mission_context_boot/mission_context_gather delegation when runtime is reachable, uses --offline for deterministic fallback only, and exposes required_tool_sequence/navigation_profile.recommended_tool_sequence so mission_context_gather, mission_memory evidence_search, and mission_repo_search precede ad-hoc shell rg."
          "When the user names a project, service, site, domain, URL, or repo-like phrase and no explicit project_id is already known, call mission_project resolve first; use its matched_project_id for KB, Board, conversations, SSOT, and worker dispatch, and treat unregistered_candidate as a registration workflow rather than a dead end."
  ...
  :required-tools [mission_context_boot mission_context_gather mission_repo_search mission_board mission_workflow mission_task_delegate mission_shared_memory mission_tool_directory]
  :forbidden-content [secret-values raw-provider-logs bulk-chat-history unreviewed-kb-preload]
  :handoff-card
    "Use MissionD as the control plane. If the user names a project, service, site, domain, URL, or repo-like phrase, call mission_project resolve before KB/Board/conversation/worker calls. If you need missing facts, call mission_context_gather. If you need repo text lookup, call mission_repo_search so profile/lane gates run before text matching. For deployed runtime facts, trust runtime_environment and its canonical Jarvis monitor endpoints before repo-local runtime files. Do not guess monitor ports. If you need to change code, ensure intent.lisp/plan.lisp/accepted_shard/write_scope exist and commit through the work-order gate. Never treat PTY text as completion authority.")
