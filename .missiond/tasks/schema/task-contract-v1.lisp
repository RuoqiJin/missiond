...
(task-contract-schema missiond.task-contract.v1
  ...
  (field-contract
    ...
    (:context-atlas-path "OPTIONAL repo-relative path to a machine-readable code/navigation atlas. Used to reduce broad grep/offset exploration before implementation; checker rejects absolute paths and '..' traversal.")
    (:pattern-card-path "OPTIONAL repo-relative path to an implementation pattern card. Used to tell the worker which established checker/CLI/renderer/smoke pattern to follow; checker rejects absolute paths and '..' traversal.")
  ...
  (renderer-contract
    ...
    :machine-context-rendered
      ["task :kind / :status / :owner"
       ...
       ":context-atlas-path / :pattern-card-path when present"
    ...
    :dispatch-efficiency-rendering
      ["thin mode preserves task-specific Goal / Ownership / Must Not Touch / Requirements / Acceptance / Commit / Report sections but omits repeated Shared Memory / Report Contract / Session Trace / Router Policy boilerplate"
       ...
       "when a task carries context atlas / pattern card metadata, thin and full modes render a Context Navigation section before Goal so workers read navigation anchors before broad repository search"
