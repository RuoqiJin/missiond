...
(report wave36-01-mission-request-review-response-v0
  ...
  :acceptance_results
    [(result :command "cargo test -p missiond-daemon handlers::knowledge::request::tests"
             :exit_code 0
             :ok true
             :note "65 passed; 0 failed; 0 ignored. Adds 25 new tests covering parse_respond_decision (response/decision aliases, missing/unknown errors), RespondDecision classification (requires_directive_ref / requires_plan_ref / record_only), extract_lisp_keyword_string|int helpers, resolve_directive_ref/plan_ref (explicit-arg-wins, artifact fallback, blocked-when-missing), next_event_seq seq+1 / max+1 / unrelated-name filtering, event_path_for_seq six-digit padding, build_review_event_lisp shape (dispatched approve_intent, blocked execute_plan with no refs, recorded reject_plan), next_action_for vocabulary, and parse_event_seq_from_filename strictness.")
     ...
     (result :command "cargo check -p missiond-daemon"
             :exit_code 0
             :ok true
             :note "Builds clean. Net additions are pure helpers (RespondDecision enum, parse_respond_decision, extract_lisp_keyword_string/int, resolve_directive_ref/plan_ref, next_event_seq, event_path_for_seq, build_review_event_lisp, next_action_for) plus action_respond which calls super::directive::handle / super::plan::handle / super::unified_entry::run_pipeline without modifying any of those files.")
     ...
     (result :command "node scripts/verify-task-contract.mjs .missiond/tasks/wave36/wave36-01-mission-request-review-response-v0.lisp"
             :exit_code 0
             :ok true
             :note "task-contract verify OK against 37421f4ae3af — commit message matches contract :commit :message and all changed files lie inside :write-scope.")]
  :notes
