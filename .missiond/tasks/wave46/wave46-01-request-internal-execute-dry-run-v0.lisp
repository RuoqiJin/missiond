;; Wave 46 task contract.

(task wave46-01-request-internal-execute-dry-run-v0
  :schema "missiond.task-contract.v1"
  :title "v3 request internal execute dry-run v0"
  :kind code-alignment
  :status ready
  :owner "claudecode"
  :depends-on []
  ...
  :context-atlas-path ".missiond/tasks/wave46/context-atlas.lisp"
  :pattern-card-path ".missiond/tasks/wave46/pattern-cards.lisp"
  ...
  :requirements
    ["Update the V3 blueprint first. Refine (execute-dry-run-smoke ...) so its audit path explicitly declares execute_mode=internal, dispatch_strategy=agent-team, dry_run=true, target=mission_task_delegate, and a workstation-dispatch dry-run no-dispatch proof."
     "Extend scripts/check-v3-request-flow-smoke.mjs so --execute-dry-run sends execute_mode='internal' and dispatch_strategy='agent-team' on the execute_plan respond call. Do not alter default --live-ipc behavior; it must still stop at awaiting_execution and must not call execute_plan."
     "In --execute-dry-run mode, assert the existing review-level invariants from wave45 still hold: respond outcome=dispatched, inner_action=unified_entry::plan_execute, respond_result.execute=true, review_packet.state=execute_requested, allowed_responses=[observe], and a request-local execute_plan event is appended."
     ...
     "Keep wave44 request-local cleanup guarantees: default flow omits compat_write_file, compat_write_audit remains clean, and --cleanup removes only .missiond/requests/<request_id>/."
     "Only touch Rust/MCP if the live internal dry-run reveals a real forwarding/schema bug. Parent probe before dispatch already observed the current daemon returns status=dry_run, runner_status=workstation_dispatch_v0, workstation_dispatch_status=dry_run_no_dispatch when execute_mode=internal + dry_run=true + dispatch_strategy=agent-team."
  ...
  :commit
    (:required true
     :message "feat(v3): require internal execute dry-run smoke"
