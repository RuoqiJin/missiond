# wave23-07-router-policy-draft-from-trace-v0 — Router policy draft from trace v0

> Generated from MissionD task-contract v1.
> Source: `.missiond/tasks/wave23/wave23-07-router-policy-draft-from-trace-v0.lisp`
...
- report_contract: `.missiond/tasks/wave23/reports/wave23-07-router-policy-draft-from-trace-v0.report.lisp`

## Goal

Codex-owned architecture task: add an architecture-only draft for future LLM router policy derived from session traces, without replacing ClaudeCode or changing runtime dispatch.
...
1. Do not delegate this blueprint task to ClaudeCode; Codex owns the architecture edit.
2. Define trace-derived router policy as architecture-designed only.
3. Describe backend classes: claudecode, missiond-llm-router, deterministic-checker, patch-worker, verifier-worker.
4. Record that Wave23 only collects/summarizes trace; it does not replace ClaudeCode.
5. Preserve source-index invariants and do not mark runtime router code-aligned.
...
Expected machine-readable report: `.missiond/tasks/wave23/reports/wave23-07-router-policy-draft-from-trace-v0.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave23/reports/wave23-07-router-policy-draft-from-trace-v0.report.lisp
```
...
git add ".missiond/v2/intent-machine-contract.lisp" \
        ".missiond/v2/intent-workstation-policy.lisp" \
        ".missiond/v2/intent-pillar-source-index.lisp"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave23/wave23-07-router-policy-draft-from-trace-v0.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave23/wave23-07-router-policy-draft-from-trace-v0.lisp \
  git commit -m "docs(v2): draft trace-derived router policy"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave23/wave23-07-router-policy-draft-from-trace-v0.lisp
```
