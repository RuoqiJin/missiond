# wave25-06-lisp-backfill-router-measurement-status — Lisp backfill router measurement status

> Generated from MissionD task-contract v1.
> Source: `.missiond/tasks/wave25/wave25-06-lisp-backfill-router-measurement-status.lisp`
...
- depends_on: `wave25-05-router-policy-measurement-smoke-v1`
- shared_memory: `.missiond/tasks/wave25/shared-memory.lisp`
- report_contract: `.missiond/tasks/wave25/reports/wave25-06-lisp-backfill-router-measurement-status.report.lisp`
- session_trace: `.missiond/tasks/wave25/session-trace.lisp`
- session_trace_writable: `false`

## Goal

Backfill v2 Lisp blueprints with Wave25 router measurement loop facts after the code tasks are committed. This task is Codex-owned; do not send it to ClaudeCode unless explicitly redirected.
...
1. Backfill only committed Wave25 facts; do not speculate beyond reports.
2. Mark evaluator/report fields/trace-index confidence as code-aligned or code-aligned-partial according to actual reports.
3. Keep runtime router replacement pending unless a future task actually changes runtime dispatch.
4. Add source-index entries for each new measurable router loop artifact.
...
Expected machine-readable report: `.missiond/tasks/wave25/reports/wave25-06-lisp-backfill-router-measurement-status.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave25/reports/wave25-06-lisp-backfill-router-measurement-status.report.lisp
```
...
git add ".missiond/v2/intent-machine-contract.lisp" \
        ...
        ".missiond/v2/intent-tools.lisp" \
        ".missiond/v2/intent.lisp"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave25/wave25-06-lisp-backfill-router-measurement-status.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave25/wave25-06-lisp-backfill-router-measurement-status.lisp \
  git commit -m "docs(v2): backfill wave25 router measurement status"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave25/wave25-06-lisp-backfill-router-measurement-status.lisp
```
