# wave27-07-lisp-backfill-router-dispatch-descriptor-status — Lisp backfill router dispatch descriptor status

> Generated from MissionD task-contract v1.
> Source: `.missiond/tasks/wave27/wave27-07-lisp-backfill-router-dispatch-descriptor-status.lisp`
...
- depends_on: `wave27-06-router-dispatch-descriptor-smoke-v0`
- shared_memory: `.missiond/tasks/wave27/shared-memory.lisp`
- report_contract: `.missiond/tasks/wave27/reports/wave27-07-lisp-backfill-router-dispatch-descriptor-status.report.lisp`
- session_trace: `.missiond/tasks/wave27/session-trace.lisp`
- session_trace_writable: `false`

## Goal

Backfill v2 Lisp blueprints with Wave27 router dispatch descriptor facts after all code tasks are committed. This task is Codex-owned; do not send it to ClaudeCode unless explicitly redirected.
...
1. Backfill only committed Wave27 facts; do not speculate beyond reports.
2. Mark descriptor schema/checker, descriptor CLI, mission_plan descriptor surface, report fields, renderer context, and smoke according to actual reports.
3. Keep runtime router replacement pending unless a future task actually changes runtime dispatch.
4. Add source-index entries for each new router dispatch descriptor artifact and status-upgrade entries for trace-derived-router-policy.
...
Expected machine-readable report: `.missiond/tasks/wave27/reports/wave27-07-lisp-backfill-router-dispatch-descriptor-status.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave27/reports/wave27-07-lisp-backfill-router-dispatch-descriptor-status.report.lisp
```
...
```bash
node scripts/recommend-task-backend.mjs --task .missiond/tasks/wave27/wave27-07-lisp-backfill-router-dispatch-descriptor-status.lisp --policy .missiond/router/router-policy-v1.lisp --backend-registry .missiond/router/router-backend-registry-v1.lisp --json
```
...
git add ".missiond/v2/intent-machine-contract.lisp" \
        ...
        ".missiond/v2/intent-tools.lisp" \
        ".missiond/v2/intent.lisp"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave27/wave27-07-lisp-backfill-router-dispatch-descriptor-status.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave27/wave27-07-lisp-backfill-router-dispatch-descriptor-status.lisp \
  git commit -m "docs(v2): backfill wave27 router dispatch descriptor status"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave27/wave27-07-lisp-backfill-router-dispatch-descriptor-status.lisp
```
