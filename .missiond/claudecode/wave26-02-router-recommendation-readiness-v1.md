# wave26-02-router-recommendation-readiness-v1 — Router recommendation readiness annotations v1
...
- report_contract: `.missiond/tasks/wave26/reports/wave26-02-router-recommendation-readiness-v1.report.lisp`
- session_trace: `.missiond/tasks/wave26/session-trace.lisp`
- session_trace_writable: `true`

## Goal

Teach the Node recommendation and corpus-evaluation CLIs to consume the backend readiness registry and report apply eligibility and blockers while staying advisory-only.
...
1. Add optional --backend-registry <path> to recommend-task-backend.mjs and evaluate-router-policy-corpus.mjs.
2. When supplied, parse the registry in-process; do not spawn check-router-backend-registry.mjs.
3. Recommendation output should include backend_readiness_status, backend_runtime_allowed, router_apply_eligible, router_apply_blockers, and backend_registry_path when a registry was supplied.
4. router_apply_eligible must be true only when policy is valid, recommendation status is computed, confidence is high, registry backend exists, backend runtime_allowed=true, readiness_status=runtime-ready, and apply_blockers is empty.
5. With the seed registry, recommendations should remain router_apply_eligible=false for non-runtime-ready backends.
6. Corpus evaluator should aggregate by_backend_readiness and apply_eligible_count without changing the existing top-level keys' meanings.
7. Without --backend-registry, outputs remain backward-compatible except for additive fields only if a registry is explicitly supplied.
...
Expected machine-readable report: `.missiond/tasks/wave26/reports/wave26-02-router-recommendation-readiness-v1.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave26/reports/wave26-02-router-recommendation-readiness-v1.report.lisp
```
...
```bash
node scripts/recommend-task-backend.mjs --task .missiond/tasks/wave26/wave26-02-router-recommendation-readiness-v1.lisp --policy .missiond/router/router-policy-v1.lisp --json
```
...
git add "scripts/recommend-task-backend.mjs" \
        "scripts/evaluate-router-policy-corpus.mjs"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave26/wave26-02-router-recommendation-readiness-v1.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave26/wave26-02-router-recommendation-readiness-v1.lisp \
  git commit -m "feat(router): annotate recommendations with backend readiness"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave26/wave26-02-router-recommendation-readiness-v1.lisp
```
