# wave24-04-plan-router-dry-run-surface-v0 — mission_plan router dry-run surface v0
...
- depends_on: `wave24-03-router-recommendation-cli-v0`
- shared_memory: `.missiond/tasks/wave24/shared-memory.lisp`
- report_contract: `.missiond/tasks/wave24/reports/wave24-04-plan-router-dry-run-surface-v0.report.lisp`
- session_trace: `.missiond/tasks/wave24/session-trace.lisp`
- session_trace_writable: `true`

## Goal

Expose a dry-run-only router recommendation block on mission_plan(action=execute) so callers can see future backend selection without changing actual dispatch.
...
1. Add optional args router_policy_mode and router_policy_path or equivalent minimal names to mission_plan execute schema.
2. Only support off/default and dry_run in this task; any apply/auto mode must return INVALID_PARAM.
3. Dry-run block must be informational and must not alter target, dispatch_strategy, workstation_dispatch, auto_spawn, or evidence side effects.
4. Implement recommendation with a pure Rust deterministic helper using existing execute args; do not spawn Node or call scripts.
5. Response block should include status, recommended_backend, confidence, reasons, policy_source, and applied=false.
...
Expected machine-readable report: `.missiond/tasks/wave24/reports/wave24-04-plan-router-dry-run-surface-v0.report.lisp` (schema `missiond.report-contract.v1`).
...
git add "crates/missiond-daemon/src/handlers/knowledge/plan.rs" \
        "crates/missiond-mcp/src/tools/knowledge/plan.rs"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave24/wave24-04-plan-router-dry-run-surface-v0.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave24/wave24-04-plan-router-dry-run-surface-v0.lisp \
  git commit -m "feat(plan): expose router dry-run recommendation"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave24/wave24-04-plan-router-dry-run-surface-v0.lisp
```
