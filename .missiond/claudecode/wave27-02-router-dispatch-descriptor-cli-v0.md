# wave27-02-router-dispatch-descriptor-cli-v0 — Router dispatch descriptor CLI v0
...
- report_contract: `.missiond/tasks/wave27/reports/wave27-02-router-dispatch-descriptor-cli-v0.report.lisp`
- session_trace: `.missiond/tasks/wave27/session-trace.lisp`
- session_trace_writable: `true`

## Goal

Build a read-only CLI that turns a task contract plus router policy, optional trace index, and backend registry into a router dispatch descriptor. This CLI must not execute the descriptor or switch backend.
...
1. Create scripts/build-router-dispatch-descriptor.mjs as a new read-only CLI.
2. Consume --task, --policy, optional --trace-index, required --backend-registry for descriptor mode, plus --json and --dry-fixture.
3. Reuse exported helpers from recommend-task-backend.mjs and check-router-backend-registry.mjs in-process; do not duplicate policy matching logic.
4. Emit schema missiond.router-dispatch-descriptor.v1 and all fields required by wave27-01.
5. Descriptor must preserve router_apply_eligible exactly from the readiness-annotated recommendation, but still set runtime_replacement=false and no_execution=true.
6. When readiness is missing, malformed, or unknown backend, emit a valid descriptor with router_apply_eligible=false and explicit blockers.
7. Output must be deterministic with stable key order.
8. No shell-out, no child_process, no git, no HTTP/network, no LLM, no backend execution.
...
Expected machine-readable report: `.missiond/tasks/wave27/reports/wave27-02-router-dispatch-descriptor-cli-v0.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave27/reports/wave27-02-router-dispatch-descriptor-cli-v0.report.lisp
```
...
```bash
node scripts/recommend-task-backend.mjs --task .missiond/tasks/wave27/wave27-02-router-dispatch-descriptor-cli-v0.lisp --policy .missiond/router/router-policy-v1.lisp --backend-registry .missiond/router/router-backend-registry-v1.lisp --json
```
...
```bash
git add "scripts/build-router-dispatch-descriptor.mjs"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave27/wave27-02-router-dispatch-descriptor-cli-v0.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave27/wave27-02-router-dispatch-descriptor-cli-v0.lisp \
  git commit -m "feat(router): build dispatch descriptors"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave27/wave27-02-router-dispatch-descriptor-cli-v0.lisp
```
