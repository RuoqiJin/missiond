# wave48-02-context-dispatch-shard-plan-v0 — context-pack investigation: dispatch shard plan and integration risks

> Thin brief rendered from MissionD task-contract v1. Task Lisp remains the SSOT.
> Source: `.missiond/tasks/wave48/wave48-02-context-dispatch-shard-plan-v0.lisp`
...
- report_contract: `.missiond/tasks/wave48/reports/wave48-02-context-dispatch-shard-plan-v0.report.lisp`
...
2. Inspect only: task-runner dispatch scripts, V3 workstation-config/context-pack checkers, Autopilot dispatch code, and wave47 evidence around dynamic slot loss.
3. Use scripts/context-pack-append.mjs to append at least two observations to .missiond/tasks/wave48/context-pack.lisp. Each observation must cite concrete files and explain an implementation or integration risk.
4. Use scripts/context-pack-append.mjs to append at least one shard-proposal. The proposal must split a later code worker's write-scope away from the autopilot recovery shard if possible, or explicitly mark the hotspot as single-owner.
5. If you detect overlapping proposed write scopes, append a conflict entry instead of pretending the shards are independent.
6. Do not edit Rust, JS, package, or blueprint files in this task. This is an investigation/proposal shard only.
...
git add ".missiond/tasks/wave48/context-pack.lisp" \
        ...
        ".missiond/tasks/wave48/session-trace.lisp" \
        ".missiond/tasks/wave48/reports/wave48-02-context-dispatch-shard-plan-v0.report.lisp"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave48/wave48-02-context-dispatch-shard-plan-v0.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave48/wave48-02-context-dispatch-shard-plan-v0.lisp \
  git commit -m "chore(tasks): record wave48 dispatch shard context"
node scripts/verify-task-contract.mjs .missiond/tasks/wave48/wave48-02-context-dispatch-shard-plan-v0.lisp
```
