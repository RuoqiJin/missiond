# wave27-00-archive-wave26-artifacts — Archive Wave 26 artifacts
...
- depends_on: `wave26-07-lisp-backfill-router-readiness-status`
- shared_memory: `.missiond/tasks/wave27/shared-memory.lisp`
- report_contract: `.missiond/tasks/wave27/reports/wave27-00-archive-wave26-artifacts.report.lisp`
- session_trace: `.missiond/tasks/wave27/session-trace.lisp`
- session_trace_writable: `true`

## Goal

Archive all untracked Wave 26 task contracts, rendered ClaudeCode briefs, reports, shared-memory ledger, and session-trace ledger after Wave 26 code and Lisp commits are complete.
...
1. Stage only Wave 26 artifacts currently untracked.
...
4. It is acceptable that Wave26 Codex-owned Lisp backfill and coordination-index tasks have no report; call that out explicitly if true.
5. Do not stage Wave 27 task files or briefs.
...
Expected machine-readable report: `.missiond/tasks/wave27/reports/wave27-00-archive-wave26-artifacts.report.lisp` (schema `missiond.report-contract.v1`).
...
```bash
node scripts/check-task-report.mjs .missiond/tasks/wave27/reports/wave27-00-archive-wave26-artifacts.report.lisp
```
...
```bash
node scripts/recommend-task-backend.mjs --task .missiond/tasks/wave27/wave27-00-archive-wave26-artifacts.lisp --policy .missiond/router/router-policy-v1.lisp --backend-registry .missiond/router/router-backend-registry-v1.lisp --json
```
...
```bash
git add ".missiond/tasks/wave26/**" \
        ".missiond/claudecode/wave26-*.md"
node scripts/task-scope-guard.mjs --task .missiond/tasks/wave27/wave27-00-archive-wave26-artifacts.lisp --mode staged
MISSIOND_TASK_CONTRACT=.missiond/tasks/wave27/wave27-00-archive-wave26-artifacts.lisp \
  git commit -m "chore(wave26): archive router readiness artifacts"
```
...
```bash
node scripts/verify-task-contract.mjs .missiond/tasks/wave27/wave27-00-archive-wave26-artifacts.lisp
```
