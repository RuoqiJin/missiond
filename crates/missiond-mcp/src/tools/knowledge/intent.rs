    ...
    vec![
        ToolDefinition::new(
            ...
            "读取项目 intent.lisp 全景声明。调查/实现任务前 MUST 先调用此工具获取系统全景图，可节省 50% 搜索步骤。\
             read: 返回完整 intent.lisp; section: 按顶级 S-EXP 块名过滤; summary: 返回 design-constraints + module-catalog 核心段; list: 列出所有可用项目及其 intent 路径; paths: 返回所有 intent 文件路径供并行 Read 加载",
            json!({
                "type": "object",
                "properties": {
                    "action": {
