...
fn build_properties() -> Value {
    ...
    p.insert("write_file".into(), prop(
        "boolean",
        "[distill persist=true | compile_methodology persist=true] (wave-14/35/38 file-first SSOT) write `<project_root>/.missiond/workflows/<topic>.lisp` after the DB row / YAML is committed. Both branches now publish the same enriched V3 workflow artifact carrying :workflow_id/:source_plans/:match_rules/:steps/:status plus :body workflow_sexp. distill stamps :workflow_id with the persisted Workflow row UUID. compile_methodology has no Workflow DB row (no workflow_id column, no schema migration) and therefore stamps :workflow_id with the deterministic generated flow_id, leaves :source_plans empty, and packs source_kind=\"methodology\" / compiler / compiler_version / source_hash / flow_id / source_path / generated_at into :match_rules so reviewers can correlate the .lisp artifact with the generated YAML. The methodology body is preserved verbatim under :body — write_file no longer publishes the raw methodology source as the on-disk artifact. Default false. Topic precedence: explicit `topic` > distill's `name` / compile_methodology's source stem. DB / YAML stay committed even on file failure — response surfaces status=\"partial\" + file_write_error.",
    ));
