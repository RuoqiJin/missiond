...
fn build_properties() -> Value {
    ...
    p.insert("compiler_mode".into(), prop_enum(
        "string",
        "[compile] dry_run (default, no LLM) emits an executable PLAN scaffold with :target / :objective hints; sonnet asks the plan-compiler actor (Sonnet) to emit a PLAN sexp anchored to board_task_id. See intent-intent-layer.lisp :: role plan-compiler.",
        &["dry_run", "sonnet"],
    ));
    ...
    // wave-26 / task 03 — OPTIONAL backend-readiness registry path for
    // surfacing apply-eligibility on the dry-run recommendation block.
    // Mirrors the wave26-02 Node-side `--backend-registry` flag's six
    // additive fields. Pure-Rust read + minimal Lisp parser; the daemon
    // NEVER spawns Node and NEVER calls scripts/check-router-backend-registry.mjs.
    // Off/default mode is byte-identical with NO file I/O even when this
    // arg AND `router_policy_trace_index_path` are both supplied.
    p.insert("router_backend_registry_path".into(), prop(
        "string",
