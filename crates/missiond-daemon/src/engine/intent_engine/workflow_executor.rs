...
impl AppState {
    ...
    pub(crate) fn execute_workflow<'a>(
        ...
        Box<dyn std::future::Future<Output = Result<missiond_core::WorkflowResult>> + Send + 'a>,
    > {
        ...
        Box::pin(async move {
            use missiond_core::{
                parse_workflow_blocks, resolve_vars, WorkflowResult, WorkflowStepPreview,
                WorkflowStepResult,
            };
            ...
            if depth > MAX_DEPTH {
                return Err(anyhow!(
                    "Workflow recursion depth exceeded (max {}). Skill '{}' action '{}'",
                    MAX_DEPTH,
                    skill_name,
                    action_id
                ));
            ...
            if !dry_run {
                if let Ok(true) = self
                    .store
                    .skill_execution_is_running(skill_name, action_id)
                    .await
                {
                    return Err(anyhow!(
                        "Action '{}' on skill '{}' is already running",
                        action_id,
                        skill_name
                    ));
                }
            ...
            let actions: Vec<missiond_core::SkillAction> =
            ...
            if let Some(ref hooks_json) = topic.context_hooks_json {
                if let Ok(hooks) =
                    serde_json::from_str::<Vec<missiond_core::ContextHook>>(hooks_json)
                {
                    for hook in &hooks {
                        ...
                        match hook_result {
                            Ok(result) => {
                                let output = result
                                    ...
                                    .first()
                                    .map(|c| match c {
                                        missiond_mcp::ToolContent::Text { text } => text.clone(),
                                    })
                                    .unwrap_or_default();
            ...
            let mut i = 0usize;
            let mut visit_counts: std::collections::HashMap<usize, u32> =
                std::collections::HashMap::new();
            ...
            while i < workflow.steps.len() {
                ...
                *visits += 1;
                if *visits > MAX_STEP_VISITS {
                    ...
                    let err_msg = format!(
                        "Step {} ('{}') visited {} times — infinite loop detected",
                        i, step.tool, visits
                    );
                    warn!(%err_msg);
                    let _ = self
                        .store
                        .skill_execution_update_with_duration(
                            &exec_id,
                            "failed",
                            (i + 1) as i32,
                ...
                let _ = self
                    .store
                    .skill_execution_update(&exec_id, "running", (i + 1) as i32, None, None)
                    .await;
                ...
                if is_error {
                    ...
                    match on_error {
                        ...
                        "retry" => {
                            ...
                            for attempt in 1..=max {
                                ...
                                if !retry_result.is_error.unwrap_or(false) {
                                    let retry_output = retry_result
                                        .content
                                        .first()
                                        .map(|c| match c {
                                            missiond_mcp::ToolContent::Text { text } => {
                                                text.clone()
                                            }
                                        })
                                        .unwrap_or_default();
                        ...
                        s if s.starts_with("fallback:") => {
                            ...
                            if let Some(target_idx) = workflow
                                .steps
                                .iter()
                                .position(|st| st.id.as_deref() == Some(target_id))
                            {
