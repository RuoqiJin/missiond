...
mod tests {
    ...
    fn master_context_pack_records_architectural_prompt_contract() {
        ...
        assert!(rendered.contains(":active-objective-contract [intent-intake-grounding unknowns-first-intake intent-inference intent-memory-capture boardtask-description-overrides-default-self-review read-only-declared-project-roots mutation-before-decision-return]"));
