...
// ─────────────────────────────────────────────────────────────────────
// mission_incident remediation actions (F-incident-reaction).
//
// `test`/`list` are inline above; `get`/`remediate`/`status`/`close`
// implement the playbook surface — read incident state and link
// remediation board tasks. Severity-aware triage is delegated to
// `aiops::triage_incident` (the same code the bus reactor runs) so
...
async fn incident_remediate(state: &AppState, args: Value) -> Result<ToolResult> {
    // Mode A: existing incident — ensure a remediation board task is linked.
    if let Some(id) = args.get("id").and_then(|v| v.as_str()).filter(|s| !s.is_empty()) {
        ...
        // No linked task yet — re-build a MissionIncident from the row
        // and run the same triage pipeline as the bus reactor.
        ...
        if let Some(ref tid) = board_task_id {
            // The triage path may have written a new incident row; back-link the
            // *original* incident so callers find the same task either way.
            let _ = state.store.update_incident_board_task_id(&row.id, tid).await;
        }
        let status = if board_task_id.is_some() { "linked" } else { "no_task" };
        return Ok(ToolResult::json_pretty(&json!({
            "incident_id": row.id,
    ...
    // Mode B: synthesize a new incident from severity/title/source/server_id.
    let title = args
        .get("title")
        .and_then(|v| v.as_str())
        .filter(|s| !s.is_empty())
    ...
    let (board_task, recent_notes) = match row.board_task_id.as_deref() {
        Some(tid) => {
            ...
            let notes = state
                ...
                .await
                .unwrap_or_default();
            // Cap to last 5 notes to keep response evidence-first.
            let mut notes = notes;
            if notes.len() > 5 {
...
async fn incident_close(state: &AppState, args: Value) -> Result<ToolResult> {
    ...
    if let Some(ref tid) = board_task_id {
        ...
        // Safe to close: mark done + add audit note.
        let update = missiond_core::types::UpdateBoardTaskInput {
