...
pub(super) async fn compute_workstation_proposal_bundle(
...
) -> Option<super::super::workstation_dispatch::WorkstationProposalBundle> {
    ...
    let gate = super::super::workstation_dispatch::WorkstationProposalGate {
    ...
    if gate.is_fully_silent() {
        ...
        Some(
            super::super::workstation_dispatch::request_workstation_proposals(
                state,
                &plan.sexp_text,
                plan.compiled_from.as_deref(),
            )
        ...
        Some(
            super::super::workstation_dispatch::WorkstationProposalBundle::plan_hints_present(
                gate.signal_summary(),
            ),
        )
    }
...
/// reads separately).
pub(super) fn plan_hints_carry_workstation_signal(h: &ParsedPlanHints) -> bool {
...
pub(super) fn attach_workstation_proposals_block(
    mut result: ToolResult,
    bundle: Option<&super::super::workstation_dispatch::WorkstationProposalBundle>,
) -> ToolResult {
...
pub(super) async fn compute_workstation_auto_spawn_gate(
    state: &AppState,
    input: &super::super::workstation_dispatch::WorkstationAutoSpawnInput,
    plan: &Plan,
    hints: &ParsedPlanHints,
    bundle: Option<&super::super::workstation_dispatch::WorkstationProposalBundle>,
) -> Option<super::super::workstation_dispatch::WorkstationAutoSpawnGateOutcome> {
    if !input.auto_spawn {
    ...
    let (parsed_contract, contract_load_error): (
        Option<super::super::workstation_dispatch::ParsedTaskContract>,
        Option<String>,
        ...
        let resolved =
            super::super::workstation_dispatch::resolve_contract_path_public(raw_path, &cwd);
    ...
    let mut outcome = super::super::workstation_dispatch::evaluate_workstation_auto_spawn_gate(
        &input,
        bundle,
    ...
    if outcome.status.was_spawned() {
        ...
        let substrate_outcome =
            super::super::workstation_dispatch::run_workstation_dispatch_with_contract(
                state,
                plan,
        ...
        match substrate_outcome {
            ...
            | super::super::workstation_dispatch::WorkstationDispatchOutcome::DryRun { .. } => {
            ...
            super::super::workstation_dispatch::WorkstationDispatchOutcome::SafeDescriptor {
                ...
                outcome.status =
                    super::super::workstation_dispatch::WorkstationAutoSpawnStatus::SkippedSubstrateRefused;
            ...
            super::super::workstation_dispatch::WorkstationDispatchOutcome::InnerError {
                ...
                outcome.status =
                    super::super::workstation_dispatch::WorkstationAutoSpawnStatus::SkippedSubstrateInnerError;
                outcome.substrate_reason = Some(detail);
            }
        }
    }
...
pub(super) fn attach_workstation_auto_spawn_gate_block(
    mut result: ToolResult,
    outcome: Option<&super::super::workstation_dispatch::WorkstationAutoSpawnGateOutcome>,
) -> ToolResult {
...
/// inference metadata.
pub(super) fn attach_inference_block(mut result: ToolResult, block: Option<Value>) -> ToolResult {
...
pub(super) fn action_execute_bridge(plan: &Plan, resolved: &ResolvedExec) -> ToolResult {
...
pub(super) async fn action_execute_internal(
    ...
    let inference_ctx = super::super::workstation_dispatch::InferenceContext {
    ...
    let dispatch_decision = super::super::workstation_dispatch::evaluate_dispatch_decision(
        args,
    ...
    let inner_result = match resolved.target {
        "mission_execution" => {
            super::super::agent_execution::handle(state, "mission_execution", inner_args.clone())
                .await?
        }
        "mission_task_delegate" => {
            super::super::super::compute::task_delegate::handle(
                state,
                "mission_task_delegate",
                inner_args.clone(),
            )
        ...
        "mission_flow_run" => {
            super::super::super::compute::flow_run::handle(
                state,
                "mission_flow_run",
                inner_args.clone(),
            )
    ...
    .await;
    if let super::super::evidence_collector::AppendOutcome::Failed { error } = &outcome {
