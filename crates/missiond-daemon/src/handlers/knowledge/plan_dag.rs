...
pub(super) fn build_rollback_descriptor(node: &DagNode) -> RollbackDescriptor {
    ...
    let objective = node
        ...
        .filter(|s| !s.is_empty())
        .map(|s| s.to_string());
    let owned_files = super::plan::split_lisp_string_list(node.rollback_owned_files_raw.as_deref());
    let acceptance_commands =
        super::plan::split_lisp_string_list(node.rollback_acceptance_commands_raw.as_deref());
    RollbackDescriptor {
        policy,
        objective,
        owned_files,
...
impl RollbackDescriptor {
    ...
    pub(super) fn safety_check_for_workstation(
        ...
        let strat = node
            ...
            .filter(|s| !s.is_empty())
            .unwrap_or("");
        if !super::workstation_dispatch::INFERABLE_DISPATCH_STRATEGIES.contains(&strat) {
            return Err(format!(
...
pub(super) fn evaluate_node_acceptance(
    ...
    let commands = super::plan::split_lisp_string_list(node.acceptance_commands_raw.as_deref());
    let evidence_keys =
        super::plan::split_lisp_string_list(node.acceptance_evidence_keys_raw.as_deref());
...
pub(super) fn derive_node_claim_scopes(
    ...
    let owned: Vec<String> = super::plan::split_lisp_string_list(node.owned_files_raw.as_deref())
        .into_iter()
...
async fn run_rollback(state: &AppState, plan: &Plan, node: &DagNode) -> RollbackEvaluation {
    let descriptor = build_rollback_descriptor(node);
    match descriptor.policy {
        RollbackPolicy::None => RollbackEvaluation {
            policy: RollbackPolicy::None,
        ...
        RollbackPolicy::Descriptor => {
            ...
            let preview = if descriptor.objective.is_some() {
                Some(truncate_rollback_brief_preview(
                    &super::workstation_dispatch::build_task_brief(plan, &hints, strategy),
                ))
            } else {
                None
            };
        ...
        RollbackPolicy::Workstation => {
            ...
            match outcome {
                ...
                super::workstation_dispatch::WorkstationDispatchOutcome::DryRun { task_brief } => {
                    ...
                    RollbackPolicy::Workstation => {
                        match descriptor.safety_check_for_workstation(n) {
                            ...
                            Ok(()) => {
                                ...
                                let outcome =
                                    super::workstation_dispatch::run_workstation_dispatch(
                                        state,
                                        plan,
                                        "mission_task_delegate",
                                        strategy,
...
pub(crate) async fn handle_review_resolved_plan_node_event(
    ...
    match decision {
        ...
        super::review_gate::ReviewDecision::Rejected => PlanNodeResumeListenerOutcome::KeptPaused {
            plan_id: id,
            node_id: node.id.clone(),
            decision: "rejected",
        },
