...
mod tests {
    ...
    fn deployment_event_unknowns_are_synthesized_from_summary() {
        let source = json!({
            ...
            "observed_candidates": {
                "candidate_count": 64,
                "deploy_center_candidate_count": 2,
                "authoritative_deploy_center_candidate_count": 0,
            ...
            "relay_diagnostics": {
                ...
                "next_actions": [{
                    "action": "inspect_deploy_event_relay_runtime_env",
                    "status": "required"
                }]
            }
        });
        ...
        let unknowns = vec![
            "Does MissionD see authoritative deploy-center.deploy_events for xjp-deploy-center?"
                .to_string(),
        ...
        assert_eq!(
            synthesized[1]
                .get("observed_candidates")
                .and_then(|value| value.get("webhook_ingest_probe_candidate_count"))
                .and_then(Value::as_u64),
            Some(2)
        );
        assert_eq!(
