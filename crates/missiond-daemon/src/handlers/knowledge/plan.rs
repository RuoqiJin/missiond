...
pub(super) use task_contract::{
    ...
    parse_task_contract_emit_mode, render_lisp_string_list,
    task_contract_inputs_from_hints, task_contract_inputs_from_hints_with_trace,
    DispatchContractMode, TaskContractEmissionRecord, TaskContractEmitMode,
};
#[cfg(test)]
pub(super) use task_contract::{
    build_task_contract_lisp, is_task_contract_eligible, render_command_for,
    task_contract_path, write_task_contract_under_root, TaskContractInputs,
};
...
use distill_chain::{
    ...
    derive_fallback_chain_id, distill_chain_requested, parse_distill_chain_id,
    json_shape_label, parse_distill_chain_mode, parse_distill_chain_name,
    CHAIN_STATUS_RECORDED, CHAIN_STATUS_RECORDED_DISTILL_WARNING,
    CHAIN_STATUS_RECORDED_WITH_DISTILL, CHAIN_STATUS_SKIPPED_NO_FINALIZATION,
...
async fn action_execute(state: &AppState, args: &Value) -> Result<ToolResult> {
    ...
    let final_result = task_runner_dry_run::attach_task_runner_block(
        final_result,
        task_runner_mode,
        args,
    );
...
"#
    ...
    fn task_runner_off_default_does_no_file_io() {
        ...
        let after = attach_task_runner_block(
            action_execute_bridge(&plan, &resolved),
            mode,
            &args,
        );
        ...
        let after2 = attach_task_runner_block(
            action_execute_bridge(&plan, &resolved),
            mode2,
            &args2,
        );
    ...
    fn task_runner_dry_run_topological_batches_correct() {
        ...
        let v = parse_payload(&result);
        let batches = v["task_runner"]["batches"].as_array().expect("batches array");
    ...
    fn task_runner_dry_run_malformed_manifest_emits_status_malformed() {
        let manifest = write_temp_manifest(
            "malformed",
            "(this-is-not-a-task-runner-manifest :foo bar)",
        );
        let args = json!({
            "task_runner_mode": "dry_run",
            "task_runner_manifest_path": manifest.to_str().unwrap(),
    ...
    fn task_runner_off_with_runner_args_and_router_args_does_no_file_io() {
        ...
        for p in [&bogus_policy, &bogus_trace, &bogus_registry, &bogus_manifest] {
