...
mod task_runner_dry_run {
    ...
    pub(super) fn parse_task_runner_mode(
        args: &Value,
    ) -> Result<TaskRunnerMode, ToolResult> {
    ...
    fn parse_manifest(input: &str) -> Result<Manifest, String> {
        ...
        let list = match form {
            ...
            _ => {
                return Err(
                    "expected (task-runner-manifest ...) at top level".to_string(),
                )
            }
        };
        let mut iter = list.into_iter();
        ...
        let productive_only = productive_only
            .ok_or_else(|| "missing :productive_only header field".to_string())?;
    ...
    fn parse_node_entry(items: &[Sexp]) -> Result<ManifestNode, String> {
        ...
        let dispatch_group = dispatch_group
            .ok_or_else(|| format!("node `{}` missing :dispatch_group", task_id))?;
                ...
                block.insert(
                    ...
                    Value::Array(
                        projection
                            ...
                            .map(|b| {
                                Value::Array(
                                    b.into_iter().map(Value::String).collect::<Vec<_>>(),
                                )
                            })
                            .collect(),
                    ),
    ...
    fn project_manifest(manifest: &Manifest) -> ManifestProjection {
        ...
        let mut remaining: BTreeSet<String> =
            sorted_nodes.iter().map(|n| n.task_id.clone()).collect();
        ...
        let overlap_diagnostics =
            collect_overlap_diagnostics(&sorted_nodes, &manifest.overlap_policy);
