...
use methodology::{
    atomic_write, build_generated_yaml, derive_flow_id, extract_methodology_lifted, extract_steps,
...
#[cfg(test)]
use methodology::{
    build_manual_review_prompt, match_form_keyword, parse_optional_form_id, phase_id_for_step,
    sanitize_id_token, unique_generated_yaml_temp_path, LocatedStep, MethodologyForm,
    MethodologyLifted, MethodologyPhase, MethodologyStep,
};
...
/// rule's `passed=true`.
fn evaluate_auto_trigger_safety_rules(ctx: &SafetyRuleContext<'_>) -> (Vec<SafetyRuleResult>, bool) {
...
async fn maybe_apply_distill_chain_layers(
    ...
    let trigger_mode = match parse_auto_chain_trigger(
        args.get("auto_chain_trigger").and_then(|v| v.as_str()),
    ) {
        Ok(m) => m,
        Err(msg) => {
...
";
    ...
    fn auto_chain_requested_default_false_and_explicit_true() {
        ...
        assert!(!auto_chain_requested(&serde_json::json!({"auto_chain": "yes"})));
        assert!(!auto_chain_requested(&serde_json::json!({"auto_chain": null})));
        assert!(!auto_chain_requested(&serde_json::json!({"auto_chain": false})));
        // Explicit true is honored.
        assert!(auto_chain_requested(&serde_json::json!({"auto_chain": true})));
    }
    ...
    fn attach_auto_chain_to_payload_no_chain_id_skips_top_level_id_shortcut() {
        ...
        attach_auto_chain_to_payload(&mut payload, block);
        assert_eq!(payload["auto_chain_status"], AUTO_CHAIN_STATUS_RESOLVE_FAILED);
    ...
    fn auto_chain_status_constants_pin_the_wire_form() {
        ...
        assert_eq!(AUTO_CHAIN_ID_SOURCE_DERIVED, "derived_from_workflow_context");
    ...
    fn parse_auto_chain_trigger_default_never_and_explicit_modes() {
        ...
        assert_eq!(parse_auto_chain_trigger(Some("")), Ok(AutoChainTrigger::Never));
        assert_eq!(
    ...
    fn evaluate_safety_rules_inner_error_blocks_trigger() {
        let inner = ToolResult::structured_error(ToolError::new(
            error_codes::INVALID_PARAM,
            "inner failed",
        ));
        let inner_payload = serde_json::json!({"distill_mode": "dry_run"});
    ...
    fn build_auto_trigger_block_emits_stable_shape() {
        let rules = render_safety_rule_results(&[SafetyRuleResult::pass(
            SAFETY_RULE_INNER_DISTILL_OK,
        )]);
        let block = build_auto_trigger_block(
            true,
    ...
    fn attach_auto_trigger_to_payload_splices_block_and_top_level_shortcuts() {
        ...
        assert_eq!(payload["auto_trigger_status"], AUTO_TRIGGER_STATUS_TRIGGERED);
    ...
    fn validate_auto_sonnet_args_accepts_missing_and_bool() {
        ...
        assert!(
            validate_auto_sonnet_args(&serde_json::json!({"auto_sonnet": true})).is_ok()
        );
        assert!(
            validate_auto_sonnet_args(&serde_json::json!({"auto_sonnet": false})).is_ok()
        );
        assert!(
            validate_auto_sonnet_args(&serde_json::json!({"auto_sonnet_approved": true}))
                .is_ok()
        );
    ...
    fn validate_auto_sonnet_args_rejects_string_typo() {
        ...
        let err = validate_auto_sonnet_args(&serde_json::json!({"auto_sonnet": "true"}))
            .unwrap_err();
        assert!(
    ...
    fn validate_auto_sonnet_args_rejects_number_typo() {
        let err = validate_auto_sonnet_args(&serde_json::json!({"auto_sonnet": 1}))
            .unwrap_err();
    ...
    fn validate_auto_sonnet_args_rejects_approved_string_typo() {
        let err = validate_auto_sonnet_args(
            &serde_json::json!({"auto_sonnet_approved": "yes"}),
        )
        .unwrap_err();
