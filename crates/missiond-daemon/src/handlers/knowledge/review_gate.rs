...
fn inspect_safety_rules(
    ...
    if ctx.deterministic_mode {
        passing.push("rule:deterministic_mode:producer ran in deterministic/dry-run mode".to_string());
    } else {
        blocking.push(
    ...
    if ctx.protected_source_or_target {
        ...
        passing.push("rule:no_protected_source_or_target:neither side is on the protected list".to_string());
    }
    ...
    // `Manual`.)
    passing.push("rule:caller_opt_in:caller explicitly supplied review_automation_policy".to_string());
...
pub(crate) fn stamp_review_automation_payload(
    ...
    if let Some(d) = outcome.suggested_decision {
        map.insert(
            "suggested_review_decision".to_string(),
            json!(d.as_str()),
        );
    }
    map.insert(
        "automation_reasons".to_string(),
        json!(outcome.reasons),
    );
}
