//! Regression tests for the review_gate facade and its split submodules.
...
fn artifact_id_without_topic_falls_back_to_legacy_layout() {
    ...
    let id2 = derive_review_question_id_for_artifact("plan", "p1", 2, "approve", Some("   "));
...
fn plan_node_review_id_uses_plan_scope_and_topic_hash() {
    let id =
        derive_plan_node_review_question_id("00000000-0000-0000-0000-000000000abc", 3, "n1", None);
    // scope=plan → wave-14 supported scope; topic-hash suffix folds in node_id.
    assert!(
        id.starts_with("review:plan:00000000-0000-0000-0000-000000000abc:v3:plan-node:"),
...
fn artifact_id_changes_when_topic_changes() {
    let a =
        derive_review_question_id_for_artifact("directive", "abc", 1, "compile", Some("topic-a"));
    let b =
        derive_review_question_id_for_artifact("directive", "abc", 1, "compile", Some("topic-b"));
...
fn auto_emit_file_not_written_records_warning_without_publishing() {
    ...
    assert_eq!(p["review_gate_policy"], "emit_question");
    assert_eq!(
        p["review_question_warning"]["code"],
        "FILE_WRITE_NOT_SUCCESSFUL"
    );
}
...
fn auto_emit_explicit_id_override_wins_over_derived() {
    ...
    let derived =
        derive_review_question_id_for_artifact("plan", "p1", 1, "compile", Some("/some/file"));
...
fn auto_emit_decision_variants_are_distinct() {
    ...
    assert_ne!(AutoEmitDecision::Emitted, AutoEmitDecision::EmitFailedBus);
}
...
fn decision_parse_accepts_canonical_strings() {
    assert_eq!(
        ReviewDecision::parse("approved").unwrap(),
...
fn decision_parse_accepts_short_aliases() {
    ...
    assert_eq!(
        ReviewDecision::parse("reject").unwrap(),
        ReviewDecision::Rejected
    );
    assert_eq!(
        ReviewDecision::parse("changes").unwrap(),
        ReviewDecision::NeedsChanges
    );
}
...
fn parse_qid_with_topic_hash_layout() {
    let p = parse_review_question_id_struct("review:plan:p-7:v3:compile:abcdef0123456789").unwrap();
...
fn parse_qid_round_trips_against_derive() {
    let original =
        derive_review_question_id_for_artifact("directive", "abc", 7, "compile", Some("topic-foo"));
...
fn validate_envelope_rejects_unsupported_scope() {
    ...
    let err =
        validate_review_resolution_envelope(&parsed, "worker", "abc", 1, &["approve"]).unwrap_err();
...
fn validate_envelope_rejects_artifact_id_mismatch() {
    ...
    let err = validate_review_resolution_envelope(&parsed, "directive", "abc", 1, &["approve"])
        .unwrap_err();
...
fn validate_envelope_rejects_stale_version() {
    ...
    let parsed = make_parsed("directive", "abc", 1, "approve");
    let err = validate_review_resolution_envelope(&parsed, "directive", "abc", 2, &["approve"])
        .unwrap_err();
...
fn subscriber_resolution_string_approve_synonyms_collapse_to_approved() {
    for raw in [
        "approved", "approve", "yes", "accepted", "Approved", "  YES  ",
    ] {
        assert_eq!(
...
fn dispatch_ignores_unknown_resolution() {
    let d = plan_review_resolved_dispatch("review:directive:abc:v1:approve", "deferred");
    match d {
...
fn dispatch_routes_directive_approved() {
    let d = plan_review_resolved_dispatch("review:directive:abc-123:v1:approve", "approved");
    match d {
        ReviewResolvedDispatch::Route { parsed, decision } => {
...
fn dispatch_routes_plan_rejected_via_synonym() {
    let d = plan_review_resolved_dispatch("review:plan:9f3c:v2:supersede", "no");
    match d {
        ReviewResolvedDispatch::Route { parsed, decision } => {
            assert_eq!(parsed.scope, "plan");
...
fn dispatch_routes_workflow_needs_changes_via_synonym() {
    let d =
        plan_review_resolved_dispatch("review:workflow:methodology-deploy-v0:v1:compile", "fix");
    match d {
        ReviewResolvedDispatch::Route { parsed, decision } => {
...
fn evaluate_manual_returns_not_evaluated_with_empty_block() {
    let outcome = evaluate_review_automation(ReviewAutomationPolicy::Manual, &safe_ctx(), None);
...
fn evaluate_suggest_returns_suggestion_without_mutation() {
    let outcome = evaluate_review_automation(ReviewAutomationPolicy::Suggest, &safe_ctx(), None);
...
fn evaluate_auto_safe_approves_when_every_rule_passes() {
    let outcome = evaluate_review_automation(ReviewAutomationPolicy::AutoSafe, &safe_ctx(), None);
...
fn evaluate_auto_safe_blocked_when_protected_source_or_target() {
    ...
    let outcome = evaluate_review_automation(ReviewAutomationPolicy::AutoSafe, &ctx, None);
...
fn evaluate_auto_safe_passes_when_no_file_write_attempted() {
    ...
    assert_eq!(outcome.status, AutomationStatus::AutoApproved);
    assert!(outcome.may_auto_resolve);
    assert!(outcome.reasons.iter().any(|r| r.contains("no_file_write")));
}
...
fn parse_auto_answer_policy_default_is_off() {
    assert_eq!(parse_auto_answer_policy(&json!({})), AutoAnswerPolicy::Off);
}
...
fn auto_answer_status_label_round_trips() {
    ...
    assert_eq!(AutoAnswerStatus::DryRunPreview.as_str(), "dry_run_preview");
}
...
fn evaluate_auto_answer_off_returns_not_evaluated_with_empty_block() {
    let outcome = evaluate_auto_answer_policy(AutoAnswerPolicy::Off, &safe_ctx(), "approve", None);
...
fn evaluate_auto_answer_deterministic_safe_blocks_destructive_archive() {
    ...
    assert_eq!(outcome.status, AutoAnswerStatus::SkippedDestructiveAction);
    assert!(outcome.requires_human);
    assert!(outcome
        .safety_rule_results
        .iter()
...
fn evaluate_auto_answer_deterministic_safe_blocked_when_protected_source_target() {
    ...
    ctx.protected_source_or_target = true;
    let outcome =
        evaluate_auto_answer_policy(AutoAnswerPolicy::DeterministicSafe, &ctx, "approve", None);
...
fn evaluate_auto_answer_dry_run_always_defers_even_on_safe_inputs() {
    ...
    let outcome =
        evaluate_auto_answer_policy(AutoAnswerPolicy::DryRun, &safe_ctx(), "approve", None);
...
fn evaluate_auto_answer_dry_run_preview_for_destructive_still_surfaces_rule() {
    ...
    let outcome =
        evaluate_auto_answer_policy(AutoAnswerPolicy::DryRun, &safe_ctx(), "supersede", None);
    assert_eq!(outcome.status, AutoAnswerStatus::DryRunPreview);
    assert!(outcome.requires_human);
    assert!(outcome
        .safety_rule_results
...
fn evaluate_auto_answer_never_promotes_destructive_actions_invariant_i2() {
    ...
    for action in DESTRUCTIVE_REVIEW_ACTIONS {
        ...
        let dry = evaluate_auto_answer_policy(AutoAnswerPolicy::DryRun, &safe_ctx(), action, None);
...
fn auto_approve_mode_recognises_off_blank_and_hyphen() {
    ...
    let hyphen =
        parse_llm_auto_approve_proposal_mode(&json!({"auto_approve_mode": "  Sonnet-Suggest  "}))
...
fn proposal_status_label_round_trip() {
    assert_eq!(
        LlmAutoApproveProposalStatus::NotInvoked.as_str(),
        "not_invoked"
    );
    assert_eq!(
        LlmAutoApproveProposalStatus::Unavailable.as_str(),
        "llm_unavailable"
    );
    assert_eq!(
        LlmAutoApproveProposalStatus::Suggested.as_str(),
        "suggested"
    );
    assert_eq!(
    ...
    assert_eq!(
        LlmAutoApproveProposalStatus::NoSuggestion.as_str(),
        "no_suggestion"
    );
}
...
fn parse_well_formed_proposal_returns_proposal_no_warnings() {
    ...
    assert!(
        warnings.is_empty(),
        "well-formed proposal must not warn: {:?}",
        warnings
    );
}
...
fn bundle_unavailable_pins_reason_and_caller() {
    ...
    assert!(b.unavailable_reason.as_deref().unwrap().contains("Sonnet"));
    assert_eq!(
        b.request_caller.as_deref(),
        Some("directive_review_proposer")
    );
...
fn stamp_proposal_payload_round_trip() {
    ...
    assert_eq!(payload["llm_auto_approve_proposal_warnings"], json!(["w1"]));
    assert_eq!(
        payload["llm_auto_approve_proposal"]["applied"], false,
        "invariant I3: applied always false"
    );
    assert_eq!(
        payload["llm_auto_approve_proposal"]["requires_human"], true,
        "invariant I3: requires_human always true in v0"
    );
    assert_eq!(
        payload["llm_auto_approve_proposal"]["decision"], "approved",
...
fn well_formed_high_confidence_proposal() -> LlmAutoApproveProposal {
    LlmAutoApproveProposal {
        ...
        destructive_check: "non_destructive:`approve` is not on the destructive list".to_string(),
        requires_human: true,
    }
}
...
fn apply_gate_parse_input_accepts_full_opt_in() {
    ...
    assert!(input.caller_approved);
    assert_eq!(
        input.proposal_hash.as_deref(),
        Some("deadbeef".repeat(4).as_str())
    );
    assert!(input.explicit);
}
...
fn apply_gate_all_six_gates_pass_applies() {
    let bundle = suggested_bundle(well_formed_high_confidence_proposal());
    let hash = compute_proposal_hash("approve", "abc-123", 1, bundle.proposal.as_ref().unwrap());
...
fn apply_gate_invariant_i1_never_applies_needs_changes() {
    ...
    assert!(
        !outcome.status.should_apply(),
        "I1: never auto-anything-non-approve"
    );
...
fn apply_gate_invariant_i2_destructive_archive_skipped() {
    ...
    let bundle = suggested_bundle(well_formed_high_confidence_proposal());
    let hash = compute_proposal_hash("archive", "abc-123", 1, bundle.proposal.as_ref().unwrap());
    let input = LlmApproveApplyGateInput {
        apply: true,
        proposal_hash: Some(hash),
        caller_approved: true,
    ...
    assert!(
        !outcome.status.should_apply(),
        "I2: destructive never auto-promoted"
    );
}
...
fn apply_gate_invariant_i2_destructive_supersede_skipped() {
    let bundle = suggested_bundle(well_formed_high_confidence_proposal());
    let hash = compute_proposal_hash("supersede", "abc-123", 1, bundle.proposal.as_ref().unwrap());
    let input = LlmApproveApplyGateInput {
        apply: true,
        proposal_hash: Some(hash),
        caller_approved: true,
...
fn apply_gate_invariant_i2_destructive_remove_skipped() {
    let bundle = suggested_bundle(well_formed_high_confidence_proposal());
    let hash = compute_proposal_hash("remove", "abc-123", 1, bundle.proposal.as_ref().unwrap());
    let input = LlmApproveApplyGateInput {
        apply: true,
        proposal_hash: Some(hash),
        caller_approved: true,
...
fn apply_gate_skips_when_caller_approved_false() {
    ...
    assert_eq!(
        outcome.status,
        LlmApproveApplyStatus::SkippedCallerNotApproved
    );
}
...
fn apply_gate_skips_when_confidence_medium() {
    ...
    assert_eq!(
        outcome.status,
        LlmApproveApplyStatus::SkippedConfidenceTooLow
    );
}
...
fn apply_gate_preflight_requires_proposal_hash_under_apply_true() {
    ...
    let err = enforce_apply_gate_preflight(&input, &bundle, "approve", "abc-123", 1).unwrap_err();
...
fn smoke_wave22_07_review_apply_gate_rejects_missing_hash_accepts_fixture_hash() {
    ...
    let canonical =
        compute_proposal_hash("approve", "abc-123", 1, bundle.proposal.as_ref().unwrap());
    ...
    let outcome = evaluate_llm_approve_apply_gate(&valid_input, &bundle, "approve", "abc-123", 1);
    assert_eq!(
        outcome.status,
...
fn smoke_wave22_07_review_apply_gate_pins_wave21_06_five_invariants() {
    ...
    let outcome = evaluate_llm_approve_apply_gate(&nc_input, &nc_bundle, "approve", "abc-123", 1);
    assert_eq!(
        outcome.status,
        LlmApproveApplyStatus::SkippedNonApprovedDecision,
    ...
    for destructive in ["archive", "supersede", "remove"] {
        let bundle = suggested_bundle(well_formed_high_confidence_proposal());
        let hash =
            compute_proposal_hash(destructive, "abc-123", 1, bundle.proposal.as_ref().unwrap());
        let input = LlmApproveApplyGateInput {
            apply: true,
            proposal_hash: Some(hash),
            caller_approved: true,
        ...
        let outcome = evaluate_llm_approve_apply_gate(&input, &bundle, destructive, "abc-123", 1);
        assert_eq!(
            outcome.status,
            LlmApproveApplyStatus::SkippedDestructiveAction,
    ...
    let outcome =
        evaluate_llm_approve_apply_gate(&unavail_input, &unavailable, "approve", "abc-123", 1);
    assert_eq!(
        outcome.status,
    ...
    let outcome =
        evaluate_llm_approve_apply_gate(&lied_input, &lied_bundle, "archive", "abc-123", 1);
    assert_eq!(
        outcome.status,
        LlmApproveApplyStatus::SkippedDestructiveAction,
