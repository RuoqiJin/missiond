...
async fn collect_flow_usage(state: &AppState) -> Result<HashMap<String, FlowStats>> {
    ...
    for t in tasks {
        ...
        if entry
            .last_used_at
            .as_deref()
            .map_or(true, |cur| cur < last.as_str())
        {
...
fn classify_flow(stats: &FlowStats, registered: bool, protected: bool) -> (String, Vec<String>) {
    ...
    if !registered && stats.total > 0 {
        evidence
            .push("flow_template fired but YAML missing under $MISSIOND_HOME/flows".to_string());
        return ("shadowed-by-better-capability".to_string(), evidence);
    }
    if registered && stats.total == 0 {
...
fn build_source_coverage_dict(
    ...
    let workflow_note = match (&workflow_probe.error, workflow_probe.workflows_observed) {
        ...
        (None, 0) => {
            "no rows in workflow table (DirectiveLayerStore::workflow_list_top_n)".to_string()
        }
...
async fn action_candidates(state: &AppState, args: &Value) -> Result<ToolResult> {
    ...
    let merge_candidate_count = buckets.get("merge-candidate").map(|v| v.len()).unwrap_or(0);
...
async fn action_mark(state: &AppState, args: &Value) -> Result<ToolResult> {
    ...
    if protected
        && matches!(
            decision,
            "deprecate" | "remove-after-compat-window" | "merge"
        )
    {
        return Ok(ToolResult::structured_error(
            ToolError::new(
...
fn parse_lisp_hints(text: &str, file_label: &str, default_kind: &str) -> Vec<SemanticHint> {
    ...
    let dispatch_pref = DISPATCH_PREFERRED
        .get_or_init(|| Regex::new(r#":workstation-dispatch-role\s+"(preferred[^"]*)""#).unwrap());
    let replacement_kv =
        REPLACEMENT_KV.get_or_init(|| Regex::new(r#":replacement\s+"([A-Za-z0-9_-]+)""#).unwrap());
    let supersedes_kv =
        SUPERSEDES_KV.get_or_init(|| Regex::new(r#":supersedes\s+"([A-Za-z0-9_-]+)""#).unwrap());
    let f_flow_ref = F_FLOW_REF.get_or_init(|| Regex::new(r"(F-[A-Za-z][A-Za-z0-9_-]+)").unwrap());
...
fn apply_hint_overlay(
    ...
    let Some(target_hint) = pick_merge_target(hints, source_id, registered_tools, registered_flows)
    else {
    ...
    let semantic_hint_source = format!("{} {}", target_hint.file_anchor, target_hint.token);
    HintOverlay {
    ...
    let rows = match state.store.workflow_list_top_n(WORKFLOW_PROBE_LIMIT).await {
        ...
        Err(e) => {
            probe.error = Some(format!(
                "DirectiveLayerStore::workflow_list_top_n failed: {}",
                e
            ));
            return probe;
        }
...
"#;
    ...
    fn apply_overlay_promotes_quiet_to_merge_candidate_with_resolved_target() {
        ...
        assert!(overlay
            .evidence
            .iter()
            .any(|e| e.contains("merge-candidate:")));
    ...
    fn apply_overlay_protected_target_keeps_merge_with_review_evidence() {
        ...
        assert!(
            overlay
                .evidence
                .iter()
                .any(|e| e.contains("protected; mark only as review")),
    ...
    fn apply_overlay_emits_missing_target_when_hint_unresolved() {
        ...
        assert!(overlay
            .evidence
            .iter()
            .any(|e| e == "semantic_hint_missing_target"));
        assert_eq!(overlay.hint_evidence.len(), 1);
    }
    ...
    fn apply_overlay_consolidated_keep_suppresses_merge_candidacy() {
        ...
        assert!(overlay
            .evidence
            .iter()
            .any(|e| e.contains("consolidated-keep")));
    }
    ...
    fn workflow_stats_lane_unavailable_when_probe_errors() {
        ...
        let dict =
            build_source_coverage_dict(&[], &hint_index, &event_probe, &workflow_probe, Some(0));
        let lane = dict
    ...
    fn workflow_stats_lane_empty_when_no_rows() {
        ...
        let dict =
            build_source_coverage_dict(&[], &hint_index, &event_probe, &workflow_probe, None);
        let lane = dict.get("workflow_execution_stats").unwrap();
