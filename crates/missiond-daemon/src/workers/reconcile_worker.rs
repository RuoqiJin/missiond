...
async fn run_reconciliation(state: &AppState) {
    ...
    for file_path in &files {
        ...
        // Reconcile the gap
        let recovered = reconcile_file_gap(state, &path_str, &session_id, last_size).await;
...
async fn reconcile_file_gap(
    ...
    // Ensure conversation record exists (INSERT ON CONFLICT DO NOTHING — don't overwrite
    // existing conversation_type/chat_type which were set correctly by the normal pipeline)
    let project_path = Path::new(jsonl_path)
        .parent()
        .map(|p| p.to_string_lossy().to_string())
        .unwrap_or_default();
    let _ = state.store.ensure_conversation_exists(
        session_id, &project_path, jsonl_path,
    ).await;
